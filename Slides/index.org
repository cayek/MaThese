...
#+BEGIN_NOTES
L'ADN une très longue séquence de nucléotide
Les séquenceurs permettent de savoir pour chaque individu sa séquence nuclotyde.
On s'intéresse aux endroits du génome ou on a pu observé un polymorphisme entre
les individus entre les individu. 
CAD que à une position données de l'ADN tout les individus n'on pas le même
nucléotyde.
Ce sont ces positions ou on pu observé des allèle différent entre les individus
qui nous interesse.
Ce qu'on appel allèle est un variant d'un nucléotyde 
...
\begin{equation*}
\mathcal{L}(\Q, \mathbf{G}) =   \|  {\bf Y} - {\bf QG}^T \|^2_{\rm F} +  \alpha {\rm Tr} (\Q^{T} \bm{\Gamma} \Q)
\end{equation*}
