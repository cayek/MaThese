...
La première étape de l'algorithme consiste à faire une régression régularisé en
...
cite:Tseng_2001 dépasse largement le cadre de la convergence de l'algorithme
d'estimation $L_{1}$ ce qui complique l'extraction des résultats intéressants.
Pour facilité la compréhension, nous proposons de démontrer un théorème plus
faible qui s'applique directement à notre cas. Pour cela nous introduisons la
fonction $f$ définie sur son domaine
\begin{equation}
\label{eq:domf}
A = A_{1} \times A_{2} \times ... \times A_{m}
\end{equation}
un produit Cartésien d'ensemble fermé et convexe. L'algorithme de descente par
bloque de coordonnées est définie par la formule de récurrence suivante :
\begin{equation}
\label{eq:blokAlgo}
...
l'algorithme de descente par bloque de coordonnées dans le cas la fonction
objectif est différentiable, nous démontrons la proposition suivante :
#+BEGIN_theorem 
...
convexes. Soit $\{x^{k}\}$ la séquence généré par eqref:eq:blokAlgo. Alors tout
...
\end{align}
où $\nabla g(\bar{x})$ et $\nabla_{i} g(\bar{x})$ sont respectivement la dérivé
et la dérivé le long de la $i\mathrm{-ieme}$ variable de $g$ en $\bar{x}$. Or
...
& \geq 0
\end{align}
où $s_{i}$ est une sous dérivé de la fonction convexe $f_{i}$. L'équation
eqref:eq:lassoProof2 nous permet de dire qu'il existe une sous dérivé $s_{i}$ tel
que 
\begin{equation}
\label{eq:3}
(\nabla_{i} g(\bar{x}) + s_{i}) = 0
\end{equation}
d'où l'équation eqref:eq:lassoProof3.
Nous avons donc en utilisant eqref:eq:lassoProof3 et eqref:eq:lasso1 que pour tout $x \in A$
\begin{equation}
f(x) - f(\bar{x}) \geq 0.
\end{equation}
#+END_proof
...
Jusque ici, nous avons seulement abordé l'estimation des paramètre de LFMM.
Cependant, l'objectif initial est de trouver la liste des variables expliquées
associées aux variables explicatives tout en prenant en compte les variables
latentes. Nous présentons dans cette partie un test d'hypothèse corrigé pour les
variables latentes.
