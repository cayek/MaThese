...
"variables explicatives", les variables qui sont explicitement modélisées dans
l'analyse et pour lesquelles les corrélations avec les variables que l'on
souhaite expliquer sont étudiées. Les variables explicatives correspondent
...
variables explicatives et aux variables expliquées (fréquences de SNP, expression
génique ou profils de méthylation de l'ADN). Ainsi un facteur latent devient un
facteur de confusion pour une étude d'association quand il y a un lien de
causalité entre les facteurs latents et les variables expliquées, entre les
variables explicatives et les facteurs latents, mais qu'il n'y a pas relation de
causalité en les variables expliquées et explicatives, cette situation est résumé
pas la figure [[graph:conf_factor]]. Si les facteurs de confusion ne sont pris en
compte dans l'étude d'association cela peut induire de la corrélation entre les
statistiques de test utilisées pour détecter l'association. Mais surtout les
facteurs de confusion peuvent faire apparaître certaines variables expliquées
comme étant significativement corrélées avec les variables cause à cause du
facteur de confusion qui n'est pas pris en compte, nous illustons cette
situation dans la partie [[sec:simu_ex]]. Le problème des facteurs de confusion est
connue depuis longtemps. En effet, on le retrouve déjà dans l'ouvrage /The
Design of Experiement/ de Ronald Fisher qui introduisit entre autre le concept
de d'hypothèse nulle en statistique cite:fisher1937design. Depuis, les facteurs
de confusion ont reçu différentes dénominations comme celle des "facteurs
latents" (la dénomination que nous utilisons ici)
...
explicatives sur les variables expliquées. La matrice $\E$ est la matrice de
l'erreur résiduelle. Enfin la fonction $f$ est la fonction de lien entre les
variables expliquées et les variables explicatives. L'objectif est de trouver
les coefficients $b_{j,l}$ qui sont significativement différents de zéro. Dans
ce cas cela veut dire que la variable expliquée $j$ est potentiellement associée
à la variable explicative $l$. Comme nous l'avons évoqué dans la partie
précédente avec ce genre d'approche il est possible qu'une variable non observée
soit corrélé à la fois aux variables expliquées et aux variables explicatives.
Dans ce cas on pourrait une situation un très grand nombre de variables
expliquées sont significativement corrélées avec les variables explicatives
(voir partie [[sec:simu_conf]] pour une simulation de cette situation). Nous allons
maintenant présenter les différentes approches qui prennent en compte les
facteurs de confusion.

**** Estimation des facteurs latent à priori
Une première approche consiste à trouver les variables latents a partir des
variables expliquée $\Y$ et sans prendre en compte les variables explicatives.
Les variables latents sont ensuite ajoutées au modèle de régression au coté des
variables explicatives 
\begin{equation}
\Y = f(\X \B^{T} + \bar{\U} \V ) + \E
\end{equation}
où $\bare{\U}$ est la matrice des variables latents calculé à priori. C'est une
technique très utilisé en association génome-phénotype. Par exemple la méthode
EIGENSTRAT calcule les variables latentes à l'aide de l'analyse en composante
principales des données génétiques cite:Price_2006. En effet dans les GWAS la
structure de population est connue pour être un facteur de confusion et les
premiers axes principaux retourné par l'ACP explique cette structure de
population. Plus récemment le même principe à été utilisé pour faire
un étude d'association entre des données de méthylation de l'adn et la maladie
polyartrite rumatoide cite:Rahmani_2016. Dans ce cas les premiers axes
principaux expliquent le type cellulaire dont on sait qu'il est un facteur de
confusion pour les EWAS.
**** Les modèles mixes
...
largement utilisé dans les GWAS cite:Kang_2008,Zhou_2014. Dans ce cas des GWAS,
l'effet aléatoire permet d'expliquer la variance qui est du à la structure de
