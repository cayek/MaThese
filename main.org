...
#+begin_src R
  ...
  ## sampler
  Y <- "./Data/ThesisDataset/3Article/1000GenomesPhase3/G_EAS.rds"
  X <- readRDS("./Data/ThesisDataset/3Article/1000GenomesPhase3/X_EAS.rds")
  X <- X[,1, drop = FALSE]
  head(X)
...
#+begin_src R
  ...
  ## run lm
  dat.clumped <- MatrixFactorizationR::LfmmDat(Y = Y, X = X, missing.ind = c())
  m.lm <- ExpRmouline(m.lm, dat.clumped)
  df.lm <- ExpRextractor_celiac(dat.clumped, m.lm, 1, 1)
  ## save expr
  save_expr(df.lm, "celiac_lm.rds")

  ## run ridgeLfmm
  m <- m.ridgeLfmm
  dat.clumped <- MatrixFactorizationR::LfmmDat(Y = Y, X = X, missing.ind = c())
  m <- ExpRmouline(m, dat.clumped)
  df <- ExpRextractor_celiac(dat, m., 1, 1)
  ## save expr
  save_expr(df, "celiac_lm.rds")
