...
d'association sont faussé et on observe en général une augmentation du nombre de
gènes associés à la variable d'étude. Une telle situation n'est pas souhaitable
car bien qu'il agissent de corrélation il ne s'agit pas de corrélation
intéressante pour la problématique biologique. Dans les données données
génétique qui nous intéresse dans cette thèse, il est possible d'inférer les
facteurs responsable de la variation génétique. On appel de tel facteur des
facteurs de confusion pour le test d'association. Les facteurs de confusion
doivent être pris en compte dans les étude d'association pour éviter les fausses
associations.
...
pose de nouveau problème aux statisticien. Dans cette thèse nous nous sommes
intéressé à deux problématique en analyse de données génétiques. L'accent a été
mis sur la rapidité des méthodes développées afin qu'elles soient applicables
aux données génétiques modernes. En outre, afin que nos méthodes statistiques
soient utilisables par la communauté scientifique en génétique, il a été
important de rendre accessible une implémentation informatique efficace des
méthodes en question.

L'inférence à partir de données génétique d'une représentation synthétique
permettant de visualiser l'organisation en populations est une des première
étape de toutes les études de génétique des population. Nous avons développées
une méthodologie statistique permettant l'inférence rapide de la structure de
population à partir de données génétique et géographique.

Les études d'association à partir de données génétiques sont très utiles pour
identifier une liste restreinte de variable 

Nous avons développé une seconde méthode statistique permettant de réaliser des
test d'association. En particulier nous souhaitons résoudre le problème très
présent dans les étude d'association en génétiques des facteurs de confusions.

Nous présentons maintenant plus en détail chacun des cadres méthologiques
statistiques et en démontrons leurs utilités sur des simulation et exemples
concrets de la génétique.
...
proposons une méthode d'estimation de la structure dans des population
...
résolvent le problème de factorisation. Le première algorithme repose sur une
optimisation quadratique alterné (AQP pour alternated quadratic programing),
l'autre sur un algorithme des moindres carrés alternés projetés (APLS pour
alternated projected least square). Le terme alterné dans les deux algorithmes
fait référence au fait que l'on alterne une étape d'optimisation selon la
matrice de coefficient de métissage puis la matrice des fréquences de génotypes
ancestraux. Alors que les algorithmes AQP ont un fondement théorique bien établi
par cite:Bertsekas_1997, ce n'est pas le cas des algorithmes APLS. En utilisant
des simulations coalescentes, nous montrons que les estimations calculées par
l'algorithme APLS sont de bonnes approximations des solutions de l'algorithme
AQP. En outre, nous montrons que les performances de l'algorithme APLS s'élève
aux dimensions des ensembles de données modernes. Sur des simulations, nous
montrons que l'erreur statistique des estimateurs des coefficients de métissage
et des fréquences d'allèle dans les clusters génétique fournis par notre méthode
est du même que l'erreur obtenu avec le logicielle =TESS= 2.3, une méthode
bayésienne. Toujours sur des simulations nous montrons que notre méthode spatial
estime mieux la structure de population que la méthode sNMF qui repose aussi sur
un problème de factorisation de matrice mais n'utilise pas l'information
spatiale. Enfin, nous discutons de l'application de nos algorithmes aux données
des écotypes européens de {\ it Arabidopsis thaliana}, pour lesquelles des
données géographiques individuelles et géométriques sont disponibles
cite:Horton_2012.
