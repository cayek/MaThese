...
expériences que $\lambda$ petit donne les meilleurs résultats dans de nombreux
cas.
...
proportion de variable expliqué par $\X$. Pour trouver un lambda qui correspond a
cette proportion nous proposons une heuristique basé sur un chemin de
régularisation inspiré par cite:friedman10_regul_paths_gener_linear_model. 

Nous commençons par la plus petite valeur du paramètre de régularisation
$\lambda$ tel que le vecteur
\begin{equation}
\bar{\B}_{t} = (\X^{T} \X + \lambda \Id_{d})^{-1} \X^{T} (\matr{E}^{1}_{t = 1})
\end{equation}
vaut zéro. Ceci est le résultat de la première étape de l'algorithme
d'estimation des moindes carré régularisé en norme $L_{1}$ présenté dans la
partie [[sec:estimator_L1]]. Nous notons cette valeur $\lambda_{\mathrm{max}}$.
Ensuite, nous construisons une séquence $m$ valeurs de $\lambda$ décroissant
selon une échelle logarithme depuis $\lambda_{\mathrm{max}}$ jusqu'à
\begin{equation}
\lambda_{\mathrm{min}} = \epsilon \lambda_{\mathrm{max}}.
\end{equation}
...
latentes tout en prenant en compte des variable explicative. Cependant,
l'objectif initial est de trouver la liste des variables expliqué associé aux
variable explicative tout en prenant en compte les variables latentes. Nous
présentons dans cette partie un test d'hypothèse corrigé pour les variables
latentes.

Une méthode classique de correction des tests d'hypothèse pour les variables de
confusion est d'utiliser une estimation de celles-ci comme variable explicative
du modèle mis en place réaliser le test d'hypothèse. C'est une méthode très
courante dans les études d'associations en général
cite:Price_2006,Song_2015,article_Leek_Storey_2008,Rahmani_2016. Nous avons
choisi de réaliser un test d'hypothèse qui repose sur un modèle linéaire, mais
l'estimation des variables latentes peuvent être utilisé comme co-variable dans
n'importe quelle modèle statistique.
...
statistique .Le détail des calculs de la statistique de test est données dans la
section 3.2 de cite:Hastie_2009. La statistique de test suit sous lh'ypothèse
nulle le loi de student à $\Yrow - \K - 1$ degrés de liberté. On peut donc
calculer pour chaque variable expliqué $j$ une \pvalue $p_{j}$.


***** Calibration du test d'hypothèse
Les hypothèses faite pour calculé la \pvalue sont parfois fausses. Quand on
réalise un seul test d'hypothèse il n'est pas facile de se rendre Dans le
cadres des test d'hypothèse multiple On peut
trouver dans cite:Efron_2004 une listes des raisons 
