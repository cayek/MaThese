...
#+begin_src R  
  ...
  ## methods
  methods <- method_PCA(scale = FALSE, K = 500) * param(lambda = lambdas)
...
#+NAME: code:ewas_screeplot_plot
#+CAPTION: Dépend de [[code:ewas_screeplot]]
#+begin_src R 
  library(MaTheseR)
  MaTheseR.params <- get_MaTheseRparams()
...
\begin{figure}[!t]
\centering
\includegraphics[with=14cm, height=7cm]{./OUTPUT/Rplots/ewas_screeplot.pdf.png}
