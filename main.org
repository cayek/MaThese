...
ainsi leurs patrimoines génétiques. Quand on observe des les génomes d'individus
provenant de plusieurs populations différentes, on remarque des différences dans
les distributions génomiques des populations. Par exemple la Figure ? montre les
fréquences de distribution des allèle du gène ... dans des populations Europèen,
Africaine et Afro-americaine. En pratique la réalité est plus complexe que des
populations isolés qui ne s'échange pas d'individus. Le modèle de métissage en
génétiques des population s'suppose que les individus que l'on observe sont le
produit de la reproduction sexué entre des individus provenant de plusieurs
population que l'on appel ancestral. Connaissant les fréquences d'allèle dans
les populations ancestrales il est possible de mesurer pour chaque individu la
proportion de son génome qui provient de chaque population ancestrale. La Figure
... montre un example de mesure des coéficient d'une population Afroaméricaine
en supoosant que les population ancestral sont les population Européen et
Africaine. Bien souvent on ne connait pas les fréquence d'allèles dans les
populations ancestrals et toute la difficulter est de l'estimer conjointement
avec les coéficients de métissage.

La mesure des coefficient de métissage est une étape importante dans l'étude de
données génétique. 

Comprendre comment l'organisation en sous populations permet de vizualiser le
variation observé dans les données génétique afin d'étudier par exemple la démographie d'une
espece citer....

De plus nous somme obliger d'étudier la
variation génétique provoqué par la structuration en sous population quand on
veut comprehenssion les variations génétique provoqué par des pressions
envinomental. De même que la structure de population constitue une facteur de
confusion dans les étude d'association avec des phénotypes ou bien des maladies
cite the effect of human population structure on larger genetic associatio
suties ..., nous parlerons plus en détail de la notion de facteur de confusion
dans le chapitre suivant. Enfin, la comprehention de la structure de population
est très imporante en medical genetics. 
...
:END:
Le modèle du structure de populations que nous venons de décrire a été introduit
par cite:Pritchard2000 et le logiciel =structure=. Le modéle présenté dans
cite:Pritchard2000 est formalisé avec une approche bayesiennes. Les données de
plus en plus grands on provoquer l'emergence de plusieurs des algorithme plus
rapide que celui de =structure= ~ \ citep { Tang2005, Alexander2011,
Frichot2014, Raj2014}. Parmis les logicielles d'inférence de la structure de
population on les modèles basé sur un modèle probabiliste et des modèles basé
l'optimisation une fonction objective.

Parmi les approches basées sur un modèle probabiliste, une fonction de
vraisemblance est définie pour la matrice des coefficients de metissage et
l'estimation est effectuée en maximisant la fonction log-vraisemblance. Pour {\
tt structure} et les modèles connexes, les hypothèses du modèle incluent
l'équilibre de liaison et l'équilibre de Hardy-Weinberg dans les populations
ancestrales. La première amélioration de l'algorithme d'origine était basée sur
un algorithme EM ~ \ citep {Tang2005}, et des algorithmes de vraisemblance plus
récents sont implémentés dans les programmes {\ tt adjuvant} et {\ tt
faststructure} \ citep {Alexander2011, Raj2014} . Dans les approches sans
modèles probabiliste, les coefficients de métissage sont estimés à l'aide de
méthodes de moindres carrés ou d'analyse factorielle. Les méthodes sans modèle
probabiliste ne font aucune hypothèse sur les processus biologiques qui ont
généré les données. Pour estimer les matrices de métissage, \ cite
{Engelhardt2010} a proposé d'utiliser l'analyse des facteurs épars, \ cite
{Frichot2014} a utilisé des algorithmes de factorisation de matrice non négative
scindés et \ cite {Popescu2014} a utilisé l'analyse des composants principaux du
noyau. Les méthodes des moindres carrés permettent de reproduire avec précision
les résultats des approches de vraisemblance selon les hypothèses types de ces
méthodes. En outre, les méthodes sans modèle fournissent des approches qui sont
valides lorsque les hypothèses des approches de vraisemblance ne sont pas
respectées ~ \ citep {Frichot2014}. Les méthodes sans modèle probabiliste sont
généralement plus rapides que les méthodes basées sur un modèle probabiliste.
*** Méthodes d'estimation des coefficients de métissage à l'aide de données géographiques

Parmi les approches basées sur un modèle probabiliste de l'estimation des
coefficients de metissage, une importante classe de méthodes a amélioré le
modèle bayésien de {\ tt structure} en incorporant des données géographiques à
travers la loi de distribution à prorie des coefficient de metissage ~ \ citep
{Chen2007, Corander2008}. Sous les modèles d'isolation par distance ~ \ citep
{Wright1943, Malecot1948}, les algorithmes spatiaux fournissent des estimations
de structure de population plus robustes que des algorithmes non spatiaux qui
peuvent conduire à des estimations biaisées du nombre de clusters ~ \ citep
{Durand2009}. Certaines méthodes bayésiennes sont basées sur des algorithmes de
Monte Carlo de chaîne de Markov qui nécessite beaucoup de calcul ~ \ citep
{Francois2010}. Les efforts récents visant à améliorer l'inférence des coefficients
ancestrales dans un contexte géographique ont principalement porté sur la
localisation des ancêtres récents ~ \ citep {Baran2013, Lao2014, Yang2014,
bhaskar2016, ranola2014}. Dans ces applications, l'information spatiale est
utilisée dans un cadre prédictif qui attribue aux ancêtres des origines
géographiques supposé. 

En gros il faut que je dise que ma méthode faire comme sNMF pour structure
c'est quoi ce bradburd2016 ????, j'ai l'impression que c'est un peu différent. 

Alors que l'estimation géographique rapide des
proportions d'ascendance individuelle a été proposée précédemment ~ \ citep
{Caye2016, bradburd2016}, il est de plus en plus nécessaire de développer des
algorithmes d'estimation d'ascendance individuels qui réduisent le coût de
calcul dans un cadre géographiquement explicite.

*** Plan du chapitre
** Nouvelles méthodes
Dans cette section, nous présentons deux nouveaux algorithmes pour estimer les
coefficients de metisse individuels et les fréquences génotypiques ancestrales
en supposant $\K$ populations ancestrales. En plus des génotypes, les nouveaux
algorithmes nécessitent les coordonnées géographiques de chaque individu.
*** Matrices $\Q$ et $\G$
Nous considérons une matrice de génotypique, $\Y$, enregistrant des données pour
$n$ individus à des loci polymorphes $ L $ pour une espèce ayant une ploidy de
$d$. 


Pour les SNP autosomiques dans un organisme diploïde, le génotype au locus
$ \ ell $ est un nombre entier, 0, 1 ou 2, correspondant au nombre d'allèles de
référence à ce lieu. Dans nos algorithmes, des formes disjonctives sont
utilisées pour coder chaque valeur génotypique comme indicateur d'un
hétérozygote ou d'un locus homozygote (Frichot et al., 2014). Pour un organisme
diploïde, chaque valeur génotypique $, 0,1,2 $ est encodée comme $ 100 $, $ 010
$ et $ 001 $. Pour les organismes dédiés $ p $, il existe des valeurs
génotypiques $ (p + 1) $ possibles à chaque locus, et chaque valeur correspond à
une forme disjonctif unique. Bien que notre attention soit centrée sur les SNP,
les algorithmes présentés dans cette section s'étendent aux loci multialaciques
sans perte de généralité. En outre, la méthode peut être facilement étendue aux
probabilités génotypiques en utilisant la probabilité de coder chaque valeur
génotypique ~ \ citep {Korneliussen2014}.
*** Problèmes d'optimisation des moindres carrés
*** Choix des hyperparamètre
*** Statistique de différentiation des populations ancestrales pour les cribles génomique pour l'adaptation locale
...
:header-args: :cache no :eval no-export :results output :exports none :session *krakR* :dir /scp:cayek@krakenator:~/Projects/Thesis/MaThese/
:END:
*** Comparaison avec TESS 2.3
*** Erreur statistique
*** Bénéfice apporté par l'ajout de l'information spatiale
*** Sensibilité des estimateurs aux mesure spatiales
