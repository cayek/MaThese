...
comment en tirer l'information qui permettra de mieux comprendre le monde qui
...
environnement. La sélection est l'hypothèse centrale de la théorie de Darwin,
...
(single-nucleotide polymorphism) qui sont les polymorphismes génétiques d'un
seul nucleotide. Un SNP est la variation (polymorphisme) d'une seule paire de
bases du génome, entre individus d'une même espèce (Figure ref:fig:SNP). Les
SNPs représentent $90 \%$ de l'ensemble des variations génétiques humaines, et
des SNPs avec une fréquence allélique supérieure à $1 \%$ sont présents toutes
les cent à trois cents paires de bases en moyenne dans le génome humain. Nous
appelons locus une position sur l'ADN, nous parlons ainsi du locus d'un SNP.
Nous représentons les données génétique comme la matrice contenant le nombre de
fois que l'allèle muté a été observé pour chaque individu et chaque locus
...
\begin{figure}[!h]
  ...
  ADNs \left \{\begin{tabular}{cccccccc}
                ...
                \cdots &C & T & \cellcolor{blue!25} A & G &G& \cdots & \cdots \\[14pt]
                \cdots &G& A & \cellcolor{blue!25} G & C &C& \cdots & \cdots\\
                \cdots &C& T & \cellcolor{blue!25} C & G &G& \cdots & \cdots 
              \end{tabular}
              
              \caption{{\bf Illustration d'un SNP.} La paire de base différente
                entre les deux séquences est un SNP.}
\label{fig:SNP}
\end{figure}
#+END_EXPORT
...
Pour modéliser la structure génétique de population, nous supposons que le
génome de chaque individu est la combinaison de morceau de génome provenant de
$K$ clusters génétiques (aussi appelé populations ancestrales). Dans chaque cluster
...
selon chacune des coordonnées de la fonction à optimiser (Figure
ref:fig:coordinate_descente).
...
longitudinal cite:Durand_2009,FRAN_OIS_2010 (Figure ref:tess3:cline). Dans ce
...
\caption{{\bf Simulation de génotype métisser spatialement.} Les populations
  sources sont simulé à l'aide du programme ms pour obtenir les matrice de
  fréquence de génotype $\G_1$ et $\G_2$. La matrice $\Y$ est générée en tirant
  des gène des deux populations avec une probabilité généré selon un gradient
  longitudinal. Les probabilitées sont stoquées dans la matrice $\Q$.}
\label{tess3:cline}
\end{figure}
#+END_EXPORT
...
#+begin_src shell
  ...
  do
      basefile=`basename $file .bed`
      echo "===== $basefile ====="
      plink --bfile $basefile --threads 16 --maf 0.01 --mind 0.05 --geno 0.05 --snps-only --autosome --make-bed --out "$basefile"_CQ
  done
...
implémentations efficace de nos algorithmes sont disponibles dans deux packages
R : tess3r et lfmm. Nous proposons maintenant différente perspectives de nos
travaux.
...
est très vivante et en particulier en analyse de données biologique. Les
...
ailleurs, le language de programmation R est un language "haut niveau"
permettant une maintenance plus facile que sur des languages de programmation
bas niveau tel que le language C par exemple. Une perspective majeur de nos
travaux est de maintenir nos packages en fonction des mises à jour du langague R
ou bien des rapports d'erreur renvoyés par les utilisateurs.

Un autre aspect important du développement des packages est la mise en place
d'une documentation complète. Au même titre que le développement des fonctions
informatiques, la documentation doit évoluer avec les retours des utilisateurs.
Les fonctions des packages tess3r et lfmm sont documentées. Nous avons également
écrit des documentations plus longue proposant une vu d'ensemble des
fonctionnalités des packages. Dans l'écosystème R les documentations longues
sont appelées vignette. Il sera donc important de faire évoluer les
documentations des fonctions ainsi que leur vignette en fonction des retours des
utilisateurs.

Les packages tess3r et lfmm reposent sur des structures de données matricielles.
Nous avons choisi d'utiliser la structure matricielle classique du language R
afin d'etre le plus général possible. Un question majeur pour les utilisateurs
est de savoir comment charger les données en mémoire vive afin d'utiliser nos
outils. Certain format de fichier se sont imposé dans la communauté de la
génétique, comme par exemple le format du logiciel plink .bed et les .vcf.
Ajouter une option permettant au utilisateurs de fournir directement un fichier
formater dans un format standard serait un réelle avantage pour les
utilisateurs. En outre, certain format de fichier ont été pensé pour permette
une accès rapide aux données génétiques à la manière des algorithmes
onlines[fn:online]. C'est le cas par exemple du format de bed. Ne pas charger
...
algorithmes sur des grandes données.
...
des packages mais aussi sur les aspects plus généraliste d'analyse de données
avec nos outils.
...
Les données réelles analysées lors de cette thèse peuvent être qualifié de
données "propre". Dit autrement, les données contiennent peu d'erreur et peu de
valeurs manquantes. Cependant, il est fréquent sur les grands jeux de données
biologiques avec beaucoup variables qu'un certain nombre de variables ne soit pas
observé pour tous les échantillons. Ou bien qu'il y ai des erreurs dans les
variables observées. Il est fréquent par exemple que les données génétiques
comportent des erreurs de séquençage. Des données génétique de très mauvaises
qualités sont par exemple les données génétiques prélevées sur des espèces qui se
sont éteintes.
...
l'imputation des valeurs manquantes a été très étudiés cite:Browning_2016.
...
génomes complets pouvant servir de référence pour l'imputation cite:Howie_2012.
...
facteurs comme l'ACP cite:josse2009gestion.
...
On attribue parfois un score sur la confiance sur la mesure expérimental des
génomes. Une méthode simple consiste à filtrer les observations ayant un score
trop faible pour les traiter comme des données manquantes. Le problème est
autrement plus complexe lorsque que nous ne somme pas capable de détecter les
erreurs de mesure présentes dans les données biologiques. Nous avons évalué,
dans le chapitre [[chap:tess3]], l'effet du bruit dans les données géographiques sur
les résultats de nos algorithmes spatiaux d'estimation de l'ascendance.
Cependant, nous pouvons nous interroger sur l'effets des valeurs aberrantes
présentes dans les données génétiques. Ainsi que comment les erreurs de mesure
peuvent influencer les études d'association faite à partir de notre package
lfmm.
...
En théorie les modèle mixe c'est en n^2p pour calculer la matrice de kindchip.
lfmm c'est du npk !!
*** Général
