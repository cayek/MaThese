...
est la matrice des coordonnées dans la base des facteurs latents et la matrice
$\V$ représente les axes de des facteurs latents. Enfin la matrice $\E$ est la
matrice d'erreur résiduelle de taille $\Yrow\times\Ycol$.

De même que dans cite:tipping1999probabilistic, nous supposons que le bruit $\E$
dans eref:eq:model est Gaussien et isotrope
$$
\Normal(0, \sigma^{2} \Id_{\Ycol})
$$

où $\sigma^{2}$ est la variance de l'erreur et $\Id_{\Ycol}$. Ainsi, l'opposé de la
log-vraisemblance fait intervenir la fonction objectif
\begin{equation}
...
#+begin_src R :results output :exports both
  ...
  samplers <-  s * param(prop.outlier = c(0.01, 0.05, 0.1, 0.2, 0.3))
