...
#+CAPTION: Depend de [[code:num_val_expr]]
#+begin_src R
  ## Compute plot !
  require(MaTheseR)
...
#+NAME: code:num_val_plotbar
#+CAPTION: Dépend de [[code:num_val_auc_gif_df]
#+begin_src R :session *krakR* :results output :dir /scp:cayek@krakenator:~/Projects/Thesis/MaThese/
  ...
  pl <- ggplot(toplot, aes(x = as.factor(prop.outlier), y = auc.mean, fill = method)) +
