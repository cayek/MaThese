...
#+begin_src R
  ...
  ## C
  h1.gif.pl <- plot_gif(toplot.h1, "proportion of H1") +
...
#+begin_src R 
  ...
  ## AUC By prop of outlier
  toplot.p.auc <- expr$df.res %>%
    dplyr::filter(pvalue.index == "pvalue1") %>%
    dplyr::mutate(x = prop.outlier, method = as.factor(method)) %>%
    compute_auc()
  lm.p.auc <- lm(AUC~method, data = toplot.h1)
#+end_src
