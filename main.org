...
cite:simpson1951interpretation. Historiquement, les études de corrélation
...
l'analyse en composante principal (ACP). Dans une population d'individus
...
démographique des individus afro-américains cite:tishkoff2009genetic.
...
l'inférence repose sur des méthodes d'échantillonage de la loi a posteriori des
coefficients d'ascendance cite:Pritchard2000. D'autres méthodes visant à rendre
plus rapide l'inférence des matrice d'ascendance minimisent la fonction
log-vraisemblance des paramètres d'ascendance génétique
cite:Tang_2005,Alexander_2011,Raj_2014. Des méthodes très rapide ne reposant pas
sur une modélisation probabiliste ont aussi été proposées pour passer à
l'échelle des grands échantillons de données modernes
cite:Frichot_2014,Popescu_2014. Par ailleurs, de nombreuses méthodes utilisent
l'information spatial individuelle afin d'améliorer l'estimation de l'ascendance
génétique et de localiser les clusters génétiques dans l'espace. Des méthodes ont
incorporé l'information géographique au modèle bayesien de =structure=
cite:CHEN_2007,Corander2008. Cependant, ces méthodes bayésiennes ne passe pas a
...
plus en détail ce que sont les facteurs de confusions dans les études
d'associations.
...
Prendre en comprendre les facteurs latentes est un problème importants des
études d'associations. Certaines méthodes très efficace utilise l'analyse en
composante principal pour estimer les facteurs de confusion et les intégrer aux
tests d'hypothèses statistiques cite:Rahmani_2016,Price_2006. D'autre méthodes
utilisent les modèle mixte cite:Kang_2008,Zhou_2014,Loh194944. Récemment, de
nombreuses méthodes ont été proposées pour permettre d'estimer dans un même
modèle les effets des variables latentes et les effets des variables
explicatives
cite:frichot13_testin_assoc_between_loci_envir,article_Leek_Storey_2007,gerard2017unifying,gerard2017empirical.
...
populations. Des méthodes très efficace ont été proposées pour l'estimation des
coefficients d'ascendance à partir de données génétique. Cependant, l'estimation
de l'ascendance génétique en intégrant l'information spatial à reçus moins
...
génétique intégrant l'information spatial est nécessaire.

Répondant à l'arrivée massive de données, le développement de méthodes pour les
études d'association à grandes échelle est très actif en ce moment. Dans les
étude d'association un aspect très important est de détecter et corriger les
source de variance indésirable pour l'étude. Chaque méthode utilise des approches
différentes et aucunes ne s'est imposées comme étant la méthode de référence.
Dans le contexte actuelle il est nécessaire de développer des méthodes rapides
et utilisables sur les grandes données. Par ailleurs, une comparaison des méthodes
de correction pour les facteurs de confusion permettrait de mieux comprendre les
spécificités de chaque méthodes.
...
afin qu'ils soient applicables aux données génétiques modernes. L'importance a
été placé aussi bien sur le développement mathématique des méthodes que sur leur
implémentation. Afin que nos méthodes statistiques soient utilisables par la
communauté scientifique, il a été important de rendre accessible une
implémentation informatique efficace des nouvelles méthodes statistiques.
...
quelques variables explicatives. Dans certaines de ces études des sources de
variation indésirable telles que la structure de population ou bien les
conditions expérimentales peuvent sévèrement biaiser les résultats des tests.
Une méthode communément utilisé consiste à utiliser des variables latentes afin
de capter les sources de variation indésirable. Dans ce chapitre nous
introduisons deux nouveaux algorithmes qui permettent d'estimer les variables
latentes afin de corriger les étude d'association pour les source de variations
indésirable. Nos algorithmes reposent sur une décomposition de la matrice des
variables étudié en une matrice des effets latents et une matrice des effets des
variables explicatives. Nos deux algorithmes reposent sur une régularisation de
l'effet des variables explicatives sur les variables expliquées différentes.
L'un utilise une régularisons en norme $L_{2}$, tandis que l'autre utilise un
régularisation en norme $L_{1}$. Pour chaque algorithme nous démontrons leur
convergence vers le minimum de leur fonction objectif respective. Nous montrons
sur des simulations numériques que nos algorithmes reproduisent les performances
d'autres algorithmes de correction des tests d'association pour les variables de
confusion. Nous montrons enfin que nos algorithmes permettent de reproduire des
résultats d'une étude d'associations entre des génomes et la maladie \celiac
ainsi qu'une étude d'association entre des niveaux de méthylation de l'ADN et la
polyarthrite rhumatoïde. Nous présentons enfin des résultats trouvés par nos
algorithmes sur un exemple d'étude d'association entre des génotypes et un
gradient climatique. Nos algorithmes sont implémentés dans le package R, =lfmm=.
