...
maladie pour comprendre les causes génétiques de celle-ci. Cependant, comme nous
...
d'association sont faussées. On observe en général une augmentation du nombre de
gènes associés à la variable d'étude. Une telle situation n'est pas souhaitable
car bien qu'il agisse de corrélations, il ne s'agit pas de corrélations
intéressantes pour l'étude biologique. Dans les données génétique qui nous
intéresse dans cette thèse, il est possible d'inférer les facteurs responsable
de la variation génétique. On appel de tels facteurs les facteurs de confusion
pour le test d'association. Les facteurs de confusion doivent être pris en
compte dans les études d'association pour éviter les fausses associations.
...
L'accent a été mis sur la rapidité des méthodes statistiques développées afin
qu'elles soient applicables aux données génétiques modernes. En outre, afin que
nos méthodes statistiques soient utilisables par la communauté scientifique, il
a été important de rendre accessible une implémentation informatique efficace
des nouvelles méthodes statistiques.
...
études d'association. Chaque chapitre commence par une introduction du contexte
et de la problématique biologique. Nous développons ensuite notre méthodologie
statistique et exposons nos algorithmes. Enfin, nos méthodes sont comparées aux
méthodes existantes et appliquées à des échantillons de données réelles de la
biologie afin d'illustrer leurs utilités.
...
\caption{{\bf Q-Q plot et rang pour l'étude d'association entre des sites de
  ...
  candidats proposés par \cite{Rahmani_2016} et \cite{Zou_2014} retrouvés dans
...
#+CAPTION: Dépend de [[code:geas_qqplot]]
#+begin_src R :results silent
  library(MaTheseR)
  MaTheseR.params <- get_MaTheseRparams()

  pl <- readRDS("./OUTPUT/Expr/eas_qqplot_toprint.rds")
  ThesisRpackage::Plots_export_png(pl,
...
\begin{figure}[!th]
\centering
\includegraphics{./OUTPUT/Rplots/geas_venn.png}
\caption{{\bf Diagramme de Venn de l'étude d'association entre des génotypes et
