...
#+end_src
****** TODO Que donne les axes factoriels ?
:LOGBOOK:
...
#+begin_src R 
  ...
  pl <- ggplot(U.df, aes(x = PC6, PC7, color = pop)) +
    geom_point() +
    xlab("Variable latente $K = 6$") +
    ylab("Variable latente $K = 7$") 
  save_plot_png(pl, "GEAS_PC67.png")
#+end_src
