...
loci. Enfin, de la même façon que pour l'étude GWAS nous avons identifié un sous
ensemble de 296948 SNPs grâce au LD pruning. Nous avons utilisé ce sous-ensemble
de loci pour calculer les variables latentes avec les méthodes PCAlm, cate,
lassoLFMM et ridgeLFMM. Nous avons calculé ensuite des \pvalues pour chacun des
5397214 loci et pour chacune des méthodes. Les candidats retournés par les
méthodes a été étudiée grâce au logiciel VEP (Variant Effect Predictor). Le
logiciel VEP permet d'annoter l'effet d'un polymorphisme sur l'expression des
