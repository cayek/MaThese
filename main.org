...
**** Estimateur régularisé en norme $L_{2}$
:LOGBOOK:
...
Pour estimer les paramètres $\U$, $\V$ et $\B$ de LFMM nous proposons
d'optimiser la fonction objectif suivante
\begin{equation}
...
Gaussienne, isotrope et de moyenne nulle. La terme de régularisation 
$$ \lambda \norm{\B}^{2}_{2} $$ 
est indispensable pour distinguer les effets des variables lattentes de celui 
des variables explicatives. En effet, si
$$ \lambda = 0 $$ 
alors pour toutes matrices $\matr{T}$ de taille $\Xcol \times \Ycol$ on a 
$$
\Lridge(\U - \X \matr{T}, \V^{T}, \B + \V \matr{T}^T}) = \Lridge(\U, \V^{T},
\B). 
$$

Nous calculons les estimateurs de la façon suivante 
\begin{align}
...
$\matr{A}$ données par la décomposition en valeurs singulière (SVD pour singular
...
ICI on va explimer Plambda en finction des valeurs singulières !!

Cette matrice définie une application linéaire qui enlève une partie de la
projection orthogonal sur $ \X$. Dans le cas particulier où il n'y a qu'une seul
variable explicative et quelle est normalisée, on a
\begin{equation*}
\obP \X = \frac{1}{\lambda + 1} \X.
\end{equation*}
On remarque que le paramètre de régularisation $\lambda$ permet de réguler la
composante restante le long de $\X$. 
Enfin $\sqrt{\obP}$ est tel 
\begin{equation*}
\obP = \sqrt{\obP}^{2},
\end{equation*}
Il s'agit de la racine carrée d'une matrice carrée. Cette dernière est bien
définie pour $\obP$ qui est symétrique définie positive, c'est à dire que toutes
ses valeurs propres sont positives.

Nous pouvons montrer le théorème suivant.
#+BEGIN_theorem
<<ridge_theorem>>
Pour $\lambda$ strictement supérieur à zéro, La fonction objective $\Lridge$
définie par l'équation eqref:eq:optim_ridge_reg admet un point de minimum
local tel que 
#+END_theorem
...
***** Remarques
:LOGBOOK:
- Note taken on [2017-06-30 ven. 10:50] \\
  parler des cas limites de lambda et du problème d'identifiabilité
:END:
