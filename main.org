...
non nulle. Enfin, les variables \Y sont calculées telles que 
\begin{equation} 
\Y = \U \V^{T} + \X \B^{T} + \E. 
\end{equation} 
Cette simulation correspond à une situation où 1 \% des variables $\Y$ sont
...
uniforme des \pvaleur car la majorité des variables $\Y$ ne sont pas associées
...
sans celle-ci on détecte presque la moitié des variables $\Y comme étant
associées à $\X$.
...
avec $\X$. Les méthodes a contrôles négatifs utilisent donc les variable
...
exemple, les variables $\Y$ peuvent être des SNPs, des niveaux de méthylation ou
bien des niveaux d'expression génique. La matrice $\X$, de taille $\Xrow \times
\Xcol$, regroupe toutes les variables explicatives. Les variable $\X$ sont les
variables d'intérêt pour l'association, c'est à dire les variable pour
lesquelles on souhaite trouver les associations avec $\Y$. Les variables $\X$
...
de variables $\Y$ soit associée à $\X$. C'est à dire que seulement une certaine
...
nécessitent pas de dupliquer la matrice des variables $\Y$. La matrice $\Y$ est
de taille $\Yrow \times \Ycol$ et donc la dupliquer pourrait poser des problèmes
sur des ordinateurs ne possédant pas assez de RAM. Il est possible d'envisager
de ne pas charger la matrice $\Y$ en RAM et d'accéder au données seulement quand
cela est nécessaire.
...
plus inverser la matrice diagonale $\D$. De plus dans le cas ou les variable
...
expliquées $\Y$, que l'on note $(\sigma_1, ..., \sigma_{\Yrow})$. Ensuite, nous
calculons
\begin{equation}
...
Cependant, l'objectif initial est de trouver la liste des variables $\Y$
...
facteurs de confusion que nous avons développées dans cette thèse on été
implémenter à l'aide du language de programmation R. Nous les avons appelées
respectivement lassoLFMM pour l'implemtation des estimateurs régularisées en
norme $L_{1}$ et ridgeLFMM pour les estimateurs régularisées en norme $L_{2}$.
Les algortihmes lassoLFMM et ridgeLFMM prennent en entré la matrice des
variables $/X$ et la matrices des variables $/Y$ pour lesquelle on souhaite
tester l'association avec $/X$. Ils prennent egualement le nombre de variables
lattentes à ajouter au model. L'algorithme ridgeLFMM prend une valeur pour
$\lambLFMM$, le paramètre de régularisation $\L_{2}$. L'algorithme lassoLFMM
prend la proportion de lignes non nulles dans la matrice $\B$ des effets de $\X$
sur $\Y$, que l'on peut interpréter comme une estimation à priori de la
proportion de variable $\Y$ associées avec $\X$. Enfin les deux algorithmes
retournent les estimations pour les paramètres de LFMM ainsi qu'une \pvaleur
pour chaque couple de variables $\Y$-$\X$ testés pour l'associon.
** Autres méthodes existantes comparées
<<sec:similar_method>>

Dans cette section nous présentons des méthodes d'étude d'association avec et
sans correction pour les facteurs de confusions. Les méthodes que nous
...
comparer à une méthode reposant sur la régression de $\Y$ par $\X$. Dans ce cas
aucun facteur latent n'est pris en compte dans l'étude d'association. De plus,
nous nous comparons à une méthode qui repose sur une estimation des variables
latents par l'ACP. Dans ce cas, il s'agit alors de faire une régression de $\Y$
par $\X$ et $\bar{\U)}$ la matrice des scores sur les $\K$ premières composantes
principales. Ces deux méthodes ont été implémentées en R et nous les appellerons
...
Cela correspond a faire l'ACP de $\matr{D}_{\lambRidge = 0} \Q \Y$, en utilisant
les notations de la section [[sec:estimator_L2]]. Ensuite la méthode subset-SVA
calcule un sous-ensemble de variable de $\Y$ qui sont le moins corrélées avec
$/X$. Ce sous ensemble de variable est utilisé pour estimer la matrice des
...
devenir une forte corrélation quand on les prends en compte. 

La deuxième version de SVA est itérative. Plutôt que d'estimer les variables
latentes sur un sous ensemble de variable de $\Y$ la méthode IRW-SVA attribue un
...
un poids à chaque variable de $\Y$ et une nouvelle estimation des variables
...
IRW-SVA est la version par défaut du package R sva, c'est celle que nous
utilisons.
...
applique une matrice de changement de base à $\Y$ de sorte que la premier axe de
la nouvelle base soit colinéaire à $\X$. Cette transformation permet d'avoir sur
le premier axe les coefficients de la régression linéaire de $\Y$ par $\X$ et
sur tout les autres axes le projeté orthogonale de $\Y$ par rapport à $\X$
correspondant au résidu de cette régression. La méthode cate utilise alors les
$\Yrow - 1$ autres coordonnées pour calculer les axes factorielles notés $\V$
dans nos notations. Cette première étape est comparable à l'étape de calcul de
$\V$ dans notre méthode ridgeLFMM (voir partie [[sec:estimator_L2]]). Dans
ridgeLFMM, plutôt que d'enlever complétement les variations de $\Y$ expliquée
...
Pour cela les auteurs de cate ont modélisé explicitement la corrélation entre les
variables explicatives $\X$ et les variables latentes $\U$ tel que
\begin{equation}
\label{eq:cateU}
...
$\matr{\alpha} la méthode cate utilise la première coordonné de la matrice $\Y$
...
correspondent aux variables associés à $\X$ que l'on cherche.
...
implémentation R de ces méthodes mis à disposition par leurs auteurs.
...
Les vrais jeu de données que nous avons choisi pour réaliser les simulation sont
issu du jeux de données 1000 génome que nous présentons dans ...
...
#+begin_src R
  ...
  ## methods order
  method.ordered <- c("lm", "PCAlm", "famt", "sva",
                      "lassoLFMM", "cate", "ridgeLFMM", "oracle")
  auc.df <- auc.df %>%
    mutate(method = factor(method, method.ordered))
...
une étude d'assocaition de celle-ci avec des données génomiques constitu un bon
test pour les méthode de corréction pour les variable lattente. Nous pourrons en
effet comparer nos résultat a ceux des nombreuses autre étude. De plus nous
s'avons que les données génomique prise dans différente population son très
structuré. De plus, nous avons choisi le jeux de données ... pour confronter les
méthodes à un grand jeux de données.
...
***** TODO Conversion dans format utilisable en R
:LOGBOOK:
