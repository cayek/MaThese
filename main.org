...
qu'un lien statistique entres variables ne correspond pas nécessairement à un
...
cite:simpson1951interpretation. Historiquement, les études de corrélations
étaient faites sur des échantillons de données construits pour l'occasion et
contrôlés pour les facteurs de confusion. Cependant, dans le contexte actuelle
les scientifiques ont accès à beaucoup de données provenant de sources éparses.
Utiliser de tel données pour trouver des liens statistiques intéressant pour une
problématique scientifique est alors beaucoup complexe qu'avec des données
contrôlées. A cela s'ajoute le fait que plus l'échantillon de données est grand
plus il est possible de détecter des liens statistiques subtiles. Par
conséquent, dans le contexte actuelle où l'accès aux données est très facile il
faut être très prudent et rigoureux dans les études de corrélations.
...
génétiques. La structure de population influence les distributions des SNPs par
le biais des quatre pressions évolutives dont nous avons parlé dans le début de
cette partie. Les pressions évolutives induisent une différenciation des
distributions alléliques entre les populations. Nous avons illustré se résultats
en représentant les distributions alléliques d'un SNPs pour des individus
humains provenant de populations africaine, européenne et afro-américaine. Nous
constatons une différence de distribution allélique dans les différentes
populations (Figure ref:fig:tess3_intro_freq).
...
l'analyse en composante principal (ACP). Dans une populations d'individus
structuré en $K$ populations, il faut $K-1$ axes principaux pour représenter la
structure de populations à partir de données génétique cite:Patterson_2006. Nous
proposons d'illustrer ce résultats en calculant les deux premiers axes
...
principaux permettent de visualiser un groupe composé des individus européens,
deux groupe composé des individus africains et les individus afro-américains
répartis entre les groupes européens et afro-américains (Figure
...
partir de données de SNPs est celui du logiciel =structure= cite:Pritchard2000.
Dans ce modèle, nous supposons que le génome de chaque individu est la
combinaison de morceau de génome provenant de $K$ clusters génétiques, aussi
...
\end{equation}
où $\Pr(\Y_{i,j} = d)$ est la probabilité d'observé l'allèle $j$ au locus $\ell$
chez l'individu $i$. Le terme $\G_{(d + 1)\ell + j, k}$ représente la fréquence
d'apparition de l'allèle $j$ au locus $\ell$ dans la cluster $k$. Le terme
$Q_{i,k}$ est la proportion de gène de l'individu $i$ prévenant de la population
$k$, appelé coefficients de métissage ou d'ascendance. Les coefficients
d'ascendance et les fréquences d'allèle dans les clusters génétiques sont
respectivement rangés dans des matrices $\Q$ et $\G$. Nous proposons d'illustrer
ce résultats en calculant les coefficients de métissage sur le jeux de données
utilisé pour illustrer la visualisation de la structure de population par l'ACP.
Nous avons utilisé le logiciel =snmf= qui permet d'inférer les coefficient de
métissage à partir de données SNPs avec $K = 2$ clusters génétique
cite:Frichot_2015. Les groupes génétique trouvé par le logiciel =snmf= sont
européen et africains. Tandis que les individus afro-américains ont des génomes
provenant des groupes génétique africain et européen (Figure
ref:fig:tess3_intro). Il s'agit du résultat attendu connaissant l'histoire
démographique des individus afro-américain cite:tishkoff2009genetic.
...
\caption{{\bf Coefficients de métissage individuel.} Estimation par le logiciel
  \texttt{snmf} des coéficients de métissage pour un jeux de données composé
...
Dans cette thèse nous proposons une méthode permettant l'inférence des
coefficients de métissage individuels à partir de données de SNPs et
géographique de chaque individu. Cette méthode repose sur une factorisation
matricielle de la matrice de SNPs avec un régularisation utilisant les données
géographiques. De nombreuse approches ont été proposées pour estimer les
coefficients de métissage ainsi les fréquences ancestrales à partir de données
génétiques. Nous présentons dans le chapitre [[chap:tess3]] un état de l'art des
méthodes d'inférence des coefficients de métissage à partir des données
...
une variable latente de sorte que le coefficient de corrélation entre ces deux
variables soit égale à $0.6$. Nous simulons ensuite une matrice de bruit
...
les tests d'association pour les facteurs de confusion. Il existe plusieurs
approche pour corriger les études d'associations que nous expliquons plus en
détail dans le chapitre [[sec:lfmm_intro]]. La méthodes que nous présentons repose
sur une décomposition de la matrice $\Y$ par en une partie expliquant la variance
systématique que l'on souhaite corriger et une partie représentant l'effet de la
variable explicative $\X$ sur les variables expliquées.
...
les espèces végétales {\it Arabidopsis thaliana}. Sans perte de précision
...
l'étude de données génétiques. Les données génétiques sont volumineuse et
...
cite:Pritchard2000. Dans chaque cluster génétique l'objectif est d'estimer les
...
L'inférence des coefficients de métissage a été largement étudiée et il existe de
nombreuse méthodes. On distingue deux types d'approches : les approches reposant
...
génétiques modernes. Il a été proposé des améliorations du logicielle
...
métissage et de fréquence d'allèle dans les clusters, cite:Engelhardt_2010
...
évolutives qui ont une réalité dans l'environnement géographique de l'espèce.
Les clusters génétiques peuvent par exemple être générés par l'isolation des
populations à cause d'une mer les séparant ou bien des différences d'altitude
entre celles-ci. L'étude réalisée par cite:Novembre_2008 a montré qu'il est
possible de prédire la position des individus à partir de l'étude de leur
...
génome cite:Baran_2013,Yang_2012,Bhaskar_2016,Ra_ola_2014. Si la structure
...
géographiques à travers la loi de distribution à priori des coefficients de
...
cite:FRAN_OIS_2010. Ainsi, les méthodes d'estimation des coefficient
...
établi par cite:Bertsekas_1997, ce n'est pas le cas de l'algorithme APLS. En
...
individuelles et génétiques sont disponibles cite:Horton_2012.
...
eqref:eq:QConst et la matrice $\G$ appartient à $\DG$ l'ensemble définie par les
...
rapport à chaque variable $\Q$ ou $\G$ lorsque l'autre est fixée. Nous pouvons
...
cite:Bertsekas_1997. Le problème d'optimisation selon $\G$, quand $\Q$ est fixé,
est un problème d'optimisation quadratique. Il en va de même quand on échange
les rôles de $\G$ et $\Q$, c'est pour cela que l'algorithme est dit
...
matrice $\G$ tandis que $\Q$ est fixé. Nous supposons que $\Q$ est fixé et
écrivons $\G$ sous une forme vectorielle comme ceci
\begin{equation*} 
...
Pour la deuxième étape de l'algorithme, nous considérons que $\G$ est la
...
= {\rm vec}(\G^T \Y^T)$. Contrairement au problème eqref:eq:AQPg de la première
...
commence par initialiser au hasard les matrices $\Q$ et $\G$ puis alterne deux
étapes. La matrice $\Q$ est calculée pendant que la matrice $\G$ est fixé et
vice versa. 

La première étape de calcul de $\G$ consiste à calculer
\begin{equation}
...
nous projetons $\G^{\star}$ sur le polyèdre $\DG$. 
...
nous notons $\U$, ainsi que la matrice diagonale $\LapVp$ formée des valeurs
...
sur le test de Mantel cite:mantel1967 et la mesure de l'auto-corrélation spatial
...
cite:Cressie1993. Le variogramme peut être étendu aux données génétiques de la
façons suivante
\begin{equation}
...
entrées masquées sont prédites à partir des estimations des matrices $\G$ et
...
locus neutres induisant une différentiation typique entre les différents clusters
...
pour lesquels on observe une différentiation anormal entre les clusters
génétiques cite:Lewontin175. Nous proposons de calculer une statistique de
différentiation entre les fréquences génomiques dans les clusters génétiques
...
\caption{{\bf Simulation de génotypes métissés spatialement.} Les populations
  ...
  matrices de fréquences de génotypes $\G_1$ et $\G_2$. La matrice $\Y$ est
...
Les études d'associations sont massivement utilisées en biologie pour trouver
par exemple les causes génétiques d'une maladie ou bien les gènes impliqués dans
un processus d'adaptation au climat. Lors de ces études nous sommes amené à
tester la corrélation entre un grand nombre de variables expliquées avec
quelques variables explicatives. Dans certaines de ces études des variables
latentes telles que la structure de population ou bien les conditions
expérimentales peuvent sévèrement biaiser les résultats des tests. Cependant
lors les test d'hypothèse multiple il est possible d'estimer la variance
systématique biaisant les résultats afin de corriger les tests statistiques.
Dans ce chapitre nous introduisons deux nouveaux algorithmes qui permettent
d'estimer les variables latentes afin de corriger les tests d'association. Nos
algorithmes reposent sur une décomposition de la matrice des variables
expliquées en une matrice des effets systématiques latents et une matrice des
effets des variables explicatives. Nos deux algorithmes reposent sur une
régularisation de l'effet des variables explicatives sur les variables
expliquées différentes. L'un utilise une régularisons en norme $L_{2}$, tandis
que l'autre utilise un régularisation en norme $L_{1}$. Pour chaque algorithme
nous démontrons leur convergence vers le minimum de leur fonction objectif
respective. Nous montrons sur des simulations numériques que nos algorithmes
reproduisent les performances d'autres algorithmes de correction des tests
d'association pour les variables de confusion. Nous montrons enfin que nos
algorithmes permettent de reproduire des résultats d'une étude d'associations
