...
toujours à rendre applicable un algorithme à un plus grand jeux de données. Par
conséquence il est toujours nécessaire de trouver des nouveaux algorithmes afin
de traiter des rapidement données plus grandes.

Un deuxième problèmes important avec les grands échantillons de données concerne
les études de liaison entre variables. Il est bien connu des statisticiens qu'un
lien statistique entres variables ne correspond pas nécessairement à un lien de
causalité. Nous pouvons par exemple citer l'effet de Yule-Simpsons, un paradoxe
statistique dans lequel un phénomène observé de plusieurs groupes semble
s'inverser lorsque les groupes sont combinés cite:simpson1951interpretation.
Historiquement, les études de corrélations étaient faites sur des échantillons
de données construits pour l'occasion et contrôlés pour les facteurs de
confusion. Cependant, dans le contexte actuelle les scientifiques ont accès à
beaucoup de données provenant de sources éparses. Utiliser de tel données pour
trouver des liens statistiques intéressant pour une problématique scientifique
est alors beaucoup complexe qu'avec des données contrôlées. A cela s'ajoute le
fait que plus l'échantillon de données est grand plus il est possible de
détecter des liens statistiques subtiles. Par conséquent, dans le contexte
actuelle où l'accès aux données est très facile il faut être très prudent et
rigoureux dans les études de corrélations.

Dans le cadre de cette thèse de doctorat nous nous sommes intéressé à développer
...
de population à partir de données génétiques et géographiques. Le deuxième est
...
contextes biologiques dans lesquels interviennent les méthodes statistiques que
...
de la fréquence des versions d'un gêne, que l'on appel allèle, dans des
populations d'être vivant. La génomique des populations est un néologisme qui
...
isolés. Les changements de fréquences d'allèle dans les populations peuvent être
expliqué par quartes pressions évolutives : la mutation, la sélection, la dérive
génétique, et la migration. La mutation créer de nouveau allèle, elle peut être
neutre, c'est à dire sans effet sur l'organisme, ou non et avoir par exemple un
effet sur la capacité de l'organisme à s'adapter à son environnement. La
sélection est l'hypothèse centrale de la théorie de Darwin, ainsi si un allèle
permet à l'organisme d'être mieux adapté à son environnement alors cette allèle
aura plus de chance d'être transmis aux générations futures puisque l'organisme
porteur de l'allèle peut se reproduire plus facilement. La dérive génétique
provient du fait qu'il n'existe pas de populations infinies. Ainsi au file des
générations les distributions alléliques changent à cause de la reproduction au
hasard des individus. Ce processus peut engendrer la disparition ou la fixation
d'un allèle neutre juste par hasard. Enfin la migration est le passage de gènes
...
découverte de la structure de l'ADN (Acide désoxyribonucléique) par Watson et
Crick en 1953, la génomique des populations moderne utilise des données
...
du locus d'un SNP. Nous représentons les données génétique comme la matrice
contenant le nombre de fois que l'allèle muté a été observé pour chaque individu
et chaque locus (Figure ref:fig:matrix).
...
représentation synthétique de la structure génétique de populations à partir des
données de SNPs. Dans cette thèse nous proposons une méthode permettant
l'inférence des coefficients de métissage individuels à partir de données de
SNPs et géographique de chaque individu. Nous expliquons plus en détail ce que
sont les coefficients de métissage dans la partie [[tess3_intro]].
** Test d'association
:LOGBOOK:
