...
ridgeLFMM prennent en entrée la matrice des variables $/X$ et la matrices des
variables $/Y$ à associer avec $/X$. Ils prennent egualement le nombre de
variables latentes $\K$. L'algorithme ridgeLFMM prend une valeur pour
$\lambLFMM$, le paramètre de régularisation $\L_{2}$. L'algorithme lassoLFMM
prend la proportion de lignes non nulles dans la matrice $\B$, la matrice des
effets de $\X$ sur $\Y$. Enfin les deux algorithmes retournent les estimations
pour les paramètres de LFMM ainsi qu'une \pvaleur pour le test d'association de
chaque colonne de $\Y$ avec $\X$.
...
$\X$. Ce sous ensemble de variable est utilisées pour estimer la matrice des
...
\Y^{'} = \U^{'} \V^{T} + \X^{'} \B^{'}^{T} + \E.
\end{equation}
Nous avons ainsi des données pour lesquelle nous savont quelles variables
expliqué est associées avec la variable explicative. Nous utilisons de plus la
structure lattente d'une matrice de données déjà existante ce qui permet d'avoir
des simulations plus realistes et d'éviter les discution sur le choix de la
structure latente.
...
Nous avons choisi de simuler 5 variables latentes pour différente valeur du
nombre de variable de $\Y$ associé avec $\X$ et diférente valeur de la
corrélation entre $\X$ et les 5 variables latentes. Pour chaque variable latente
une valeur entre $-1$ et $1$ de la corélation avecv $\X$ est tiré selon une
loie uniforme et multiplié par une coéficient entre 0 et 1. Le meme coéficient
est utilisé pour calculer toutes les corrélations. Plus le coéficient est proche
de 1 plus les variables latentes seront corélé à $\X$ et donc plus la confusion
lors du test d'association avec la matrice $\Y^{'}$ simulé sera importante. 

**** Mesure de comparaison des performances
Pour comparrer les méthodes entre elles nous avons choisi deux critères. Toutes
les méthodes renvoie une \pvalue pour chaque colonne de $\Y$ et plus cette
\pvalue est faible plus il est raisonnable de déclarer qu'il y a une association
de cette variable avec $\X$. A partir des \pvalues de chaque méthode nous avons
calculé des courbe de precision-rappel. Pour cela nous avons classé pour chaque
méthode les pvaleurs dans l'ordre croissant et calculé pour le nombre
d'association correct retrouvé ainsi que le nombre de fausse découverte en
prenant toute les tops listes. Cela permet ensuite de calculer une aire sous la
courbe de precision-rappel. Cette mesure que nous appelons AUC donne une idée la
capacité de la méthode à proposer une liste avec le moins possible de fausse
association.
