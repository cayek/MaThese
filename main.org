...
#+CAPTION: Dépend de [[gwas_CV_encore_encore]]
#+begin_src R 
  library(MaTheseR)
...
#+begin_src R 
  ...
  ## cross validation K
  expr <- readRDS("./OUTPUT/Expr/gwas_CV_lfmm_encore_encore.rds")
  toplot <- expr$errs %>%
    mutate(lambda = as.factor(lambda)) %>%
    group_by(lambda, K) %>%
    summarise(err.mean = mean(err), N = length(err), sd = sd(err), se = sd / sqrt(N)) %>%
...
De la même facon que pour la GWAS nous avons filtrer les individus trop a
apparenté les variants génétique pas assez fréquent (voir Partie [[sec:gwas]]). De
plus pour faire une étude d'association entre un génotype et un environement il
faut garder des individus qui vive dans leur habitat depuis plusieurs
générations, nous avons donc enlever les populations afro-américaine et
africaine des caraïbes. Il reste finalement 

#+BEGIN_EXPORT latex
\begin{figure}[!t]
\centering
\includegraphics{./OUTPUT/Rplots/eas_hyperparams.pdf.png}
\caption{Choix des paramètres pour l'étude d'association entre des SNPs et la
  maladie polyarthrite \celiac. A) Proportion de variance expliquées par chacune
  des variables latentes estimé par l'algorithme ridgeLFMM pour déférente valeur
...
#+begin_src R 
  ...
  methods <- list()
  methods$m.lasso <- method_lassoLFMM(K = K.method, nozero.prop = nozero.prop,
                                      lambda.K = 100, lambda.eps = 0.001, lambda = 5.9e-2,
                                      relative.err.epsilon = 1e-6,
                                      col.mask = col.mask,
...
:      method      mad    median
:       <chr>    <dbl>     <dbl>
: 1      cate 1.727900 0.1291523
: 2 lassoLFMM 2.377960 0.2881817
: 3        lm 8.760657 0.3815248
: 4     PCAlm 1.128414 0.0248036
: 5 ridgeLFMM 1.593141 0.1152645
...
****** TODO Comparaison des liste avec contrôle du FDR à $0.01$
:LOGBOOK:
- State "TODO"       from              [2017-08-17 jeu. 09:31]
:END:
