...
$\matr{A}$ définie comme la somme ses valeurs singulières.
...
régularisation $\L_{1}$ est connue pour produire des estimations parcimonieux
...
Nous présentons maintenant un algorithme alterné qui permet d'estimer les
paramètres de LFMM qui minimise la fonction objective $\Llasso$ définie en
eqref:eq:optim_lasso_reg.

il faut démontrer la strict convexité !!! voir
