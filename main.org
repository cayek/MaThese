...
génome par liaison génétique. Nous avons donc filtré les SNPs très corrélés
...
trouver les SNPs associés avec la maladie \celiac. Afin d'estimer les variables
latentes pour corriger le test d'association entre les SNPs et la maladie
\celiac, nous avons appliqué les méthodes sur le sous-ensemble de 80275 SNPs
identifiés par l'étape de pruning. Par la suite, nous avons effectué le test
d'hypothèse présenté dans la partie [[sec:hypothese]] sur les 281122 SNPs en
...
ridgeLFMM. Nous avons choisi la proportion de lignes non nulles valant $1\%$
pour lassoLFMM.
...
cite:Storey_2011. La figure ref:fig:ewas_venn montre les intersections entre les
méthodes. Nous avons écarté lm et sva-irw car ils renvoyaient des listes trop
différentes des autres. Parmi les 19 sites de méthylation renvoyés par toutes
les méthodes on retrouve les 5 candidats identifiés par
...
méthodes avec 9 variables latentes (voir Figure ref:fig:gwas_params A et B).
Nous avons choisi une proportion de lignes non nulles pour la matrice des effets
$\B$ valant $1 \%$ pour lassoLFMM. La validation croisée du paramètre
$\lambRidge$ tend à faire choisir une forte valeur pour celui-ci (Figure
ref:fig:gwas_params C), nous avons choisi $1e^{3}$. La validation croisée
présentée sur la Figure ref:fig:gwas_params C tend à faire choisir une valeur
plus grande que $1e^{3}$ cependant comme nous l'avons discuté dans la partie
[[sec:paramL2]] si $\lambdaRidge$ est trop grand alors ridgeLFMM renvoie les mêmes
résultats que PCAlm.
...
#+begin_src R :results output latex :exports results :session *R* :dir ~/Projects/Thesis/MaThese/
  ...
  table.df %>%
    head(10) %>%
    xtable(align = "llllp{1.5cm}p{2cm}llp{2cm}",
    type = "latex",
    caption = "") %>%
    print(include.rownames=FALSE,
...
% Thu Sep 21 18:24:06 2017
\begin{sidewaystable}[ht]
\centering
\begin{tabular}{lllp{1.5cm}p{2cm}llp{2cm}}
  \hline
Chr & Position (pb) & Snp & LD block (Mb) & Odds ratio [$95\%$ IC] & \pvalue & \qvalue & Gènes dans la touffe \\ 
  \hline
\textbf{3} & \textbf{189595248} & \textbf{rs1464510} & \textbf{189.56-189.61} & \textbf{1.30 [1.24-1.36]} & \textbf{$3.79 \times 10^{-23}$} & \textbf{$1.46 \times 10^{-20}$} & \textbf{LPP} \\ 
  \textbf{3} & \textbf{161147744} & \textbf{rs17810546} & \textbf{160.99-161.32} & \textbf{1.35 [1.26-1.45]} & \textbf{$1.84 \times 10^{-16}$} & \textbf{$6.08 \times 10^{-14}$} & \textbf{IQCJ-SCHIP1, IL12A-AS1, IL12A} \\ 
  \textbf{4} & \textbf{123334952} & \textbf{rs13151961} & \textbf{123.19-123.56} & \textbf{0.73 [0.68-0.78]} & \textbf{$1.68 \times 10^{-14}$} & \textbf{$5.31 \times 10^{-12}$} & \textbf{KIAA1109, ADAD1} \\ 
  \textbf{12} & \textbf{110492139} & \textbf{rs653178} & \textbf{110.25-110.49} & \textbf{1.22 [1.16-1.28]} & \textbf{$6.80 \times 10^{-13}$} & \textbf{$2.05 \times 10^{-10}$} & \textbf{CUX2, LINC02356, SH2B3, ATXN2} \\ 
  \textbf{2} & \textbf{102437000} & \textbf{rs917997} & \textbf{102.26-102.61} & \textbf{1.27 [1.20-1.35]} & \textbf{$1.54 \times 10^{-12}$} & \textbf{$4.61 \times 10^{-10}$} & \textbf{IL1RL1, IL18R1, IL18RAP, MIR4772, SLC9A4, SLC9A2} \\ 
  4 & 123774157 & rs6840978 & 123.73-123.77 & 0.77 [0.72-0.82] & $1.20 \times 10^{-11}$ & $3.52 \times 10^{-9}$ & IL21-AS1 \\ 
  3 & 161179692 & rs9811792 & 161.12-161.18 & 1.18 [1.12-1.24] & $6.57 \times 10^{-11}$ & $1.88 \times 10^{-8}$ & IL12A-AS1 \\ 
  \textbf{3} & \textbf{46210205} & \textbf{rs13098911} & \textbf{45.98-46.21} & \textbf{1.32 [1.22-1.43]} & \textbf{$2.07 \times 10^{-10}$} & \textbf{$5.81 \times 10^{-8}$} & \textbf{FYCO1, FLT1P1, CCR3} \\ 
  \textbf{1} & \textbf{190803436} & \textbf{rs2816316} & \textbf{190.77-190.80} & \textbf{0.78 [0.72-0.83]} & \textbf{$2.21 \times 10^{-10}$} & \textbf{$6.20 \times 10^{-8}$} & \textbf{} \\ 
  \textbf{3} & \textbf{46327388} & \textbf{rs6441961} & \textbf{46.26-46.33} & \textbf{1.21 [1.15-1.27]} & \textbf{$1.71 \times 10^{-8}$} & \textbf{$4.55 \times 10^{-6}$} & \textbf{CCR3, UQCRC2P1} \\ 
   \hline
\end{tabular}
\caption{} 
\end{sidewaystable}
...
#+begin_src R 
  ...
  table.df <- aux.df %>%
    transmute(Chr = chromosome %>% bold(`Gwas catalog`),
              ...
              `\\pvalue` = sanitize.numbers(format(pvalue, scientific = TRUE, digits = 3),
                                            type = "latex", math.style.exponents = TRUE) %>% bold(`Gwas catalog`),
              `\\qvalue` = sanitize.numbers(format(qvalue, scientific = TRUE, digits = 3),
                                            type = "latex", math.style.exponents = TRUE) %>% bold(`Gwas catalog`),
              `Gènes dans la touffe` = `hgnc symbol genes in clump` %>% bold(`Gwas catalog`)
              )
...
#+begin_src R :results output latex :exports results
  ...
  union.candidates.df.biomart %>%
    ...
    dplyr::transmute(SNP = snps, `Méthodes` = methods, `Phénotype` = phenotype_description) %>%
    xtable(digits = -2,
           type = "latex",
           caption = "") %>%
    print(include.rownames=FALSE,
...
#+RESULTS: code:geas_table
#+BEGIN_EXPORT latex
% latex table generated in R 3.4.1 by xtable 1.8-2 package
% Tue Sep 19 19:18:22 2017
\begin{sidewaystable}[ht]
\centering
\begin{tabular}{lll}
  \hline
SNP & Méthodes & Phénotype \\ 
  \hline
rs10908907 & ridgeLFMM, cate & Alcoholism (heaviness of drinking) \\ 
  rs10496731 & lassoLFMM & Body Height \\ 
  rs2472297 & ridgeLFMM, cate, lassoLFMM & Caffeine metabolism (plasma 13-dimethylxanthine (theophylline) level) \\ 
  rs2472297 & ridgeLFMM, cate, lassoLFMM & Caffeine metabolism (plasma 137-trimethylxanthine (caffeine) level) \\ 
  rs2472297 & ridgeLFMM, cate, lassoLFMM & Caffeine metabolism (plasma 17-dimethylxanthine (paraxanthine) to 137-trimethylxanthine (caffeine) ratio) \\ 
