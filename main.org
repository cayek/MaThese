...
coefficients de metisse individuels et les fréquences de génotypes ancestraux en
supposant $\K$ populations ancestrales. En plus des génotypes, les nouveaux algorithmes nécessitent les coordonnées géographiques de
chaque individu.
*** Matrices $\Q$ et $\G$
Nous considérons une matrice de génotypique, $\Y$, enregistrant des données pour
$n$ individus à des locus (un locus est une position sur l'ADN) polymorphes $p$
pour une espèce ayant une ploidy de $d$, c'est à a dire qui possède un génome
composé de $d$ exemplaire de chaque chromosome. Pour les SNP autosomiques dans
un organisme diploïde, le génotype au locus $\ell$ est un nombre entier, 0, 1 ou
2, correspondant au nombre d'allèles de référence observé à ce locus. Dans nos
algorithmes, des formes disjonctives sont utilisées pour coder chaque valeur
génotypique comme indicateur d'un hétérozygote ou d'un locus homozygote
cite:Frichot_2014. Pour un organisme diploïde, chaque valeur génotypique
$,0,1,2$ est encodée comme $100$, $010$ et $001$. Pour les organismes de ploidy
$d$, il existe $(d + 1)$ valeurs génotypiques possibles à chaque locus, et
chaque valeur est encodée sous une forme disjonctif unique. Bien nous nous
interessons ici données génétyque composé de SNP, les algorithmes présentés dans
ce chapitre s'étendent aux locus multialaciques sans perte de généralité. De
plus, la méthode peut être facilement étendue aux probabilités d'observation
d'un génotype plutot que la valeur observé étant le plus probable.

De la même manière que dans cite:Frichot_2014, si l'on suppose qu'il y a $K$
population ancestral, nous cherchons à décomposer la matrice $\Y$ en une matrice
de coefficient de métissage $\Q$, de taille $\n \times K$ et une matrice de
fréquence de génotype dans les $K$ populations ancestrale $G$, de taille $p
\times K$. Nous notons $\Q_{i,k}$ le coéficient de métissage de l'individu $i$
population ancestrale $k$. Nous avons de plus 
\begin{equation}
\label{eq:QConst}
...
\begin{equation}
\label{eq:GConst}
\G \geq 0 \, , \quad \sum_{j=0}^{d} {\bf G}_{(p+1)\ell + j, k} = 1.
\end{equation}
Enfin, une estimation du des matrice $\Q$ et $\G$ peuvent être obtenu en
factorisant la matrice de génotype de la façon suivante 
\begin{equation*}
\Y = \Q \G^{T}.
\end{equation*}
Ainsi le problème d'inférence peut être résolu en utilisant les méthode
factorization de matrice non négatives avec les contraintes convexe décrite par
les équations ref:eq:QConst et ref:eq:GConst en plus
...
matrice en utilisant des poids entre les individus. Les poids sont utilisé pour
imposer une contrainte de régularité de l'éstimateur des coéficient de métissage
sur l'espace géographique. Nous voulons souhaitons que des individu proche dans
l'espace géographique est des coéficient de métissage proche. Les poids sont
définis à partir coordonnées géographiques des individus que l'on note $x_{i}$
pou chaque individu $i$. Nous atribuons aux individu proche dans l'espace un
poid plus grand que des individu éloigné. Les poids son calculé en construisant
un graphe complet poudéré entre les individus. Entre chaque individu $i$ et $j$,
on construit la matrice des poids du graphe $\W$ de la manière suivante
\begin{equation}
\label{eq:tess3Graph}
\W_{ij} = \exp( - {\rm dist}( x_i, x_j )^2/ \sigma^2),
\end{equation}
où la fonction ${\rm dist}( x_i, x_j)$ définie n'importe quelle distance entre
les coordonnées géographique $x_{i}$ et $x_{j}$ des individus $i$ et $j$. 

Ensuite nous introduisons la matrice laplacienne associé à la matrice des poids
...
\end{equation}
Par le calcul,  cite:DengCai2011 ont montré que 
\begin{equation}
\label{eq:tess3Reg}
{\rm Tr} (\Q^{T} \Laplacienne \Q)  = \frac{1}{2} \sum_{i,j = 1}^n  \W_{i,j}  \|   \Q_{i,.}  \Q_{j,.} \|^2.
\end{equation}
Dans notre approche, nous supposons que les individus géographiquement proche
ont plus de chance d'avoir des ancettres commun que des individus éloigné. Ainsi
nous utilisons le terme défini par l'equation ref:eq:tess3Reg pour regulariser
l'estimateur de la matrice des coéficient de métissage.
...
\label{eq:tess3LS}
\end{equation}
où la matrice $\Q$ appartient a $\DQ$ l'ensemble définie par les contraintes
ref:eq:QConst et la matrice $\G$ appartient à $\DG$ l'ensemble définie par les
contraintes ref:eq:GConst. La notation $ \| \matr{M} \| _ {\ rm F} $ désigne la
norme Frobenius d'une matrice, $\matr{M}$. Le paramètre de régularisation
$\alpha$ contrôle la régularité des estimations des coefficients de métissage
dans l'espace géographique. Les grandes valeurs de $\alpha$ impliquent que les
coefficients de métissage ont des valeurs similaires pour les personnes proches,
alors que les valeurs faibles ignorent l'autocorrélation spatiale dans les
...
fonction $\mathcal{L}$ est convexe par rapport à chaque variable $\Q$ ou $\G$ lorsque
...
cite:Bertsekas_1997. L'algorithme APQ commence a partir de valeurs initiales
pour les matrices $\G$ et $\Q$, et alternent deux étapes d'optimisation. La
première étape calcule la matrice $\G$ tandis que $\Q$ est fixé, et inversement.
Nous supposons que $\Q$ est fixé et écrivons $\G$ sous une forme
vectorielle
\begin{equation*} 
...
\begin{equation}
\begin{aligned}
\underset{g \in \DG}{\arg \min}  ( -2  v^T_Q \, g + g^T \D_Q g ) \, ,
\end{aligned}
\label{eq:AQPg}
\end{equation}
...
\begin{equation}
\begin{aligned}
\underset{q \in \DQ}{\arg \min} ( -2 v^T_G \, q + q^T \D_G q ) \,  ,
\end{aligned}
\label{eq:AQPq}
\end{equation}
...
jeux de données avec beaucoup d'individus. Pour l'algorithme AQP nous pouvons
énoncer le résultat de convergence suivant. 
#+BEGIN_theorem
<<AQP_theorem>> L'algorithme AQP qui alterne les étapes d'optimisation définis
par ref:eq:AQPg et ref:eq:AQPq converge vers minimum local de la fonction $\mathcal{L}$
défini par l'équation ref:eq:tess3LS.
#+END_theorem
...
cite:Grippo_2000 tout point limite de l'algorithme AQP converge vers un point
de minimum local de la fonction $\mathcal{L}$.
#+END_proof
...
pas de résultat sur la convergence d'APLS mais à une complexité algorithme plus
faible que l'algorithme AQP. L'algorithme APLS commence par initialiser au
hasard les matrice $\Q$ et $\G$ et alterne deux étapes. La matrice $\Q$ est
mis a jour pendant que la matrice $\G$ est fixé et vice versa. La première étape
consiste à calculer 
\begin{equation}
{\bf G}^\star = \arg \min  \|  {\bf Y} - {\bf QG}^T \|^2_{\rm F} \, .
\end{equation}
Cette étape peut être séparé en $(d+1) p$ (le nombre de colonnes de $\Y$)
problème indépendant. Cette opération peut être parallélisé. Ensuite nous
...
** FAQ                                                            :noexport:
*** tess3
**** Pourquoi cette fonction de poid pour le graphe ?
