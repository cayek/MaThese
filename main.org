...
\noindent *Résumé :* Nous présentons des méthodes statistiques reposant sur des problèmes
de factorisation matricielle. Une première méthode permet l'inférence rapide de
la structure de populations à partir de données génétiques en incluant
...
manuscrit les modèles, ainsi que les aspects théoriques des algorithmes
d'inférence. De plus, nous comparons les performances de nos méthodes à ceux des
méthodes existantes à l'aide de simulations numériques. Enfin, nous illustrons
l'utilisations de nos méthodes sur des données biologiques réelles. Nos méthodes
ont été implémentées et distribuées sous la forme de packages R : =tess3r= et
=lfmm=.

\noindent *Mots-clés :* optimisation, factorisation de matrices, structure génétique des
populations, métissage, test d'association, facteurs de confusion, bio-informatique.
...
\noindent *Summary:* We present statistical methods based on matrix factorization
problems. A first method allows efficient inference of population structure from
genetic data and using geographic proximity information. A second method
corrects the association studies for confounding factors. We present in this
manuscript the statistical models, as well as the theoretical aspects of their
inference algorithms. In addition, we compare our methods with existing methods
on simulations; and we illustrate their uses on real biological data. Our
methods have been implemented and distributed as R packages: =tess3r= and =lfmm=
.

\noindent *Keyword:* optimization, matrix factorization, genetic structure of populations,
admixture, association mapping,  confounding factors, bioinformatics.
* Publications
...
qu'ils viennent compléter. Nous parlons en particulier de =TESS= 2.3, un
logiciel d'inférence spatiale de la structure génétique des populations
citep:Durand_2009; ainsi que de =LEA=, un package R proposant une suite de
...
dans deux packages R : =tess3r= (https://bcm-uga.github.io/TESS3_encho_sen/) et
=lfmm= (https://github.com/bcm-uga/lfmm). Le package =tess3r= contient les
...
#+begin_src R :session *R* :dir ~/Projects/Thesis/MaThese/ :results silent
  ...
  pl <- readRDS("./OUTPUT/Expr/eas_pc_toprint.rds")
...
\caption{{\bf Variables latentes retournées par ridgeLFMM pour l'étude
    d'association entre génotype et un gradient environnemental (GEAS).}}
\label{fig:eas_params}
\end{figure}
#+END_EXPORT
...
#+begin_src R 
  ...
  expr <- readRDS("./OUTPUT/Expr/Eas_U_ridgeLFMM_409192b1e7bfbf4affc37da98f3ea0bd.rds")
  str(expr)
  ...
  colnames(U.df) <- paste0("PC",1:9)
  U.df <- U.df %>% cbind(indiv.df) %>% as_tibble() %>%
    mutate(Population = pop)
  pl1 <- ggplot(U.df, aes(x = PC6, PC7, color = Population)) +
    geom_point() +
  ...
  pl2 <- ggplot(U.df, aes(x = PC8, PC9, color = pop)) +
    geom_point() +
  ...
  pl <- plot_grid(pl1 + theme(legend.position = "none"),
                  pl2 + theme(legend.position = "none"),
                  mylegend,
                  ncol = 3,
                  labels = c("A","B"), rel_widths=c(5,5,2))
  save_plot_png(pl, "eas_pc.png")
  save_expr(pl, "eas_pc_toprint.rds")
#+end_src
...
#+begin_src R 
  ...
  ggplot(toplot, aes(pvalue)) +
    geom_histogram()
#+end_src
***** TODO Run seulement avec 7 premières variables latentes
****** TODO Run
:LOGBOOK:
...
#+END_SRC
****** TODO Annotation de l'union controlé à un FDR de 1%
:LOGBOOK:
...
méthodologiques et logiciels, afin d'ajouter des fonctionnalités à nos packages
en fonction des retours des utilisateurs. D'autre part, nos algorithmes
résolvent des problèmes de factorisation matricielle. Ainsi, nos travaux ouvrent
des perspectives sur l'utilisation de la factorisation matricielle dans d'autres
