...
\end{equation*}
Ainsi, les points de minima d'une tel fonction objective ne sont pas pertinent pour
notre problème.
...
à l'espace engendré par $\X$. Si $\lambRidge$ vaut zéro la multiplication par
$\matr{D} \matr{Q}^{T}$ revient à prendre le résidu d'une régression linéaire
par $\X$, on enlève alors toute la corrélation linéaire avec $\X$. Si $\lambRidge$
...
$\lambRidge$ qui enlève la bonne proportion de corrélation avec les variables
...
\begin{equation*}
\left\{ \frac{\lambRidge}{\lambRidge + \sigma_{1}^{2}}, ..., 
\frac{\lambRidge}{\lambRidge + \sigma_{d}^{2}}, 
1, ..., 1 \right\}.
\end{equation*}
...
\begin{align*}
\hat{\U} \hat{\V} & =  \matr{Q} \matr{D}^{-1} \svd_{\K}( \matr{D} \matr{Q}^{T} \Y ) \\
\hat{\B} & = (\X^{T} \X + \lambRidge \Id_{d})^{-1} \X^{T} (G - \hat{\U} \hat{\V})
\end{align*}
...
1. calculer $\B_{t}$ le point minimum de 
   \begin{equation}
   \label{eq:lasso1}
   \mathcal{L}_{lasso}^{2}(\B) =  \frac{1}{2} ||(\Y - \W_{t-1}) - \X \B^T||_{F}^2 + \lambdaLasso ||\B||_1
   \end{equation}
...
\begin{equation}
\B_{t} = \sign(\bar{\B}_{t}) (\bar{\B}_{t} - \lambdaLasso)_{+}
\end{equation}
où 
\begin{equation}
s_{+} = \mathrm{max}(0, s)
...
\begin{equation*}
\bar{\B}_{t} = (\X^{T} \X + \lambdaLasso \Id_{d})^{-1} \X^{T} (\matr{E}^{1}_{t}).
\end{equation*}
...
vers un minimum quand la fonction n'est pas continûment différentielle, comme
c'est le cas pour $\Llasso$. Cependant, les théorèmes démontrer dans
cite:Tseng_2001 permette de démontrer la convergence de l'algorithme
d'estimation $L_{1}$ dans un cadre plus large qui complique l'extraction des
résultats intéressants dans notre cas. Pour facilité la compréhension, nous
proposons de démontrer un théorème plus faible qui s'applique directement à
notre cas. Pour cela nous introduisons la fonction $f$ définie sur son domaine
\begin{equation}
\label{eq:domf}
X = X_{1} \times X_{2} \times ... \times X_{N}
\end{equation}
un produit Cartésien d'ensemble fermé et convexe. L'algorithme de descente par bloque de
coordonnées est définie par la formule de récurrence suivante :
\begin{equation}
\label{eq:blokAlgo}
x_{i}^{k+1} \in \mathrm{arg} \min_{\zeta \in X_{i}} f(x_{1}^{k}, ...,x_{i-1}^{k},\zeta,x_{i+1}^{k},..., x_{N}^{k}), ~
i = 1,..., N.
\end{equation}
...
l'algorithme de descente par bloque de coordonnées dans le cas différentiable,
nous démontrons la proposition suivante :
#+BEGIN_theorem 
Si $f$ est continue, convexe et 
\begin{equation}
f(x_{1},..., x_{N}) = g(x_{1}, ..., x_{N}) + \Sum_{i = 1]^{N} f_{i}(x_{i}) 
\end{equation}
où g est convexe et différentielle et les fonctions $f_{i}$ sont continues et
convexes. Nous supposons de plus que pour tout $i \in {1,...,N}$ le minimum
\begin{equation}
\min_{\zeta \in X_{1}} f(x_{1}, ...,x_{i-1},\zeta,x_{i+1},..., x_{N})}
\end{equation}
est atteint en un point unique. Soit la séquence généré par ${x^{k}}}$ généré
par eqref:eq:blokAlgo. Alors tout point limite de ${x^{k}}}$ est un point de
minimum global de $f$.
#+END_theorem
...
\end{equation}
un point limite de ${x^{k}}}$, $/bar{x}$ est bien dans $X$ car cette ensemble
est fermé. Comme $g$ est convexe et différentiable on a pour tout $x \in \X$
\begin{align}
\label{eq:lassoProof1}
f(x) - f(\bar{x}) & \geq & \nabla g(\bar{x})(x - \bar{x}) + 
\sum_{i = 1}^{N} (f_{i}(x_{i}) - f_{i}(\bar{x}_{i})) \\
 & & = \sum_{i = 1}^{N} ( \nabla_{i} g(\bar{x})(x - \bar{x}) + 
 f_{i}(x_{i}) - f_{i}(\bar{x}_{i}))
\end{align}
où $\nabla g(\bar{x})$ est la dérivé de $g$ en $\bar{x}$ et $\nabla_{i}
g(\bar{x})$ la dérivé le long de la $i\mathrm{-ieme}$ variable. Or nous savons
par construction de $\bar{x}$ on a 
\begin{equation}
\label{eq:lassoProof2}
f(\bar{x}) \leq f(\bar{x}_{1}, ...,x_{i},..., \bar{x}_{N}), \forall x_{i} \in
X_{i}.
\end{equation} 
On a donc pour chaque variable d'indice $i$
\begin{align*}
\nabla_{i} g(\bar{x})(x - \bar{x}) + f_{i}(x_{i}) - f_{i}(\bar{x}_{i}) & \geq  (\nabla_{i} g(\bar{x}) + s_{i})(x - \bar{x}) \\
\label{eq:lassoProof3}
& \geq 0
\end{align*}
où $s_{i}$ est une sous dérivé de la fonction convexe $f_{i}$. L'equation
ref:eq:lassoProof2 nous permet de dire qu'il existe une sous dérivé $s_{i}$ tel
que 
\begin{equation}
\label{eq:3}
(\nabla_{i} g(\bar{x}) + s_{i}) = 0
\end{equation}
d'où l'équation ref:eq:lassoProof3.
Nous avons donc en utilisant ref:eq:lassoProof3 et ref:eq:lasso1 que $x \in \X$
\begin{equation}
f(x) - f(\bar{x}) \geq 0.
\end{equation}
...
paramètre que nous avons introduit dans les parties [[sec:estimator_L2]] et
[[sec:estimator_L1]]. On peut distinguer deux grandes étapes présents dans les
algorithmes d'estimation $L_{1}$ et $L_{2}$. La première est le calcul de la
décomposition en valeur singulière tronqué (voir le calcule de la matrice
...
$L_{1}$). La seconde est le calcule de la projection orthogonal sur l'espace
engendré par les variables explicatives (voir le calcule de $\W$ donné par
...
si on utilise une méthode avec projections aléatoires.

La deuxième étape importante des algorithmes que nous avons présenté est l'étape
de projection du résidu de l'approximation de rang faible sur l'espace engendré
par $\X$. Si on prend en compte toutes les opérations matricielles, cette étape
nécessite $O(\Xcol (\Yrow \Ycol + \K (\Yrow + \Ycol) + \Xcol ( \Xcol + \Yrow +
\Ycol + 1))$.

Finalement, si on considère les opération restantes dans les deux algorithmes,
le nombre d'opération est de l'ordre de $O(\Yrow \Ycol \K)$. L'algorithme
d'estimation $L_{1}$ est bien entendu plus long car il réalise plusieurs
fois (le nombre d'itérations dépend de la vitesse à laquelle
l'algorithme converge) les opérations de décomposition en valeur
singulière et de projection alors que l'algorithme ne les réalise qu'une seul
fois.

Outre la complexité temporelle il est important d'étudier la complexité
spatiale, surtout pour ce genres d'algorithme qui prennent en entré des données
potentielle trop grand pour la mémoire vive de l'ordinateur (RAM). Les
algorithmes d'estimation $L_{1}$ et $L_{2}$ ne nécessite pas de dupliquer la
matrice des variables explicatives $\Y$. En effet, la matrice $\Y$ est de taille
$\Yrow \times \Ycol$ et donc la dupliquer pourrait poser des problèmes sur des
ordinateur ne possédant pas assez de RAM. Il est même possible d'envisager de ne
pas charger la matrice en RAM en accédant au données seulement quand cela est
nécessaire. 
...
La sélections des hyperparamètres est un problème très fréquents en analyse de
données. Nous présentons plusieurs approches pratiques pour choisir les
hyperparamètre qui interviennent dans les algorithmes que nous présenté ici.
Nous commençons par présenter les différentes approches possible pour choisir le
nombre de variables latentes $K$. Nous présentons ensuite plusieurs heuristiques
qui permettent de choisir les paramêtres de régularisation. Enfin nous présentons
un algorithmes de cross validation adapté aux algorithmes que nous avons
présentés.

***** Nombre $\K$ de variables latentes
Nous avons vu dans la section [[sec:estimator_L2]] que la matrice latente peut être
estimée grâce à l'analyse en composante principale (ACP) de $\D
\Q^{T} \Y$. Ainsi, nous proposons d'employer les techniques d'estimation
du nombre variables latentes utilisées pour le modèle de l'ACP sur la matrice
$\D \Q^{T} \Y$ afin d'estimer le nombre $\K$ de variable latente
pour le modèle LFMM.

Il existe de nombreuses approches pour déterminer le nombre de composantes
principales de l'ACP, celle-ci sont très bien expliqué dans
cite:jolliffe1986principal. On peut grouper ses approche en trois catégories.
Les approches subjective comme l'utilisation du scree plot, il s'agit du graphes
des valeurs singulières. Les approches basé sur une modélisation de la
distribution des données observées, comme par exemple la méthode présenté dans
cite:choi2014selecting. Les approches basées sur la validation croisée, comme
celle que nous détaillons plus loin. 

Aucune méthode ne s'est imposé comme la référence, et il est préférable d'en
utiliser plusieurs. Ainsi, l'utilisateur peut utiliser différentes méthodes sur
la matrice $\D \Q^{T} \Y$ pour différente valeurs de $\lambda$ le paramètre de
régularisation $L_{2}$. 

***** Paramètre de régularisation $L_{2}$
Le paramètre de régularisation $L_{2}$ intervient dans le calcule de
...
nulle dans la matrice des effets $\B$. Nous savons à priori que seulement une
partie des variable expliqué sont corrélé avec les variables explicatives.
Ainsi, il est possible d'interpréter la proportion de ligne non nulle dans $\B$
comme la proportion des variables qui sont corrélé avec $\X$. Ainsi plutôt que
de choisir le paramètre de régularisation, il est plus simple de choisir la
proportion de variable expliqué par $\X$. Pour trouver un paramètre de
régularisation qui correspond à cette proportion nous proposons une heuristique
basé sur un chemin de régularisation inspiré par
...
d'estimation des moindres carré régularisé en norme $L_{1}$ présenté dans la
partie [[sec:estimator_L1]]. Nous notons cette valeur $\lambLasso^{\mathrm{max}}$.
Ensuite, nous construisons une séquence $m$ valeurs de $\lambLasso$ décroissant
selon une échelle logarithme depuis $\lambLasso^{\mathrm{max}}$ jusqu'à
\begin{equation}
\lambLasso^{\mathrm{min}} = \epsilon \lambLasso^{\mathrm{max}}.
\end{equation}
Nous calculons le nombre de valeurs non nulle dans $\hat{\B}$ l'estimation de la
matrice des effets et stop si la proportion de valeurs non nulle souhaitée est dépassée.


***** Paramètre de régularisation de la norme nucléaire
Le paramètre de régularisation de la norme à une influence sur le rang nucléaire de la
matrice latente $\W$. Il est plus simple d'un point de vu utilisateur de choisir
le rang de cette matrice, qui correspond au nombre de variables latentes $K$,
que de choisir le paramètre de régularisation $\gamma$. En effet cela permet
d'utiliser la même valeur de $K$ pour l'estimation $L_{1}$ que celle utilisé
pour l'estimation $L_{2}$. 

Nous proposons l'heuristique suivante pour calculer $\gamma$ à partir de $K$.
Nous commençons par caculer les valeurs singulières de la matrice des variables
expliquées $\Y$, que l'on note $(\sigma_1, ..., \sigma_{\Yrow})$. Ensuite, 
\begin{equation}
...
Jusque ici, nous avons présenté une méthode qui permet d'estimer les variables
latentes de LFMM. Cependant, l'objectif initial est de trouver la liste des
variables expliquées associées aux variables explicatives tout en prenant en compte
les variables latentes. Nous présentons dans cette partie un test d'hypothèse
corrigé pour les variables latentes.

Une approche simple consiste a considérer que l'estimation des variables
latentes $\hat{\U}$ comme les vraies valeurs $/hat{\U}$ et des les utilisé à
coté des variable explicative du modèle mis en place réaliser le test
d'hypothèse. C'est une méthode très courante dans les études d'associations qui
a montrer de très bon résultats surtout quand il y assez d'observation
cite:gerard2017unifying,Price_2006,Song_2015,article_Leek_Storey_2008,Rahmani_2016.
Nous avons choisi de réaliser un test d'hypothèse qui repose sur un modèle
linéaire car cela correspond au modèle LFMM quand on suppose que $\U$ est
connue, mais l'estimation des variables latentes peuvent être traité comme des
variables explicatives dans n'importe quel modèle statistique.

Afin de simplifier les notation et sans perte de généralité, nous supposons
qu'il n'y a qu'une seul variable explicative, c'est à dire que $\Xcol$ vaut $1$.
De plus nous rappelons que l'estimation de la matrice des $\K$ variables
latentes $\hat{\U}$ est défini de façon unique grâce à l'ACP de la matrice
$\hat{\W}$. La matrice $\hat{\W}$ est l'estimation $L_{1}$ ou $L_{2}$ de la
matrice latente du modèle LFMM.

***** Calcul de la statistique de test
Afin de réaliser le test d'hypothèse le plus général possible nous avons choisi
d'utiliser le modèle linéaire suivant 
\begin{equation}
\Y_{j} =  \hat{\U} \matr{\gamma}_{j}^{T} + \X \beta_{j} + \matr{\epsilon_{j}}
\end{equation}
pour chaque variable expliqué $j$. La matrice $\hat{\U}$ est l'estimation
$L_{1}$ ou $L_{2}$ de la matrice des variables latente du modèle LFMM. On
suppose que l'erreur $\matr{\epsilon_{j}}$ est Gaussienne de moyen nulle. On
veut tester l'hypothèse de nullité du coefficient de régression $\beta_{j}$.
Sous ces hypothèses on peut calculer pour chaque variable expliqué une
statistique $z_{j}$. Le détail des calculs de la statistique de test est données dans la
section 3.2 de cite:Hastie_2009. La statistique de test suit sous l'hypothèse
nulle le loi de Student à $\Yrow - \K - 1$ degrés de liberté. On peut donc
calculer pour chaque variable expliqué une \pvalue $p_{j}$.
...
Dans les exemples que nous présentons ici ont s'attend a ce que la majorité des
variables ne soit pas associé avec la variable explicative, ainsi une large
majorité des statistiques de test sont distribué selon l'hypothèse nulle. Nous
utilisons l'approche choisi dans cite:Sun_2012, qui consiste à calculer la
