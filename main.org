...
d'observations. Par exemple, les variables observées peuvent être des SNPs ou
...
STOP ca c'est une remarque pour après !!

Afin d'estimer les paramètre du model eqref:eq:model, nous supposons comme dans
cite:tipping1999probabilistic que le bruit est Gaussien et isotrope, c'est a
dire que $$ \E \sim \Normal(0, \sigma^{2} \Id_{\Ycol}) $$ où $\sigma^{2}$ est la
variance de l'erreur et $\Id_{\Ycol}$. Nous pouvons alors estimer les paramètres
du modèle en minimisant l'opposé de la log-vraisemblance. Cela fait intervenir
...
Cependant, la fonction $\Lnoreg$ ne permet de définir de façons unique la
matrice latente $\W$ et la matrice des effets $\B$. 
...
#+begin_src R :results output :exports both
  ...
  K <- 5
  cs <- cs_sampler(K)
  s <- ExpRsampler_fromTrueData(Y = Y, K = 5, prop.outlier = 0.05, cs = cs, rho.B = 1.0)
...
#+begin_src R :results output :exports both
  ...
  samplers <-  s * param(prop.outlier = c(0.01, 0.1, 0.2, 0.3))
  ...
  pl <- plot_AUC_prop_outlier(toplot)
  save_plot_png(pl, "validation_num_prop_auc.png")
  pl <- plot_pvalue_grid(toplot)
  save_plot_png(pl, "validation_num_prop_grid.png")
  pl <- plot_precision_recall(toplot)
  save_plot_png(pl, "validation_num_prop_pr.png")
  pl <- plot_gif_prop_outlier(toplot)
  save_plot_png(pl, "validation_num_prop_gif.png")
#+end_src

#+RESULTS:
[[./OUTPUT/Rplots/validation_num_prop_auc.png]]
[[./OUTPUT/Rplots/validation_num_prop_grid.png]]
[[./OUTPUT/Rplots/validation_num_prop_pr.png]]
[[./OUTPUT/Rplots/validation_num_prop_gif.png]]
#+begin_example
  >   toplot <- expr$df.res %>%
  +     dplyr::filter(pvalue.index == "pvalue1")
  >   pl <- plot_AUC_prop_outlier(toplot)
...
#+begin_src R :results output :exports both
  ...
  nb.cluster <- 10
  cv <- method_CV_ridgeLFMM(n.fold.col = 10, n.fold.row = 10,
                            lambdas = lambdas,
                            Ks = c(1,2,5,10,15, 30))
...
#+RESULTS:
[[./OUTPUT/Rplots/file62412903fe66.png]]
[[./OUTPUT/Rplots/file624177c6cec6.png]]
#+begin_example
  > pl <- plot_CV_ridgeLFMM(res.cv$errs, major = "lambda")
  > save_plot_png(pl)
  [[./OUTPUT/Rplots/file62412903fe66.png]]
...
#+RESULTS:
[[./OUTPUT/Rplots/file5cab74a51d2c.png]]
[[./OUTPUT/Rplots/file5cab19e1136c.png]]
#+begin_example
  > pl <- plot_CV_ridgeLFMM(res.cv$errs, major = "lambda")
  > save_plot_png(pl)
  [[./OUTPUT/Rplots/file5cab74a51d2c.png]]
