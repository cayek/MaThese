...
précisément, l'algorithme AQP alterne des résolutions de problèmes
d'optimisation quadratique. Alors que l'algorithme APLS lève les contraintes des
problèmes d'optimisation quadratique, afin de les transformer en problèmes des
moindres carrés régularisé par une norme $L_{2}$. Ainsi, la complexité de
l'algorithme APLS augmente linéairement avec le nombre d'individus dans
l'échantillon. De plus, nous avons mis en place un test de détection de
...
par blocs de coordonnées. Nous apportons égualement une preuve de la
convergeance de l'algorithme lassoLFMM vers un minimum global de sa fonction
objectif. 
...
d'assocaition faire a partir de données réelles. Sur les données réelles 
*** Publications
