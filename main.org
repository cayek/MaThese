...
population à partir de données génétiques et géographiques. Le deuxième est
l'estimation des facteurs de confusion pour corriger les tests d'association.
Les méthodes statistiques développer lors de cette thèse repose sur la
factorisation de matrice. Nous allons maintenant introduire plus en détails les
contextes biologiques dans lesquels interviennent les méthodes développées
durant cette thèse.
...
l'estimation de la structure de population et les test d'associations. L'accent
a été mis sur la rapidité des méthodes développées afin qu'elles soient
...
données génétique. Les données génétiques sont volumineuse et multivariées. La
structure de population fournit une représentation synthétique qui de visualiser
la variation génétique induite par la stratification en population. La
stratification en population ainsi observée fourni des informations sur
l'histoire et l'évolution démographique de l'espèce étudié cite:Li_2008; il est
également indispensable de l'utiliser comme facteur de correction dans les
études d'associations avec un phénotype, un gradient environnementale ou encore
une maladie cite:marchini2004effects. De même, il existe de nombreuses
applications en médecine génétique nécessitant de connaitre la structure de
population comme par exemple le calcule d'un score de risque génétique pour une
maladie cite:Wray_2013. Enfin, l'étude de la répartition en population d'une
espèce dans son habitat est une étape clée en génétique du paysage
cite:Fran_ois_2015.
...
génétiques (aussi appelé population ancestrale). Dans chaque cluster génétique
nous devons estimer les fréquences d'allèle pour chaque SNPs. Pour chaque
individu nous devons estimer la proportion de son génotype qui provient de
chaque cluster génétique. Ces proportions sont appelées coefficients de métissage
individuel (aussi appelés coefficients d'ascendance).
...
fréquence d'allèle dans les clusters génétiques. Cependant les algorithmes de
monte carlo ne passe à l'échelle des grands jeux de données génétiques modernes.
Il a été proposé des améliorations du logicielle structure reposant sur une
fonction de vraisemblance définie pour la matrice des coefficients de métissage
et les fréquences d'allèle dans les clusters. L'estimation est effectuée en
maximisant la fonction log-vraisemblance. La première amélioration de
l'algorithme =structure= est fondée sur un algorithme EM (Expectation
...
Les algorithmes que nous avons développé ont été penssé pour fonctionné sur des
...
Les algorithme que nous avons dévelopé peuvent être découpé en une suite
...
cite:mazumder10_spect_regul_algor_learn_large_incom_matric. Cetet approche
...
Nous pouvons nous interroger sur l'effets des valeurs aberrantes sur nos
méthodes. On attribu parfois un score sur la confiance que l'on a sur la mesure
d'un génome. Une méthode simple consiste à filtrer les observation ayant un
score trop faible pour les traiter comme des données manquante. De la même
manière les valaur extrème d'un variable peuvent être considéré comme abbérante
et considéré comme manquante. Le problème est autrement plus complexe quand on
on nous ne somme pas capable de detecter les erreurs de mesure présentes dans
les données. 


Ainsi
que comment adapter nos méthodes pour prendre en compte les valeurs manquantes.





Nous avons évalué dans le chapitre [[chap:tess3]]. 
