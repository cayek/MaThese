...
causalité qui existe entre des événements. En statistique, un événement est
modélisé par une variable aléatoire et il est seulement possible détecter des
liens statistique entre les variables aléatoires que l'on appel corrélation. La
corrélation renseigne les probabilités jointes des variables aléatoires en
question mais pas sur leur lien de causalité. Dans un cadre statistique on
parlons de test d'association quand on test si deux variables aléatoires sont
corrélées. 

Les tests d'association sont très utilisée en génétique pour comprendre les
fonctions des gènes. Par exemple on peut chercher quelle SNP est corrélé à une
maladie pour comprendre les cause génétique de la maladie. Cependant, comme nous
...
corrélé à la variable d'étude (la maladie par exemple) alors les études
d'association sont faussé et on observe en général une augmentation du nombre de
gène associé à la variable d'étude. On parle de facteurs de confusion pour le
test d'association. 

Dans dans cette thèse nous proposons une méthode statistique qui permet de
corriger les tests d'association pour les facteurs de confusion. Nous expliquons
plus en détail ce que sont les facteurs de confusions et comment nous les
modélisons dans la partie [[sec:lfmm_intro]].
...
La génétique produit beaucoups de données et de en plus vite grace aux
technologie de séquencage toujours plus efficace. Cette affluence de données
pose de nouveau problème aux staticien. Dans cette thèse nous nous sommes
intéressé à deux problématique en analyse de données génétiques. L'accent a été
mis sur la rapidité des méthodes dévelopé afin qu'elle soit applicable aux
données génétique moderne. En outre, afin que nos méthodes statistiques soit
utilisable par la communauté scientifique en génétique il a été important de
rendre accessible une implémentation informatique efficace des méthodes en
question.

La première méthode statistique développé dans cette thèse concerne la génétique
des populations. A partir de données génétique et géographique nous souhaitons
estimer une représentation synthétique permettant de visualiser l'organisation
en populations structurées des individus observés. Nous avons dévelopé une
seconde méthode statistique permettant de réaliser des test d'association. En
particulier nous souhaitons résoudre le problème très présent dans les étude
d'association en génétiques des facteurs de confusions.

Nous présentons maintenant plus en détail chacune de nos nouvelles méthodes
statistiques et en démontrons l'utilités sur des simulation et exemples concrets de
la génétique.
