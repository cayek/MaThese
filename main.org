...
\end{minipage}
\caption{{\bf Erreurs statistiques des estimations de APLS et TESS 2.3 sur des
    simulations de génotype métissé.} A) Racine carrée de l'erreur quadratique
  moyenne (RMSE) des esimations de $\G$ en fonction du niveau de diférentiation
  entre les deus clusters génétiques avant le métissage $F_{\rm ST}$. B) Racine
  carrée de l'erreur quadratique moyenne (RMSE) des esimations de $\Q$ en
  fonction du niveau de diférentiation entre les deux cluster génétiques avant
  le métissage $F_{\rm ST}$.}
