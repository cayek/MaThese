...
citep:Tang_2005,alexander2009admixture, ou bien utilise des méthodes d'inférence
variationnelle bayésiennes citep:Raj_2014. Des méthodes très rapides ne reposant
...
Par ailleurs, de nombreuses méthodes utilisent l'information spatiale
...
citep:CHEN_2007,Corander2008,GUEDJ_2011. Cependant, les méthodes bayésiennes
repose sur de nombreuse hypothèses et passe plus difficilement à l'échelle des
grands jeux de données. Aucune méthode non basée sur un modèle probabiliste n'a
été proposé pour l'inférence spatiale des coefficients d'ascendance. Nous
résumons les méthodes d'inférence des coéficients d'ascendance génétique dans la
Table [[table:tess3_etat_art]]

#+ATTR_LATEX: :align lp{3cm}lx{6cm}p{6cm} :float sideways
...
| sNMF          | factorisation de matricielles sous contraintes | non      | optimisation quadratique alterné avec projection | citet:Frichot_2014                          |
...
| TESS3-AQP     | factorisation de matricielles sous contraintes | oui      | optimisation quadratique alterné                 |                                             |
| TESS3-APLS    | factorisation de matricielles sous contraintes | oui      | moindres carré alterné projeté                   |                                             |
...
pour l'association.
La plupart de ces méthodes reposent sur l'équation 

Les modèle reposant sur l'équation on recu plusieurs nom dans la litérature :
Latent Fator Mixed Models (LFMM) citep:frichot13_testin_assoc_between_loci_envir,
regression-based latent models citep:agarwal09_regres, factor-augmented
regression model cite:gerard2017empirical, surrogate variable analysis
cite:article_Leek_Storey_2007. 



Nous résumons les méthodes reposant sur des dérrivé du modèle d'écrit par l'equation
dans le tableau.

#+ATTR_LATEX: :align llp{3cm}l
#+CAPTION: *Méthodes de correction des étude d'association reposant sur le modèle LFMM d'écrit par l'équation* 
#+NAME: table:lfmm_etat_art
| Méthode              | Modèle | Algorithme | Référence       |
|----------------------+--------+------------+-----------------|
| sva-twostep          |        |            |                 |
| sva-irw              | dd     |            | cite:GUEDJ_2011 |
| famt                 |        |            |                 |
| cate                 |        |            |                 |
| lea-lfmm             |        |            |                 |
|                      |        |            |                 |
...
toujours plus efficace. Cette affluence de données pose de nouveaux problèmes
...
concrètes sur le vivant. Dans cette thèse nous nous sommes intéressé à deux
problématiques en analyse de données génétiques : l'estimation de la structure
génétique de population et les études d'associations. L'accent a été mis sur la
complexité des algorithmes développées afin qu'ils soient applicables aux
données génétiques modernes. De plus, l'importance a été placé aussi bien sur le
développement mathématique des méthodes que sur leurs implémentations
informatiques. Afin que nos méthodes statistiques soient utilisables par la
communauté scientifique, il a été important de rendre accessible des
...
sur des problèmes de factorisation de matrice. Nos algorithmes ont été
implémenté dans deux packages R : =tess3r=
(https://bcm-uga.github.io/TESS3_encho_sen/) et =lfmm=
(https://github.com/bcm-uga/lfmm). Le package =tess3r= contient les algorithmes
AQP et APLS d'inférence de l'ascendance génétique en incluent l'information
spatiale. Le package =lfmm= contient les algorithme lassoLFMM et ridgeLFMM
d'estimation des facteurs de confusion pour corriger les étude d'association.

*** =tess3r=

Dans le package =tess3r=, nous avons développé des algorithme d'estimation
rapide des coéficients de métissage à partir de données génétique et
géographique. Les algorithmes repose sur un problème de factorisation de la
matrice génétique. Les termes de la factorisation sont les coéficients de
métissage et les fréquences d'allèles dans les clusters génétiques. L'information
spatiale est ajouté au moyen d'une régularisation sur la matrice des coéficients
de métissage. La régularisation spatiale permet d'ajouter l'hypothèse que des
individu proche ont plus de chance de partager des ancestre communs que des
individus éloignés. Afin d'estimer les matrices d'ascendance nous avons proposé
deux algorithmes que nous avons apelé AQP et APLS. Les deux algorithmes
diffèrent dans les approximations qu'ils font pour diminuer la complexité
algorithmique. Plus précisément, l'algorithme AQP alterne des résolutions de
problèmes d'optimisation quadratique. Alors que l'algorithme APLS lève les
contraintes des problèmes d'optimisation quadratique afin de les transformer en
problèmes des moindres carrés régularisé par une norme $L_{2}$. La complexité de
l'algorithme APLS augmente linéairement avec le nombre d'individus dans
l'échantillon. En outre, nous avons mis en place un test de détection de la
sélection reposant sur les estimations spatiales des matrices d'ascendance.

En utilisant des simulations coalescantes, nous montrons que les deux
algorithmiques AQP et APLS retournes des résultats avec la même précision
statistique. Toujours sur des simulations de coalescant nous avons montré que
nos algorithmes reproduise les même erreur statistique que le logiciel =TESS=
2.3 CITER. Le logiciel =TESS= 2.3 permmet egalement l'estimation des coéficient
de métissage a partir de données génétiques et géographique mais en utilisant un
modèle bayésien. Sur les simulations que nous avons considéré nos algorithmes
était 10 à 100 fois plus rapide que le logiciel =TESS= 2.3.
...
celles observées pour un algorithme non spatial =snmf= CITER. Dans nos
expériences numériques les erreurs des méthodes spatiales sont inférieures à
celles observées avec des méthodes non spatiales, et les algorithmes spatiaux
ont permis de détecter une structure de population plus subtile. 

Enfin, nous avons illustrer l'utilisation de notre package R sur millier de
génotypes /A.thaliana/, chacun incluant plus de 210k SNP. Notre méthode a permis
d'exiber la structure de population de l'espece /A.thaliana/ en Europe. Par
...
#+begin_src shell
  ...
  do
      basefile=`basename $file .bed`
      echo "===== $basefile ====="
      plink --bfile $basefile --threads 16 --maf 0.01 --mind 0.05 --geno 0.0 --snps-only --autosome --make-bed --out "$basefile"_CQ
  done
...
#+begin_src R 
  ...
  saveRDS(G, "./Data/ThesisDataset/Simons/G_pruned.rds")

  ## scaling for lfmm and pca
  G.scaled <- scale(G)
  saveRDS(G.scaled, "./Data/ThesisDataset/Simons/G_pruned_scaled.rds")
  ...
  dim(G.scaled.binom)
  saveRDS(G.scaled.binom, "./Data/ThesisDataset/Simons/G_pruned_scaled_binom.rds")
#+end_src
...
#+begin_src R
  ...
  G <- readRDS("~/Projects/Thesis/Data/ThesisDataset/Simons/G_pruned_scaled.rds")
  ...
  ## plot
  expr <- readRDS("./OUTPUT/Expr/Simons_pca.rds")
  indiv.df <- readRDS("./Data/ThesisDataset/Simons/indiv.df")
...
#+begin_src R 
  ...
  ## data
  G <- readRDS("./Data/ThesisDataset/Simons/G_pruned.rds")
  p <- 10000
  G.subsampled <- G[,sample.int(ncol(G), p)]
  dim(G.subsampled)
  indiv.df <- readRDS("./Data/ThesisDataset/Simons/indiv.df.rds")
  coord <- indiv.df[,c("Longitude", "Latitude")] %>% as.matrix()

  ## expr
  expr <- list()
  start.time <- Sys.time()
  expr$vario.gen <- CalculateEmpiricalGenSemivariogram(G.subsampled, 2,  coord)
  end.time <- Sys.time()
  expr$time <- end.time - start.time
