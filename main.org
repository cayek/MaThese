...
causalité entre le facteur latent et les variable expliquées et explicatives mais
...
corrélation entre les variables explicatives et les variables expliquées. Mais
surtout les facteurs de confusion peuvent faire apparaître certaines variables
expliquées comme étant significativement corrélées avec les variables explicatives
...
- Note taken on [2017-08-01 mar. 18:14] \\
  ...
  2017 dans son intro,
:END:
Le cadre méthodologique le plus utilisé en étude d'association est celui des
modèle de régression linéaire. Il s'agit d'un cadre très commun en statistique
...
explicative sur les variables expliquées. La matrice $\E$ est la matrice des
...
cas cela veut dire que la variable explicative $l$ à un effet sur la variable
expliquée $j$. Comme nous l'avons évoqué dans la parti précédente avec ce genre
d'approche il est possible qu'une variable non observé soit corrélé à la fois au
variable expliquées et au variables explicatives (Figure [[graph:conf_factor]]). 

graphe ou pas? 

**** Estimation des facteurs latent à priori
En génétiques des populations, une méthodes il est courant d'estimer les
variables latentes a priori. Par exemple la méthode EIGENSTRAT consiste en une
analyse en composante principale (ACP) de la matrice des données génétiques
cite:Price_2006. L'objectif est alors de capter la structure de population qui
est confondante pour les études d'association. On ajoute ensuite la matrice des
coordonnées sur $K$ axes choisi au coté des variables explicative du modèle de
régression
\begin{equation}
\Y = f(\X \B^{T} + \bar{\U} \V ) + \E
\end{equation}
où $\bare{\U}$ est la matrice des coordonnées calculé par l'analyse en composante
principale. Plus récemment le même principe à été utilisé pour faire un étude
d'association entre des données méthylation de l'adn avec la maladie polyartrite
rumatoide cite:Rahmani_2016.
**** Les modèles mixes
Une autres approche de correction pour facteurs de confusion est le modèle mixe.
Dans un tel modèle modèle ajoute un effet aléatoire à la régression 
\begin{equation}
\Y = f(\X \B^{T} + \U) + \E
\end{equation}
où $\U$ est la matrice des effets aléatoires à estimer. Ce genre de modèle à été
largement utilisé dans les GWAS cite:Kang_2008,Zhou_2014. Classiquement, afin
d'estimer la fois la matrice des effet $\B$ et la matrice des effet aléatoire
$\U$, on fait l'hypothèse que la covariance des effets aléatoires est connue.
Pour les GWAS, cette matrice de covariance est en général estimé à priori sur
...
n'est pas nécessaire de faire une hypothèse sur la structure de covariance des
matrices $\U$ et $\V$. Parmi les différentes méthodes d'estimation des
paramètres du modèle mixes à facteurs latents, on distingue des approche qui
repose sur des algorithmes de Monte Carlo
cite:frichot13_testin_assoc_between_loci_envir,carvalho08_high_dimen_spars_factor_model;
des approches qui repose des algorithme EM (Expectation Maximixation)
cite:friguet09_factor_model_approac_to_multip,agarwal09_regres,zhou16_spars_multiv_factor_analy_regres;
et enfin les approche qui repose sur une estimation des variables latentes à
partir d'une transformation de $\Y$ qui a pour but de séparer la variabilité 
expliquées par les variable latentes de celle expliqué par les variables
explicative cite:gerard2017unifying,wang2015confounder,article_Leek_Storey_2007.
Parmi les méthodes qui estime les facteurs latents sur une transformation de la
matrice des données, on distingue des autres les méthodes dites à contrôles
négatifs qui suppose que connait un sous ensemble de variables expliquées qui ne
sont pas associées avec les variables explicatives. Ces méthodes utilisent donc
les variables dites nulles pour estimer les variables latentes.

*** Problématique et plan
Comme nous l'avons vu dans la parti précédente l'estimation des variable
...
méthodes d'estimation rapides et efficaces des paramètres du modèle LFMM
introduit par l'équation eqref:eq:glfmm dans le cas où la fonction de lien $f$
est l'identité. Nos deux méthodes d'estimation consiste à a séparer isolé le
plus possible la variabilité expliqué de façon systématique par les variable
latentes de celle expliqué par les variables explicatives. Elles sont donc
directement comparable à SVA cite:article_Leek_Storey_2007 et CATE
cite:wang2015confounder. Chacun des algorithmes que nous présentons découle de
l'optimisation d'une fonction objectif pour lesquelles nous montrons qu'ilx
converge vers le point de minimum global. Enfin, nous comparons nos méthodes à
SVA et CATE sur des simulations numériques ainsi que des exemples de GWAS, EWAS
et GEA.
