...
algorithmes d'estimation $L_{1}$ et $L_{2}$. La première est le calcule de la
...
D'après cite:Halko_2011, le calcul des $K$ composante dominante de la
décomposition en valeur singulière demande $O(\Yrow \Ycol \K)$ opérations. Cette
complexité temporelle peut même être réduite à $O(\Yrow \Ycol \log(\K))$ si on
utilise une méthode avec projections aléatoires. 

La deuxième étape importante des algorithmes que nous avons présenté est l'étape
de projection du résidu de l'aproximation matricielle de rang faible sur
l'espace engendré par $\X$. Si on prend en compte toutes les opérations
matricielle, cette étape nécéssite $O(\Xcol (\Yrow \Ycol + \K (\Yrow + \Ycol) +
\Xcol ( \Xcol + \Yrow + \Ycol + 1))$.

Finalement, si on prend en compte les opération matricielle restantes dans les
deux algorithmes, le nombre d'opération est de l'ordre de $O(\Yrow \Ycol \K)$.
L'algorithme d'estimation $L_{1}$ est bien sur plus long car il réalise
plusieurs fois (ce nombre de fois dépend de la vitesse a laquel l'algorithme
itératif converge) les opération de décompostion en valeur singuière et de
projection alors que l'algorithme ne les réalise qu'une seul fois.

Enfin il est important de signaler que les algorithmes d'éstimation $L_{1}$ et
$L_{2}$ ne nécésssite pas de dupliquer matrice des variables explicatives n'est
pas dupliqué en mémoire. En effet, la matrice $\Y$ est de taille $\Yrow \times
\Ycol$  et donc la dupliquer pourrait poser des problèmes pour les grands jeux
de données. Il est même possible d'envisager de ne pas charger la matrice en
mémoire rapide de l'ordinateur (la RAM).
...
* COMMENT perspectives : Vers le big data
:LOGBOOK:
