...
Nous présentons maintenant un algorithme itératif et alterné qui permet
d'estimer les paramètres de LFMM qui minimise la fonction objective $\Llasso$
définie par eqref:eq:optim_lasso_reg.

Nous commencons avec nulle tel que : 
\begin{align*}
\W_{t = 0} & = 0 \\
\B_{t = 0} & = 0.
\end{align*}
...
Enfin nous pouvons démontrer le théorème suivant : 
#+BEGIN_theorem
...
#+BEGIN_proof
PROBLEME : la fonction n'est pas différentiable !!!! je ne peut pas utiliser les
resultats de l'algo de GAuss Saubel cite:Grippo_2000,Bertsekas_1997
A suivre !!!
#+END_proof
...
******* Charger l'expérience et la liste trouver dans les autres papier

******* Que donne la recalibration ?
#+BEGIN_SRC R
  expr$df.res %>%
    group_by(method) %>%
...
#+begin_example
  [[./OUTPUT/Rplots/EWAS_qqplots.png]]
  Warning message:
  Removed 8126 rows containing non-finite values (stat_qq).
#+end_example
