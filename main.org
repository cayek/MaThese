...
$K$ clusters génétiques, aussi appelé populations ancestrales. Dans chaque
cluster génétique l'objectif est d'estimer les fréquences d'allèle pour chaque
SNPs. Pour chaque individu il faut estimer la proportion de son génotype qui
provient de chaque cluster génétique. Les proportions sont appelées coefficients
de métissage individuel, aussi appelés coefficients d'ascendance.
...
illustrer cela nous avons pris un échantillon de données de SNPs d'individus
humains provenant de populations européennes, africaines et afro-américaines des
États Unis d'Amérique. La Figure ref:fig:tess3_intro A montre les fréquences des
...
dans les différentes populations. À partir l'échantillon de données de SNPs
humain, nous avons calculé les coefficients de métissage en supposant 2 clusters
génétiques à l'aide du logiciel =snmf=. Nous constatons que la méthode sNMF
trouve des clusters génétiques africain et européen. Les individus afro-américain
ont des génomes provenant des clusters africain et européen (Figure
ref:fig:tess3_intro B). Il s'agit du résultat attendu connaissant l'histoire
démographique des individus afro-américain.

#+BEGIN_EXPORT latex
\begin{figure}[!t]
\centering
\includegraphics{./OUTPUT/Rplots/tess3_intro.pdf}
\caption{{\bf Structure génétique des populations d'invidus humains européens,
    africains et afro-américains.} A) Distribution des allèles du SNP rs499627 dans
  des populations européennes, africaines et afro-américaines. B) Estimation par
  le logiciel \texttt{snmf} de la proportion pour chaque individu de SNPs provenant des
  clusters génétiques africains et européens.}
