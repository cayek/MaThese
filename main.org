...
#+LATEX_HEADER: \usepackage[natbib=true, style=authoryear, backend=bibtex, sorting=none]{biblatex}
...
les scientifiques. En particulier en génétique l'abondance des données permet de
...
trouver de nouvelles méthodes pour passer à une échelle de données plus grandes.

Par ailleurs, plus nous avons d'observation de variables plus il est possible de
détecter des corrélations faibles entre celles-ci. Ce qui paradoxalement n'est
pas forcément une bonne chose puisque toute corrélation aussi faible soit elle
est significative quand on à trop de données. De plus, une corrélation n'est pas
un lien de cause à effet. Une corrélation entre des variables peut ne plus être
observé quand on considère dans l'étude d'autres variables explicatives
(paradoxe de Simpson). Il faut donc être très prudent dans la recherche de
corrélation dans des données, surtout dans le contexte actuelle où l'accès au
données est très facile.

Dans le cadre de cette thèse nous nous sommes intéressé à développer des
méthodes statistiques adaptées au traitement de données génétiques afin de
répondre à deux problématiques. Le premier est l'estimation de la structure
génétique de population à partir de données génétiques et géographiques. Le
deuxième est l'estimation des facteurs de confusion pour corriger les tests
d'association. Les méthodes statistiques développer lors de cette thèse repose
sur la factorisation de matrice. Nous allons maintenant introduire plus en
détails les contextes biologiques dans lesquels interviennent les méthodes
développées durant cette thèse.
...
de comprendre la transmission des maladies génétiques. En agronomie, où des
...
ensemble d'individus qui montrent une unité de reproduction : les individus
d'une population peuvent se croiser entre eux, ils se reproduisent moins avec
les individus des populations voisines, desquelles ils sont géographiquement
isolés. On considère 4 pressions évolutives qui permettent d'expliquer des
changements de fréquences d'allèle dans les populations : la mutation, la
sélection, la dérive génétique, et la migration. La mutation créer de nouveau
allèle, elle peut être neutre, c'est à dire sans effet sur l'organisme, ou non
et avoir par exemple un effet sur la capacité de l'organisme à s'adapter à son
environement. La sélection est l'hypothèse centrale de la théorie de Darwin,
ainsi si un allèle permet à l'organisme d'être mieux adapté à son environnement
alors cette allèle aura plus de chance d'être transmis aux générations futures
car l'organisme porteur de l'allèle peut se reproduire plus facilement. La
dérive génétique provient du fait qu'il n'existe pas de populations infinies.
Ainsi au file des générations les distributions allélique change à cause de la
reproduction au hasard des individus. Ce processus peut engendrer la disparition
ou la fixation d'un allèle neutre juste par hasard. Enfin la migration est le
passage de gènes d'une population à une autre (par des individus qui passent
d'une population à l'autre, par exemple sous forme de graines ou de pollen chez
les plantes). Si cet échange se fait entre des populations ayant des fréquences
alléliques différentes, il va tendre à modifier les fréquences alléliques.
...
des populations moderne utilise des données d'observation de génomes de
plusieurs individus provenant de plusieurs populations. De plus, les récentes
améliorations en séquence de l'ADN ont permis le séquence, qui était jusqu'alors
réservé aux espèces modèles, de nombreuses espèces différentes. Dans le cadre de
cette thèse nous nous sommes intéressé seulement au données dîtes de SNPs
...
de bases du génome, entre individus d'une même espèce. Les SNPs représentent $90
\%$ de l'ensemble des variations génétiques humaines, et des SNPs avec une
fréquence allélique supérieure à $1 \%$ sont présents toutes les cent à trois
cents paires de bases en moyenne dans le génome humain. Nous appelons locus une
position sur l'ADN, nous parlons ainsi du locus d'un SNP. Nous représentons les
données génétique comme une matrice avec une ligne par individu et une colonne
par locus.

Une étape très imporante en génétique des populations et particulièrement quand
on étudie des données de SNPs est d'inférer une représentation synthétique de la
structure de population à partir des données de SNPs. Dans cette thèse nous
proposons une méthode permettant l'inférence des coefficients de métissage
individuels à partir de données de SNPs et géographique de chaque individu. Nous
...
Un problème fondamental en science du vivant est de détecter les relations de
causalité qui existes entre des événements. En statistique, un événement est
modélisé par une variable aléatoire. Il est seulement possible détecter des
liens statistique entre les variables aléatoires, qu'on appel corrélation. La
corrélation renseigne sur les probabilités jointes des variables aléatoires en
question. Dans un cadre statistique nous parlons de test d'association quand on
test si deux variables aléatoires sont corrélées.

Les tests d'association sont très utilisée en génétique pour comprendre les
fonctions des gènes. Par exemple on peut chercher quel SNP est corrélé à une
maladie pour comprendre les causes génétiques celle-ci. Cependant, comme nous
l'avons vu dans la partie précédente il existe de nombreux facteurs responsables
de la diversité génétique. Quand les facteurs de variation du génome sont
corrélés à la variable d'étude (la maladie par exemple) alors les études
d'association sont faussées et on observe en général une augmentation du nombre
de gènes associés à la variable d'étude. Une telle situation n'est pas
souhaitable car bien qu'il agit de corrélation, il ne s'agit pas de corrélation
intéressante pour la biologique. Dans les données génétique qui nous
intéresse dans cette thèse, il est possible d'inférer les facteurs responsable
de la variation génétique. On appel de tel facteur les facteurs de confusion
pour le test d'association. Les facteurs de confusion doivent être pris en
compte dans les étude d'association pour éviter les fausses associations.

Dans cette thèse nous proposons une méthode statistique qui permet de corriger
les tests d'association pour les facteurs de confusion. Nous expliquons plus en
détail ce que sont les facteurs de confusions et comment nous les modélisons
dans la partie [[sec:lfmm_intro]].
...
pose de nouveau problème aux statisticien. L'objectif de cette thèse de doctorat
est d'améliorer les outils statistique qui permettent aux biologistes de
répondre à des question concrète sur le vivant. Dans cette thèse nous nous
sommes intéressé à deux problématique en analyse de données génétiques :
l'estimation de la structure génétique de population et les test d'associations.
L'accent a été mis sur la rapidité des méthodes développées afin qu'elles soient
applicables aux données génétiques modernes. En outre, afin que nos méthodes
statistiques soient utilisables par la communauté scientifique en génétique, il
a été important de rendre accessible une implémentation informatique efficace
des nouvelles méthodes.

Dans un premier chapitre nous présentons un cadre méthodologique permettant
l'inférence de l'ascendance à partir de données génétique et géographiques. Dans
une deuxième partie nous présentons un cadre méthodologique permettant d'estimer
les facteurs de confusion qui augmente le nombre de fausses découvertes dans les
test d'association. Chaque chapitre commence par une introduction du contexte et
de la problématique biologique. Nous développons ensuite notre méthodologie
statistique. Enfin, nos méthodes sont comparées aux méthodes existantes et
appliquées à des jeux de données réelles de la biologie afin d'illustrer leurs
utilités.
...
l'étude de données génétique. Les données génétiques sont volumineuse et
multivariées. La structure de population fournit une représentation
synthétique qui de visualiser la variation génétique induite par la
stratification en population. La stratification en population ainsi observée
fourni des informations sur l'histoire et l'évolution démographique de l'espèce
étudié cite:Li_2008; il est également indispensable de l'utiliser comme facteur
de correction dans les études d'associations avec un phénotype, un gradient
environnementale ou encore une maladie cite:marchini2004effects. De même, il
existe de nombreuses applications en médecine génétique nécessitant de connaitre
la structure de population comme par exemple le calcule d'un score de risque
génétique pour une maladie cite:Wray_2013. Enfin, l'étude de la répartition en
population d'une espèce dans son habitat est une étape clée en génétique du
paysage cite:Fran_ois_2015.

Pour représenter la structure génétique de population, nous supposons que le
génome de chaque individu est la combinaison de morceau de génome provenant de
$K$ cluster génétiques (aussi appelé population ancestrale). Dans chaque cluster
génétique nous devons estimer les fréquences d'allèle pour chaque SNPs. Pour
chaque individu nous devons estimer la proportion de son génotype qui provient
de chaque cluster génétique. Ces proportions sont appelées coefficients de
métissage individuel (aussi appelés coefficients d'ascendance).
...
des génomes provenant des clusters africain et européen. Il s'agit du résultat
attendu connaissant l'histoire démographique des individus afro-américain.
...
L'acces à des données génétype de plus en plus grandes on provoquer l'émergence
...
de monte carlo pour estimer la distribution à posteriori des coefficients de
métissage et les fréquence d'allèle dans les clusters génétiques. Cependant les
algorithmes de monte carlo ne passe à l'échelle des grands jeux de données
génétiques modernes. Il a été proposé des améliorations du logicielle structure
reposant sur une fonction de vraisemblance définie pour la matrice des
coefficients de métissage et les fréquences d'allèle dans les clusters.
L'estimation est effectuée en maximisant la fonction log-vraisemblance. La
...
cite:Tang_2005. Des algorithmes de vraisemblance plus récents sont implémentés
dans les programmes =admixture= et =fastStructure= cite:Alexander_2011,Raj_2014.
...
précision les résultats des approches considérant une vraisemblance
cite:Frichot_2014. En outre, cette catégorie de méthodes fournie des algorithmes
...
de la structure de populations sont induits par les pressions évolutives qui ont
une réalité dans l'environnement géographique de l'espèce. Les clusters
génétiques peuvent par exemple être générés par l'isolation des populations à
cause d'une mer les séparant ou bien des différences d'altitude entre celles-ci.
L'étude réalisé par cite:Novembre_2008 a montré qu'il est possible de prédire la
position des individus à partir de l'étude de leur structure de population. De
nombreuse méthodes ont permis d'améliorer la prédiction de la position
géographique des individus à partir de leur génomes
...
l'information géographique. Cette idée a été exploitée pour améliorer le modèle
...
cite:FRAN_OIS_2010. Ainsi, les méthodes d'estimation des coefficient d'ascendance
à l'aide de données géographiques ne sont pas adapté aux grands jeux de données
modernes. 
...
régularisé avec des contraintes convexes. Nous proposons deux algorithmes qui
résolvent le problème de factorisation. Le première algorithme repose sur
...
génotypes ancestraux. Alors que les algorithmes AQP ont un fondement théorique
bien établi par cite:Bertsekas_1997, ce n'est pas le cas des algorithmes APLS.
En utilisant des simulations coalescentes, nous montrons que les estimations
...
APLS est du même que l'erreur obtenu avec le logicielle =TESS= 2.3, une méthode
bayésienne. Toujours sur des simulations nous montrons que notre algorithme
spatial APLS estime mieux la structure de population que la méthode sNMF.
L'algorithme de sNMF repose aussi sur un problème de factorisation de matrice
mais n'utilise pas l'information spatiale. Enfin, nous discutons de
l'application de nos algorithmes aux données des écotypes européens de
/Arabidopsis thaliana/, pour lesquelles des données géographiques individuelles
et géographiques sont disponibles cite:Horton_2012.
...
*** Matrices d'ascendance génétiques

Nous considérons une matrice de génotype, $\Y$, enregistrant des données de $n$
individus à $p$ locus polymorphes pour une espèce ayant une ploidy de $d$, c'est
à dire qui possède un génome composé de $d$ exemplaires de chaque chromosome.
Pour les SNPs autosomiques dans un organisme diploïde, le génotype au locus
$\ell$ est un nombre entier, 0, 1 ou 2, correspondant au nombre d'allèles de
référence observé à ce locus. De même que dans cite:Frichot_2014, dans nos
algorithmes nous utilisons des formes disjonctives pour coder les génotypes. Par
exemple pour un organisme diploïde, le nombre d'allèles observés à chaque locus
$,0,1,2$ est encodée comme $100$, $010$ et $001$. Pour les organismes de ploidy
$d$, il existe $(d + 1)$ génotypes possibles à chaque locus, et chaque valeur
est encodée sous une forme disjonctif unique.
...
l'autre est fixée, nous pouvons appliquer l'algorithme de descente par bloc de
...
étape, le problème ref:eq:AQPq ne peut pas être séparé en plus petits
...
pas de résultat qui garantisse la convergence de d'APLS vers un minimum local de
...
hasard les matrices $\Q$ et $\G$ puis alterne deux étapes. La matrice $\Q$ est
calculée pendant que la matrice $\G$ est fixé et vice versa. La première étape de
...
problèmes indépendants. Cette opération peut être parallélisée. Ensuite nous
...
nombres réels non-négatifs. D'après le théorème spectral nous avons
\begin{equation}
...
$\hat{\Q}$, puis la matrice $\Q$ est calculée par la projection de $\U \hat{\Q}$
...
longue tradition en génétique des populations. Des approches populaires
reposent sur le test de Mantel cite:mantel1967 et la mesure de
l'auto-corrélation spatial cite:HARDY_1999,Epperson_1996. Avant d'utiliser notre
méthode spatiale d'estimation des coefficients de métissage, nous proposons de
choisir des valeurs de l'échelle géographique en visualisant le variogramme
spatial cite:Cressie1993. Le variogramme peut être étendu aux données génétique
de la facons suivante
\begin{equation}
...
utilisant l'erreur quadratique moyenne (RMSE, Root Square Mean Error) entre le
génotype prédit et le génotype réellement observé.
...
large majorité des locus observé sont neutres. La migration et la dérive
génétique influence de la même façon les distributions alléliques de tous les
locus neutre induisant une différentiation typique entre les différents clusters
génétiques identifiés par les méthode d'estimation de la structure. Les locus
impliqué dans l'adaptation local peuvent être identifier en cherchant les locus
où on observe une différentiation anormal entre les clusters génétiques
cite:Lewontin175. Nous proposons de calculer une statistique de différentiation
entre les fréquences génomique dans les clusters génétiques inférées par nos
algorithmes pour détecter les locus sous adaptation à l'environnement.
...
\end{equation*}
pour lequel une distribution du chi2 à $K-1$ degrés de liberté est attendue sous
l'hypothèse nulle. L'hypothèse nulle est ensuite calibrée empiriquement en
mesurant le niveau différentiation correspondant à une différentiation neutre.
Nous utilisons pour cela le facteur d'inflation génomique
cite:Devlin_1999,Fran_ois_2016. Après une calibration du test, le contrôle du
taux de découverte fausse est effectué en utilisant l'algorithme de
...
modèle très proche de celui considéré ici.
...
Des sous-échantillons provenant du jeux de données réelles d'/Arabidopsis
...
génotypes en mélangeant les génotype des deux populations simulées avec =ms=
avec des proportions de mélange qui varient continuellement selon un gradient
longitudinal cite:Durand_2009,FRAN_OIS_2010. Dans ce scénario, les individus à
chaque extrémité de la zone géographique sont représentatifs de leur population
d'origine, tandis que les individus au centre de la gamme partagent des niveaux
intermédiaires d'ascendance dans les deux populations ancestrales. Pour ces
simulations, la matrice des coefficients de métissage, noté $\Q_0$, est
entièrement décrite par la position des individus échantillonnés.

*DESSIN comme celui que je mets dans mes slides ??*
...
10^6$ pour chaque population. Le taux de mutation par paire de base et génération a
...
#+end_src
***** RUNNING Fusion et conversion en bed
...
#+begin_src shell
  ...
  do
      ...
      cd $dir
      rm all_beds.txt
      for file in *.chr[0-9][0-9].vcf.gz *.chr[0-9].vcf.gz
      do
          echo "===== $file ====="
          ## convert into bed
          basefile=`basename $file .vcf.gz`
          plink --vcf $file --biallelic-only --out $basefile
          echo $basefile >> all_beds.txt
      done
      ## fusion
      plink --merge-list all_beds.txt --make-bed --out ../"$dir"
      ## rm bed files
      cd ../
      rm -f "$dir"/*.{bed,bim,fam,log}
  done
...
#+begin_src shell
  ...
  plink --bfile Simons --maf 0.05 --mind 0.05 --geno 0.05 --snps-only --autosome --make-bed --out Simons_CQ --threads 16
#+end_src
***** TODO Prunning
:LOGBOOK:
- State "TODO"       from              [2017-09-28 jeu. 19:45]
:END:
***** TODO Conversion au format R et scalling
:LOGBOOK:
- State "TODO"       from              [2017-09-28 jeu. 19:45]
:END:
****** Data for tess3r
Il me faut les données pruned et la représentation binaire
****** Data not pruned for lfmm
Il me faut les données non pruned au format matter que je scale 
****** Data pruned for lfmm
...
estimations des matrices $\Q$ et $\G$ obtenues avec AQP et APLS. La référence
...
sources simulées par =ms=. Pour la matrice $\G$, la matrice de référence, notée
($\G_0$), est calculée à partir des fréquences empiriques des génotypes des
deux populations sources. Les erreurs quadratiques moyennes (RMSE) pour les
estimations des matrices $\Q$ et $\G$ diminue à mesure que la taille de
...
matrices $\Q_0$ et $\G_0$. Les résultats permettent également de vérifier que
...
niveau de différentiation moyen $F_{\rm ST}$. Cela a permis de créer des jeux de
...
matrices $\Q$ et $\G$ varie entre $0.02$ et $0.15$ (Figure
...
du nord-ouest de l'Europe cite:Horton_2012.
...
sélection naturelle cite:rellstab15_pract_guide_to_envir_assoc. En un court laps
de temps, les études d'association ont permis des progrès considérables dans
...
autre le concept de d'hypothèse nulle en statistique cite:fisher1937design. Dans
...
simulation numérique. Pour cela nous simulons une variable $\X$ et une variable
...
contrôles négatifs qui supposent connu un sous ensemble de colonnes de $\Y$ qui ne
sont pas associées avec $\X$. Les méthodes à contrôles négatifs utilisent les
...
présentons sont comparables aux méthodes sva cite:article_Leek_Storey_2007; et
cate cite:wang2015confounder; qui procède d'une façon très similaire. Nous
...
** Nouvelles méthodes de correction pour les facteurs de confusions
...
composante principale de la matrice des variables expliquées $\Y$. Nous
...
prédiction cite:Bro_2008. Nous présentons maintenant plus formellement notre
...
** Autres méthodes de correction pour les facteurs de confusions
<<sec:similar_method>>
...
matrice de variables expliquées $\Y$ d'un jeux de données réelles. Pour obtenir
les données simulées nous réalisons une analyse en composante principale de
...
cite:Devlin_1999. Si les \pvalues des fausse associations sont réparties
...
utiliser l'algorithme de Benjamini-Hoshberg cite:benjamini1995controlling ou
bien la \qvalue cite:Storey_2011.
...
polyarthrite rhumatoïde et chez 335 individus sains cite:Liu_2013. Il est connu
...
ici cite:Rahmani_2016,Zou_2014. Afin d'évaluer la capacité des méthodes
...
par les études cite:Rahmani_2016 et cite:Zou_2014 qui prennent en compte
...
fortes associations avec certains gènes cite:dubois2010multiple; laissant
...
population cite:Price_2006. Nous proposons ici de faire une étude d'association
entre la maladie \celiac et des données génétique présentées dans
cite:dubois2010multiple. Ces données comportent 281122 SNPs pour 15155
...
cite:Purcell_2007. Enfin nous avons utilisé le logiciel beagle pour imputer les
données manquantes de la matrice de SNPs cite:Browning_2016.
...
environnement cite:Darwin. Ainsi les organismes les mieux adaptés à leur habitat
...
génétiques du projet 1000Genome phase 3 cite:1000Genome_2015. Ces données
...
climatiques à partir de la base de données WorldClim 2.0 cite:fick17_world. Nous
...
cite:McLaren_2016. Les annotations retournés par VEP sont classées en catégorie
...
facteurs de confusions dans les études d'association. Les deux nouveaux
...
proportion des candidats identifiés par cite:Zou_2014,Rahmani_2016 qui sont
retrouvés dans les "top" listes de chaque méthode. Nous rappelons que les
candidats identifiés par cite:Zou_2014 et cite:Rahmani_2016 sont au nombre de 5.
...
le rang 11881 pour trouver le premier candidat de cite:Zou_2014,Rahmani_2016 et
le rang 138038 pour tous les avoir. Pour la méthode sva-irw, les candidats de
cite:Zou_2014,Rahmani_2016 sont tous identifiés entre les rangs $5111$ et $87659$.
...
cite:Zou_2014 et cite:Rahmani_2016 (Figure ref:fig:ewas_venn). Nous remarquons
...
#+begin_src R :results output latex :exports results :session *R* :dir ~/Projects/Thesis/MaThese/
  ...
  ewas.table %>%
    xtable(align = "lllllcccc",
           digits = -2, type = "latex",
           caption = "{\\bf Sites de méthylation associés avec la maladie polyarthrite rhumatoïde}. \\pvalues des candidates détectés par les méthodes PCAlm, lassoLFMM, ridgeLFMM et cate pour un taux de fausse décourverte controlé à 1$\\%$ pour l'EWAS. Les Sites de méthylation en gras sont les candidats de \\cite{Zou_2014, Rahmani_2016}",
           label = "table:ewas") %>%
...
\celiac cite:dubois2010multiple. Nous avons séparer les SNPs candidats du GWAS
...
#+begin_src R :results output latex :exports results :session *R* :dir ~/Projects/Thesis/MaThese/
  ...
  table.df %>%
    xtable(align = "lp{4cm}ll", type = "latex",
           label = "table:geas",
           caption = "{\\bf SNPs associés avec un gradiant climatique qui ont été associés avec des phénotypes dans d'autres études}. SNPs présents dans l'union des SNPs détectés par les méthodes lassoLFMM, ridgeLFMM, cate et PCAlm pour un le FDR à $1\\%$.") %>%
    print(include.rownames=FALSE,
          sanitize.colnames.function=identity,
          sanitize.text.function=identity,
          floating = TRUE,
...
- Note taken on [2017-09-25 lun. 15:47] \\
  ...
  - quand on prend pas en compte les variables de confusions on est mal calibré et
...
facteurs de confusions. En outre, la méthode lm a montré moins de puissance pour
...
la méthode oracle qui connait les facteurs de confusions. Les \pvalues renvoyé
...
résultats qu'une autre sur des simulations bien choisis cite:Wolpert_1997. Sur
...
évoluent, il est donc nécessaire que les méthodes d'analyse évoluent également.
Dans le cadre de cette thèse nous avons proposé plusieurs algorithmes basés sur
des problèmes de factorisation de matrice. Pour certain algorithme nous avons
prouvé leur convergence vers un point critique de leur fonction objectif. Nous
avons démontré l'utilité de chacun de nos algorithmes sur des données réelle
afin de répondre à des questions de la génétique moderne. Des implémentations
efficace de nos algorithmes sont disponibles dans deux packages R : tess3r et
lfmm. Nous proposons maintenant différente perspectives de nos travaux.
...
Le choix de l'implémentation des package en R n'est pas anodin. La communauté R
est très vivante et en particulier en analyse de données biologique. Les package
que nous avons développé peuvent ainsi être des pipeline d'analyse de donénes
faisant intervenir d'autre package R. Par ailleurs, le language de programmation
R est un language "aux niveau" permettant une maintenance plus facile que sur
des language de programmation bas niveau. Une perspective majeur de nos traveaux
est de maintenir le package en fonction des mise à jour du langague R ou bien
des rapport d'erreur renvoyé par les utilisateurs. 

Un autre aspect important du développement de nos package est la mise en plca
d'une documentation complète. Au même titre que le développement des fonction
informatique la documentation dois évoluer avec les retours des utilisateurs.
Les fonctions des packages tess3r et lfmm sont documenté et nous proposons des
documentation plus longue proposant une vu d'ensemble des fonctionnalité des
packages. Dans l'écosystème R les documentation longue d'un package sont appelé
vignette. Il sera donc important de faire évoluer la documentation des fonctions
ainsi que leur vignette en fonction des retours des utilisateurs. 

Les packages tess3r et lfmm repose sur des structure de données matricielle.
Nous avons choisi d'utilisé la structure matricielle classique du language R
afin d'etre le plus généraliste possible. Un question majeur pour les
utilisateur est comment charger les données en mémoire vive afin d'utiliser nos
outils. Certain format de fichier se sont imposer dans la communauté de la
génétique comme le format du logiciel plink .bed et les .vcf. Ajouter une option
permettant au utilisateur de fournir dirrectement un fichier dans un de ces
format standard serait un réelle avantage. En outre, certain format de fichier
ont été pensé pour permette une acces rapide aux données génétique à la manière
des algorithmes onlines[fn:online], c'est le cas par exemple du format de bed.
Ne pas charger toutes les données dans la mémoire vive de l'ordinateur
permettrait au utilisateur ayant des capacité en mémoire vive modeste d'utiliser
nos algorithmes.

Enfin pour rendre nos outils utilisable par le plus grand nombre il est très
important de communiquer sur ceux-ci. En particulier, il faut inciter les
utilisateurs a poser des questions sur les platforme d'échange du type Stack
Exchange comme [[https://www.biostars.org/][biostars]] et [[https://stackoverflow.com/documentation/r/topics][stackoverflow]]. C'est platform permettent des échange
avec les utilisateurs a la fois sur les aspect pratique d'utilisation des
pacakge mais aussi sur les aspect plus généraliste d'analyse de données avec nos
outils.
...
Les données réelles analysée lors de cette thèse peuvent être qualifier de
"propre". Il y a peut d'erreur dans les données de SNPs et peut de données
manquante. Cependant, il est fréquent sur les grands jeux de données biologique avec
beaucoup variables qu'un certain nombre de variables ne soit pas observé pour
tous les échantillons. Ou bien qu'il y ai des erreurs dans les variables
observé. Il est fréquent par exemple que les données génétique comporte des
erreurs de séquençage. Des données génétique de très mauvaise qualité sont par
exemple les données génétique prélevé sur des espèce qui se sont éteintes.

Les algorithmes que nous avons développé ont été pensé pour fonctionné sur des
matrice de données complètes. Dans le cas des données génétique l'imputation
pour les données manquantes est très étudiés cite:Browning_2016. L'approche la
plus simple est d'imputer les valeurs manquantes avant de considéré les étude
d'assocaition ou d'inférence de la structure de population. Cette approche
fonctionne très bien pour les espèce des étudié comme l'humain car on a accès à
beaucoup de génomes complet pouvant servir de référence pour l'imputation
cite:Howie_2012. Pour des espèce ancienne ou simplement moins étudier
l'imputation est moins évidentes. 

Les algorithme que nous avons développé peuvent être découpé en une suite
d'opération matricielle. Les opération matricielle n'étant pas défini avec des
matrice incomplète il est nécéssaire d'adapter les algorithmes. Nos algorithmes
repose sur des fonctions objectifs. Il est possible d'écrire les fonction
objectifs avec des matrices imcomplète. Nous perdons cependant les structure
matricielle des variable à a optimisé. Il faut alors envisager d'autre
algorithme d'optimisation que les algorithmes de descente par bloque de
coordonnées utilisé dans nos algorithmes. Une autre approche simple a implémenté
consiste à imputer les données manquante par leur moyenne ou une autre
statistique. Quand il y a peut de données manquante, on s'attend a ce que cette
méthode ne biaise pas trop les facteurs dans les méthodes à facteurs comme l'ACP
cite:josse2009gestion. 

Une autre approche consiste pour prendre en compte les données manquante consite
à imputer au hasard une première fois. Puis dans le cadre d'un algorithme
d'estimation itératif à chaque étape on utilise la valeur prédite par le modèle.
Il s'agit de l'approche utilisé par l'algorithme SOFT-IMPUTE prévu pour
fonctionner sur des matrice de données très incomplète
cite:mazumder10_spect_regul_algor_learn_large_incom_matric. Cette approche
pourrait être directement adaptable au algorithme APLS, AQP et lassoLFMM qui
sont des algorithme itératif. Nous perdons cependant les propriété de
convergence démontré sur le algorithme AQP et lassoLFMM.

On attribue parfois un score sur la confiance que l'on a sur la mesure d'un
génome. Une méthode simple consiste à filtrer les observation ayant un score
trop faible pour les traiter comme des données manquante. De la même manière les
valeur extrême d'un variable peuvent être considéré comme abbérante et considéré
comme manquante. Le problème est autrement plus complexe lorsque que nous ne
somme pas capable de détecter les erreurs de mesure présentes dans les données
biologiques. Nous avons évalué dans le chapitre [[chap:tess3]] l'effet du bruit dans
les données géographique sur nos algorithme spatiaux d'estimation de
l'ascendance. Cependant, nous pouvons nous interroger sur l'effets des valeurs
aberrantes présentes dans les données génétique. Ainsi que comment les erreurs
de mesure peuvent influencer les étude d'association et structure faite à partir
de notre package lfmm.

** Les études d'associations statistiques
Il e
** Source de confusion dans les études d'association avec données de SNPs
