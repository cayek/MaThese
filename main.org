...
\Y = \X \B^T + \U \V^T + \E,
\end{equation}
nous appellerons ce modèle le modèle mixte à facteurs latents. Dans cette
équation, $\Y$ est la matrice des variables expliquée de taille $\Yrow
\times \Ycol$, où $\Ycol$ est le variables et $\Yrow$ le nombre d'observations.
Par exemple, les variables observées peuvent être SNPs ou bien des niveau de
méthylation. La matrice $\X$ de taille $\Xrox \times \Xcol$ représente les
variables explicatives. Les variables explicatives peuvent être par exemple une
maladie ou un gradient environnementale comme la température. La matrice des
effets de taille $\Ycol \times \Xcol$ est notée $\B$. 
...
***** RUNNING Comparaison numérique sur des simulations
:LOGBOOK:
...
#+begin_src R :results output :exports both
  ...
  samplers <-  s * param(prop.outlier = c(0.01, 0.05, 0.1))
...
#+begin_src R :results output :exports both
  ...
  plot_CV_ridgeLFMM(res.cv$errs, major = "lambda")
  plot_CV_ridgeLFMM(res.cv$errs, major = "K")
#+end_src
***** TODO GWAS
:LOGBOOK:
...
#+begin_src R :results output :exports both
  ...
  lambdas <- c(1e-5, 1.0, 1e10) / n
  nb.cluster <- 4
  cv <- method_CV_ridgeLFMM(n.fold.col = 10, n.fold.row = 4,
                            lambdas = lambdas,
                            Ks = c(1,2,5,10,25, 50))
