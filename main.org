...
régularisé en norme $L_{2}$. Nous montrerons que cette algorithme correspond a
un point de minimum local du problème d'optimisation.
...
\lambda \norm{\B}^{2}_{2}
\end{equation*}
est indispensable pour distinguer l'effet des variables latentes de celui des
...
\end{equation*}
Ainsi, les points minimuns d'une tel fonction objective ne sont pas pertinents pour
notre problème.

Afin de calculer les estimateurs qui minimise $\Lridge$ nous commençons par
...
\times \Yrow$ contenant les éléments
\begin{equation*}
...
$\lambda$ qui enlève la bonne proportion de corrélation avec la variable
explicative $\X$. Nous expliquons dans la partie [[sec:hyperparametre]] plus en
...
donc trouver le point de minimum global en annulant la dérivé de $\Lridge$
par rapport à $\B$
\begin{equation}
...
\left \{ \sigma_{j} \left \}_{j = 1..\Xcol}.
\end{equation*}
L'écriture de $\mathcal{L}^{'}$ se simplifie comme ceci
\begin{equation*}
...
\end{align*}
est bien un point de minimum global de $\Lridge$.
#+END_proof
