...
est fermé. Comme $g$ est convexe et différentielle on a pour tout $x \in \X$
\begin{equation}
\label{eq:lassoProof1}
f(x) - f(\bar{x}) \geq \nabla g(\bar{x})(x - \bar{x}) + 
\Sum_{i = 1]^{N} (f_{i}(x_{i}) - f_{i}(\bar{x}_{i}))
\end{equation}
où $\nabla g(\bar{x})$ est la dérivé de $g$ en $\bar{x}$. On a ensuite 
\begin{equation}
...
\end{equation}
où $\nabla_{i} g(\bar{x})$ est la dérivé le long de la $i$ième variable. Or nous
savons par construction que $\bar{x}$ est tel que pour tout $i$ 
\begin{equation}
f(\bar{x}) \leq f(\bar{x}_{1}, ...,x_{i},..., \bar{x}_{N}), ~ \forall x_{i} \in X_{i}
\end{equation}
Or comme les fonctions $f_{i}$ sont convexe on sait qu'il existe $s_{i}$ tel que 
\begin{equation}
\Sum_{i = 1]^{N} ( \nabla_{i} g(\bar{x})(x - \bar{x}) + 
 f_{i}(x_{i}) - f_{i}(\bar{x}_{i}))
\end{equation}
