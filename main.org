...
La méthode cate proposée par cite:wang2015confounder est comparable à notre méthode
ridgeLFMM. La 
...
#+begin_src R 
  ...
  methods <- m.ridgeLfmm * param(force = FALSE, save = TRUE) +
    ...
    m.sva * param(force = FALSE, save = TRUE) +
  ...
  df.res <-
    ...
    foreach(f = dat.files, .combine = 'rbind') %dopar%
    {
      ...
      ExpRextractor_fdr(dat, m.res, rep.sampler = dat$meta$i, rep.method = 1)
    }
  doParallel::stopImplicitCluster()
  parallel::stopCluster(cl)
...
plot et de la validation croisée de l'estimateur $\L_{2}$ de LFMM.
