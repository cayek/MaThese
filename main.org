...
***** TODO Format =Matter=
:LOGBOOK:
- State "TODO"       from              [2017-08-02 mer. 15:59]
:END:
#+NAME: code:1000g_G_matter
#+CAPTION: Dépend de [[code:1000g_G_bigsnpr]]
#+begin_src R :session *krakR* :results output :dir /scp:cayek@krakenator:~/Projects/Thesis/MaThese/
  setwd("~/Projects/Thesis/MaThese/Data/1000Genomes/Phase3/")
  library(matter)
  G.matter <- matter_mat(nrow = 1758,
                         ncol = 5398440)

  ## for col and row names
  G.bigsnpr <- readRDS("bigsnpr_G/G.rds")

  ## convert in matter format
  rownames(G.matter) <- G.bigsnpr$fam$sample.ID
  colnames(G.matter) <- G.bigsnpr$map$marker.ID
  file.copy(paths(G.matter), "~/Projects/Thesis/MaThese/Data/1000Genomes/Phase3/G.matter.bin")
  paths(G.matter) <- "/home/cayek/Projects/Thesis/MaThese/Data/1000Genomes/Phase3/G.matter.bin"
  saveRDS(G.matter, "G.matter.rds")

  ## copy data
  setwd("~/Projects/Thesis/MaThese/Data/1000Genomes/Phase3/")
  library(matter)
  G.matter <- readRDS('G.matter.rds')
  G <- readRDS("G.rds")
  G.matter[,] <- G[,]
  for(i in 1:nrow(G)) {
    message("row ",i)
...
d'association à l'échelle de l'épigenome (EWAS pour epigenome-wide association
...
caractérisation de la variation observé dans l'ARN par rapport à différents
...
considérables dans l'identification des variantes de gènes qui confèrent une
...
aux problèmes des facteurs de confusion et de causalité. Nous appelons ici
"variables explicative", les variables qui sont explicitement modélisées dans
l'analyse et pour lesquelles les corrélations avec les variables que l'on
souhaite expliquer sont étudiées. Les variables explicative correspondent
généralement à un état pathologique, à un trait phénotypique ou à gradient
environnementale. La confusion surviennent lorsque il existe une ou plusieurs
variables latentes, c'est à dire non observées, qui sont corrélées à la fois aux
variables explicatives et aux variables expliquées (fréquences de SNP,
expression génique ou profils de méthylation de l'ADN).
Ainsi un facteur latent devient un facteur de confusion pour une étude
d'association quand il y a un lien de causalité entre le facteur latent et les
variable expliquées et explicatives mais qu'il n'y a pas relation de causalité
en les variables expliquées et explicatives, cette situation est résumé pas la
figure [[graph:conf_factor]]. Si les facteurs de confusion ne sont pris en compte
dans l'étude d'association cela peut induire de la corrélation entre les
statistiques de test utilisées pour détecter la corrélation entre les variables
explicatives et les variables expliquées. Mais surtout les facteurs de confusion
peuvent faire apparaître certaines variables expliquées comme étant
significativement corrélées avec les variables explicatives à cause du facteur
de confusion qui n'est pas pris en compte cite:wang2015confounder. Le problème
des facteurs de confusion est connue depuis longtemps. En effet, on le retrouve
déjà dans l'ouvrage /The Design of Experiement/ de Ronald Fisher qui introduisit
entre autre le concept de d'hypothèse nulle en statistique
cite:fisher1937design. Depuis les facteurs de confusion ont reçu différentes
dénominations comme celle des "facteurs latents" (la dénomination que nous
utilisons ici)
cite:wang2015confounder,frichot13_testin_assoc_between_loci_envir; les
"variables de substitution" cite:Chen_2017,article_Leek_Storey_2007; et la
"variation indésirable" cite:Gagnon_Bartsch_2011,gerard2017unifying.
...
#+CAPTION: Graphe de corrélation entre la variable expliqué $y$ la variable explicative $x$ et le facteur de confusion u. Dans cette situation la variable explicative n'a pas d'effet sur la variable expliqué ...
...
latents peuvent être confondus avec les variables primaires en raison de la
...
\Y = f(\X \B^{T}) + \E
\end{equation}
où $\Y$ est la matrice des variables expliquées de taille $\Yrow \times \Ycol$,
où $\Yrow$ est le nombre d'observations des $\Ycol$ variables expliquées. La
matrice $\X$ de taille $\Xrow \times \Xcol$ est la matrice les variables
explicatives et la matrice $\B$ représente les effets de chacune des variables
explicatives sur les variables expliquées. La matrice $\E$ est la matrice des
erreurs. Enfin la fonction $f$ est la fonction de lien entre les variables
expliquées et les variables explicatives. L'objectif est de trouver les
coefficients $b_{j,l}$ qui sont significativement différents de zéro. Dans ce
cas cela veut dire que la variable expliquée $j$ est potentiellement associée à
la variable explicative $l$. Comme nous l'avons évoqué dans la partie précédente
avec ce genre d'approche il est possible qu'une variable non observée soit
corrélé à la fois aux variables expliquées et aux variables explicatives. Dans
ce cas on pourrait une situation un très grand nombre de variables expliquées
sont significativement corrélées avec les variables explicatives (voir partie
[[sec:simu_conf]] pour une simulation de cette situation). Nous allons maintenant
présenter les différentes approches qui prennent en compte les facteurs de
confusions.

**** Estimation des facteurs latent à priori
En génétiques des populations, il est courant d'estimer les variables latentes à
priori. Par exemple la méthode EIGENSTRAT consiste en une analyse en composante
principale (ACP) de la matrice des données génétiques cite:Price_2006 et
d'inclure ensuite les scores de l'analyse dans un modèle de regression linéaire.
L'objectif est alors de capter la structure de population qui est confondante
pour les études d'association. On ajoute ensuite la matrice des coordonnées sur
$K$ axes choisi au coté des variables explicatives du modèle de régression
\begin{equation}
\Y = f(\X \B^{T} + \bar{\U} \V ) + \E
\end{equation}
où $\bare{\U}$ est la matrice des coordonnées calculée par l'analyse en composante
principale. Plus récemment le même principe à été utilisé pour faire un étude
d'association entre des données de méthylation de l'adn et la maladie polyartrite
rumatoide cite:Rahmani_2016.
**** Les modèles mixes
Une autres approche de correction pour les facteurs de confusions est le modèle
mixe. Dans un tel modèle on ajoute un effet aléatoire à la régression
\begin{equation}
\Y = f(\X \B^{T} + \W) + \E
\end{equation}
où $\W$ est la matrice des effets aléatoires à estimer. Ce genre de modèle à été
largement utilisé dans les GWAS cite:Kang_2008,Zhou_2014. Dans les GWAS, l'effet
aléatoire permet d'expliquer la variabilité qui est du à la structure de
population. Classiquement, afin d'estimer la fois la matrice des effet $\B$ et
la matrice des effet aléatoire $\W$, on fait l'hypothèse que la covariance de
chaque colonnes des effets aléatoires est connue à priori. Pour les GWAS, cette
matrice de covariance est en général estimé à priori sur les données génétique.
**** Les modèles mixes à facteurs latents (LFMM, latent factor mixed model en anglais)
Nous introduisons maintenant les modèles mixes a facteurs latents. L'équation de
...
matrice des axes des facteurs latents. A la différence des modèles mixes, il
n'est pas nécessaire de faire supposer connue la covariance des matrices $\U$ et
$\V$. Alors pour les modèles mixe la matrice des effets aléatoires à pour but
d'expliquer une variabilité connu a priori, l'objectif de LFMM est d'apprendre
la variabilité systématique observé dans les variables expliquées grâce aux
paramètres $\U$ et $\V$. Il existe différentes méthodes pour estimer les
paramètres de LFMM. On distingue d'abord des approches qui reposent sur des
algorithmes de Monte Carlo
cite:frichot13_testin_assoc_between_loci_envir,carvalho08_high_dimen_spars_factor_model.
Ces approches repose sur une modélisation bayésienne de LFMM qui
d'échantillonner les lois a posteriori des paramètres. L'avantage de ces
méthodes est quelles permettent d'estimer la variance du paramètre $\B$. Cela
...
Ces approches sont plus rapide que les méthodes utilisant des algorithmes de
...
transformation qui a pour but de séparer la variabilité expliquée par les
variables latentes de celle expliquée par les variables explicatives. Parmi
cette dernière catégorie de méthodes, on distingue des autres les méthodes dites
à contrôles négatifs qui suppose connu un sous ensemble de variables expliquées
qui ne sont pas associées avec les variables explicatives. Ainsi, les méthodes a
contrôles négatifs utilisent donc les variables dites nulles pour estimer les
variables latentes.
...
nous simulation une variable explicative et une variable latente de sorte que
leur corrélation vaille $0.6$. Nous simulons ensuite une matrice de bruit
gaussien afin de calculer la matrice des variable expliqué grâce à l'équation
eqref:eq:glfmm du modèle mixe à facteur latent en considérant $f$ comme étant
l'identité. La matrice des effets $\B$ est simulé de sorte que $1 \%$ de ses
lignes soit non nulle. Cette simulation correspond a une situation où 1 \% des
variables expliquées sont associé avec les variables explicative et la variable
latente est facteur de confusion pour cette étude d'association. Afin de
détecter les variables expliquées associés à la variables explicative, nous
avons réalisé une régression linéaire avec seulement la variable explicative
$\X$ en covariable de régression. Nous effectuons une autre régression linéaire
avec cette fois la variable explicative $\X$ ainsi que la variable latente $\U$
comme covariable. Nous avons ensuite réalisé un test de Student pour tester la
nullité des coefficient associé à la variable explicative dans chacune des deux
régressions. La figure ref:fig:simu_intro montre que quand on ne prend pas en
compte la variable latente plus de 40 \% des \pvalues sont proche de zéro, on
détecte alors beaucoup de candidat pour l'association avec la variable
explicative. Alors que quand on prend en compte les facteurs latent la
distribution des \pvalues est bien uniforme. On s'attend bien a une distribution
uniforme car la majorité variable expliquées ne sont pas associé avec la
variable explicative. Dans le cas de cette simulation il est impossible de ne
pas prendre en compte la variable latente, sans celle-ci on détecte presque la
moitié des variables expliquée comme étant associé à la variable explicative.
...
expliqué par les variables explicatives. Elles sont donc directement comparable
à SVA cite:article_Leek_Storey_2007 et CATE cite:wang2015confounder qui procède
d'une façon très similaire, nous décrivons plus en détail ces méthodes dans la
partie [[sec:similar_method]]. Chacun des algorithmes que nous présentons découle de
l'optimisation d'une fonction objectif. Nous montrons montrons que nos
algorithmes d'estimation convergent vers le point de minimum global de leur
fonction objectif respective. Enfin, nous comparons nos méthodes à SVA et CATE
sur des simulations numériques ainsi que des exemples de GWAS, EWAS et GEA.
...
régularisation) est indispensable pour séparer la variabilité expliqué par les
...
**** sva
**** famt
*** Simulation et vrai jeux de données                           :noexport:
...
**** DEBUG Comparaisons numériques sur des simulations
:LOGBOOK:
