...
#+begin_src R :results output latex :exports results :session *R* :dir ~/Projects/Thesis/MaThese/
  ...
  table.df %>%
    ...
    caption = "{\\bf 20 premiers groupes de SNPs associés avec la maladie \\coeliac retrounés par l'algorithme lassoLFMM.} Les lignes en gras sont les SNPs qui sont référencés dans la base de données GWAS catalog.") %>%
    print(include.rownames=FALSE,
