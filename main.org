...
10^{-5}$ pour ridgeLFMM. Nous avons choisi une proportion de lignes non nulles
pour de $\B$ valant $1 \%$ pour lassoLFMM.
...
\caption{Choix des paramètres pour l'étude d'association entre des sites
  ...
  prédiction moyen et les bar l'erreur standard. La ligne pointillée vertical
  marque le nombre de variables latentes choisies, c'est à dire 10.}
...
identifiés par cite:Zou_2014,Rahmani_2016 qui sont retrouvé dans les top listes
de chaque méthode. Nous rappelons que les candidats identifiés par
cite:Zou_2014,Rahmani_2016 ont été identifié en prenant en compte les facteurs
latents tel que l'age le sex et des méthodes comparables à l'ACP sur la matrice
des niveaux de méthylation pour identifier la composition cellulaire. Toutes les
méthodes retrouvent ces candidats dans leurs liste top 40 sauf lm et sva_irw. Pour la
...
méthode sva-irw ses candidats son tous identifié entre le top 5111 et 87659.
...
le taux de fausse découverte (FDR) à $1 \%$. Toutefois les algorithmes de contrôle
du FDR nécessitent que les \pvalues soient correctement calibrées. Pour cela
nous avons calibrées les \pvalues grâce à la méthode présenté dans la partie
[[sec:calibration]]. Le contrôle du FDR a été fait à l'aide du package R qvalue
...
les méthodes on retrouve les 5 candidats identifiés dans cite:Zou_2014 et
cite:Rahmani_2016.
...
#+begin_src R  
  ...
  ## projection
  P.list <- MatrixFactorizationR::compute_P(dat$X, lambda = 0.0)
...
#+begin_src R 
  ...
  toplot <- expr$errs %>%
    ...
    summarise(err.mean = mean(err), N = length(err), sd = sd(err), se = sd / sqrt(N)) %>%
    ungroup()
  plC <- ggplot(toplot, aes(x = log10(lambda), y = err.mean, color = K)) +
    geom_point() + 
    geom_errorbar(aes(ymin = err.mean - se,
                      ymax = err.mean + se,
                      width = 0.5)) +
...
connu que les SNPs sont corrélés le long du génome, nous avons donc filtrer les
SNPs trop corrélé entre eux, cette étape est identifié dans la littérature comme
le LD pruning. Pour cela, pour chaque SNP nous ne gardons que le SNP de variance
maximum sur une fenêtre de 100 SNP et cela sur les SNP ayant un coefficient de
corélation linéaire au carré qui est supérieure à $0.2$, cela nous a permis
d'identifier un sous ensemble de 80275 SNPs. Ces opérations de filtrage ont été
faites à l'aide du logiciel plink cite:Purcell_2007. Enfin nous avons utilisé
beagle pour imputer les données manquantes de la matrice de SNPs
...
ridgeLFMM et lassoLFMM sur le sous ensemble de SNPs identifié par l'étape de LD
pruning. Par la suite, nous avons effectué le test d'hypothèse présenté dans la
partie [[sec:hypothese]] sur le jeux de données génétiques complet mais avec
l'estimation des variables latentes calculée sur le sous ensemble de SNPs. Nous
avons choisi 9 variables latentes (voir Figure ref:fig:gwas_params A et B). 

La validation croisée du paramètre $\lambRidge$ pousse à choisir une valeur
grande ($10^{10}) ne permet pas de conclure (Figure ref:fig:gwas_params C), nous
...
Enfin comme on s'attend à detecter un
petit nombre d'association nous avons choisi une proportion de ligne non nulle
pour l'estimateur $L_{1}$ de $\B$ valant 1\%.

#+BEGIN_EXPORT latex
\begin{figure}[!t]
\centering
\includegraphics{./OUTPUT/Rplots/gwas_hyperparams.pdf.png}
\caption{Choix des paramètres pour l'tude d'association entre génotype et la
  maladie \celiac. A) Proportion de variance expliquées de la projection de $\Y$
  sur l'espace orthogonal à $\X$ ($\matr{D}_{0} \Q^{T} \Y$) par chacune des
  composantes principales B)C) Erreur de prédiction calculée grâce à la
  validation croisée des estimateurs $L_{2}$ des paramètres de LFMM pour
  différente valeurs du paramètre de régularisation $\lambda$ et du nombre
  variable latentes $\K$, le point représente l'erreur de prédiction moyen et
  les bar l'erreur standard. La ligne pointillée vertical marque le nombre de
  variables latentes choisies, c'est à dire 9.}
\label{fig:gwas_params}
\end{figure}
#+END_EXPORT

La figure ref:fig:gwas_qqplot_top A montre la distribution observée des \pvalues
renvoyé par chaque méthode contre la distribution théorique sous l'hypothèse
...
Figure ref:fig:gwas_qqplot_top B montre le nombre de SNPs identifiés dans le
GWAS catalog comme étant associés avec la maladie \celiac qui sont retrouvés
dans les top listes des différente méthodes. On observe que lm retrouve moins
facilement dans sa top liste les candidats du GWAS catalogue, $78\%$ des
candidats du GWAS catalogue sont dans le top 3000 de lm contre $83\%$ pour PCAlm
et $88\%$ pour cate, ridgeLFMM et lassoLFMM.
...
le FDR à 1\%, nous avons procédé de la même façon que pour
l'EWAS pour calculer cette liste (voir section [[sec:ewas]]). La Figure
ref:fig:gwas_venn montre l'intersection des listes de candidats contrôlés à un
...
liste avec 1319 candidats. Nous constatons que l'intersection des listes à FDR
...
#+begin_src R
  ...
  run_celiac <- function(m) {
    ...
    if (!exist_res(m, out.file.res.df)) {
      ## mask data
      message("mask data")
      dat.masked <- MatrixFactorizationR::LfmmDat(Y = NULL, X = dat$X, missing = FALSE)
      ...
      X <- cbind(dat$X, m.U$U)
      d <- ncol(dat$X)
      hp <- MatrixFactorizationR::hypothesis_testing_lm(dat, X)
...
#+begin_src R 
  ...
  ## cross validation lambda
  expr <- readRDS("./OUTPUT/Expr/gwas_CV_lambda_lfmm_round2.rds")
  toplot <- expr$errs %>%
    mutate(K = as.factor(K)) %>%
    group_by(lambda, K) %>%
    summarise(err.mean = mean(err), N = length(err), sd = sd(err), se = sd / sqrt(N)) %>%
...
: Error in readRDS("./OUTPUT/Expr/gwas_screeplot_expr.rds") : 
:   error reading from connection
: [[./OUTPUT/Rplots/gwas_screeplot.png]]
: [[./OUTPUT/Rplots/gwas_CV_K.png]]
 [[./OUTPUT/Rplots/gwas_CV_lambda.png]]
: [[./OUTPUT/Rplots/gwas_hyperparams.pdf.png]]
...
habitat on plus de chance de survivre et de se reproduire. Si l'avantage
adaptatif à des causes génétiques, c'est à dire qu'il existe une combinaison de
gènes qui confère à l'organisme une fonction lui permet d'être mieux adapté à
son habitat, alors le patrimoine génétique qui confère l'avantage adaptatif sera
transmis aux futures générations. On peut donc alors chercher à détecter les
signatures génétiques laissé par l'adaptation à un environnement. C'est
...
données regroupes $84.4$ million de variants génétiques pour 2506 individus
...
lassoLFMM lancé sur d'association entre les données du 1000Genome et un
...
environnemental, il faut garder les individus qui vivent dans leur habitat
depuis plusieurs générations, nous avons donc enlever les populations
...
sous ensemble de 296948 grâce au LD pruning des 5397214 locus. Nous avons
...
\pvaleurs pour chacun des 5397214 locus grâce au test d'hypothèse présenté dans
la partie [[sec:hypothese]].

Afin de calculer le gradient environnemental utilisé pour l'étude d'assocaition
nous avons attribué une position géographique à chaque individu en prenant la
capital de leur pays d'origine. Cela nous a parmi de récupérer les données de la
base WorlClim pour les position géographique ainsi calculées. Nous ne gardons
que la première composante principale des variables de la base Wordclim.
La Figure ref:fig:eas_gradient représente la valeur du gradient climatique pour
chaque population. On remarque que celui-ci est très corrélé avec la latitude.
Comme les populations sont à différentes latitudes il y a de la corrélation entre
la structure de population et le gradient environnemental.
...
variables latentes. Ce choix à été motivé par la décroissance des valeurs propre
représenté par la Figure ref:fig:eas_params A. Les erreurs de prédiction
calculées à partir de la validation croisée donne des résultat compliqué a
interpréter (Figure ref:fig:eas_params B et C). Nous avons choisi $\lambdaRidge
= 10^{-5}$ pour ridgeLFMM et une proportion de lignes non nulles de $\B$
valant $1 \%$ pour lassoLFMM.
...
\caption{Choix des paramètres pour l'étude d'association entre un génotype et un
  gradient environmental. A) Proportion de variance expliquées de la projection
  de $\Y$ sur l'espace orthogonal à $\X$ ($\matr{D}_{0} \Q^{T} \Y$) par chacune
  des composantes principales B)C) Erreur de prédiction calculée grâce à la
  validation croisée des estimateurs $L_{2}$ des paramètres de LFMM pour
  différente valeurs du paramètre de régularisation $\lambda$ et du nombre
  variable latentes $\K$, le point représente l'erreur de prédiction moyen et
  les bars l'erreur standard. La ligne pointillée verticale marque le nombre de
  variables latentes choisies, c'est à dire 5.}
...
#+begin_src shell
  ...
  vep --cache -i vep.input.txt -o vep.output.txt --force --variant_class --sift b
#+end_src
...
#+begin_example
  #Uploaded_variation   Location            Allele              Gene          
   Length:18311186     Length:18311186    Length:18311186    Length:18311186   
   ...
   Length:18311186    Length:18311186    Length:18311186    Min.   :     1    
   Class :character   Class :character   Class :character   1st Qu.:   390    
   Mode  :character   Mode  :character   Mode  :character   Median :  1009    
                                                            Mean   :  1977    
                                                            3rd Qu.:  2299    
                                                            Max.   :204149    
                                                            NA's   :17780202  
    CDS_position      Protein_position   Amino_acids           Codons         
   Min.   :     1     Min.   :    1      Length:18311186    Length:18311186   
   1st Qu.:   301     1st Qu.:  101      Class :character   Class :character  
   Median :   768     Median :  256      Mode  :character   Mode  :character  
   Mean   :  1632     Mean   :  544                                           
   3rd Qu.:  1655     3rd Qu.:  552                                           
   Max.   :106324     Max.   :35442                                           
   NA's   :18123626   NA's   :18123626                                        
   Existing_variation    Extra                IMPACT           SIFT.score      
   Mode:logical       Length:18311186    HIGH    :   17099   Min.   :0         
   NA's:18311186      Class :character   LOW     :   78582   1st Qu.:0         
                      Mode  :character   MODERATE:  123794   Median :0         
                                         MODIFIER:18091711   Mean   :0         
                                                             3rd Qu.:0         
                                                             Max.   :1         
                                                             NA's   :18234190  
      SIFT.prediction         snps          
   deleterious:   44309   Length:18311186   
   tolerated  :   32687   Class :character  
   NA's       :18234190   Mode  :character  
#+end_example
...
****** TODO Comparaison des listes avec contrôle du FDR à $0.01$ and venn diag
:LOGBOOK:
- State "TODO"       from              [2017-08-17 jeu. 09:31]
:END:
...
#+begin_src R 
  ...
  ## 2 area
  paste0("2:",
         min(inter.candidates.df.vep$pos[inter.candidates.df.vep$chrm == "2"]),
         "-",
         max(inter.candidates.df.vep$pos[inter.candidates.df.vep$chrm == "2"]))
  paste0("4:",
         min(inter.candidates.df.vep$pos[inter.candidates.df.vep$chrm == "4"]),
         "-",
         max(inter.candidates.df.vep$pos[inter.candidates.df.vep$chrm == "4"]))
#+end_src
****** TODO manhattan plot
:LOGBOOK:
...
** Discussion                                                     :noexport:
Les études d'association sont largement utilisé en biologie trouver des lien de
corrélation des variables. Cependant une corrélation n'est pas une lien de
causalité. Nous nous somme intressé dans ce chapitre à la situation ou une
variable non observé induire des associations quand on ne les considère pas
explicitement dans l'étude. Dans une tel situation on observe que une forte
inflations les \pvalues renvoyé par la regression linéaire de $\Y$ par $\X$ dans
les simulation numérique (Figure ref:fig:method_comp C et D) ainsi que dans l'
EWAS (Figure ref:fig:ewas_qqplot_top A), la GWAS (Figure ref:fig:gwas_qqplot_top
A) et la GEAS (Figure ref:fig:geas_qqplot A) que nous avons présentés dans ce
chapitre. La forte inflation des \pvalues renvoyées par lm ne correspond pas
seulement à une mauvaise calibration de celles-ci car dans les simulations que
l'AUC obtenu par lm est largement inferieur à celui de la méthode oracle (Figure
ref:fig:method_comp A et B) ainsi pour les études d'associations à partir de
données lm donne les moins bon résultat quand il s'agit de retrouver des
candidats validé par d'autre méthode (Figure ref:fig:ewas_qqplot_top B et Figure
ref:fig:gwas_qqplot_top B). 

Une méthode de correction pour les facteurs de confusion consite à consite à
utiliser les scores de l'ACP pour apprendre les facteurs de confusions; nous
l'avons nommé dans ce chapitre PCAlm. Sur les simulations numériques PCAlm
renvoie des \pvalues bien calibré (Figure ref:fig:method_comp C et D) mais l'AUC
de PCAlm décroit avec le nombre de variables associées et la corrélation entre
les variables latentes et la variable explicative (Figure ref:fig:method_comp A
et B). On retrouve le fait que les \pvalues sont bien calibré dans les étude sur
des vrai de données (Figure ref:fig:ewas_qqplot_top A, Figure
ref:fig:gwas_qqplot_top A et ref:fig:geas_qqplot). Les candidats trouvé par
d'autr méthodes sont bien retrouvé par PCAlm (Figure ref:fig:ewas_qqplot_top B
et Figure ref:fig:gwas_qqplot_top B). On peut supposer que PCAlm donne à de
bonne performance sur la GWAS et l'EWAS car il y à peu de variables à détecter.
De plus certain candidats trouvés dans d'autre étude ont trouvé avec
des méthodes similaires à PCAlm cite:Rahmani_2016,Price_2006.

Les deux méthodes SVA comparé dans ce chapitre donne des résultats similaires à
PCAlm sur les simualtion numérique (Figure ref:fig:method_comp). La méthode
sva-two-step donne des résultat très similaire à PCAlm sur l'EWAS alors que
sva-two-step donne des résultats très atypique comparé aux autre méthode
considéré pour l'EWAS. Avec les bons hyper paramètres, il est possible que sva-irw
donnerait des résultats comparable autres méthodes. Cependant il n'y a aucune
garentit de la convergence de sva-irw ce qui rend compliqué l'exploration pour
trouver les bons hyper paramètres. 

Nous proposons dans ce chapitre deux méthodes, ridgeLFMM et lassoLFMM, de
correction des étude d'association pour les facteurs latents qui repose sur
l'optimisation de fonctions objectifs. Sur les jeux de données simulées ces
méthodes ont les mêmes performances que la méthode oracle (Figure
ref:fig:method_comp), sont bien calibré sur les vrais jeux de données (Figure
ref:fig:ewas_qqplot_top A, Figure ref:fig:gwas_qqplot_top A et
ref:fig:geas_qqplot) et retrouve les candidats validé par la litératures (Figure
ref:fig:ewas_qqplot_top B et ref:fig:gwas_qqplot_top B). Les performances
globales de ces méthodes sont très comparable à la méthode cate. Bien que cate,
ridgeLFMM et lassoLFMM ont des performances très similaire pour nos critère il
existe tout de même des différence entre les liste de candidat renvoyé pas ces
trois méthodes (Figure ref:fig:ewas_venn, Figure ref:fig:gwas_venn et Figure
ref:fig:geas_venn). Dans les études d'association considéré ici il n'y a de
vérité terrain et une méthodes peut toujours donner de meilleurs résultat qu'un
autre sur des simulations bien choisis (citer no free lunch th). Ceux deux nouvelles méthodes s'ajoutes a
l'arsenal es méthodes permettant de corriger pour les facteurs de confusion. Il
serait interessant de mettre en évidence des situation mettant en défaut une
méthodes par rapport a une autre. Il existe aussi des méthodes permmettant de
combiner les résultats vennant de plusieurs méthodes, cela permet de d'augmanter
la confiance que l'on a en les résultats d'une étude d'association (citer meta
analysis).
