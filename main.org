...
aléatoire permet d'expliquer la variance qui est du à la structure de
...
utilisé les implémentation R de ces méthodes mis à disposition par leur auteur.
...
l'on souhaites simuler. Cette étape permet d'obtenir la matrice des axes principaux
orthonormé, que l'on note $\bar{\V}$, ainsi que la matrice des coordonnées sur
ces axes, que l'on note $\bar{U}$. Nous simulons ensuite 
