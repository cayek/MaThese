...
la capacité des méthodes à bien corriger pour les facteurs de confusion, nous
...
l'ADN associés à la polyarthrite. Nous avons choisi $\K = 10$ pour le nombre de
variable latentes (voir Figure ref:fig:ewas_params A et B). Pour $\K = 10$, les
valeurs du paramètre de régularisation $L_{2}$ $\lambRidge$ entre $10^{-10}$ et
$1$ donnent les mêmes valeurs d'erreur de prédiction moyen (Figure
ref:fig:ewas_params C), nous avons choisi de prendre $\lambRidge = 10^{-5}$.
Nous avons choisi une proportion de ligne non nulle pour l'estimateur $L_{1}$ de
$\B$ valant 1\%.
...
corélation liénaire au carré est supérieure à $0.2$, cela nous a permis
d'identifier un sous ensemble de 80275 SNPs. Ces opérations de titrage on été
...
l'estimation des variables latentes calculé sur le sous ensemble de SNPs. Nous
avons choisi 9 variables latentes (voir Figure ref:fig:gwas_params A et B). 

La
validation croisée du paramètre $\lambRidge$ pousse a choisir une valeur grande
($10^{10}) ne permet pas de conclure (Figure
ref:fig:gwas_params C), nous avons choisi $1e^{-5}$ car nous avons remarqué dans
nos expérimentation que les petites valeurs du paramètres de régularisation
$L_{2}$ donne les meilleur résultats. 
...
latitute il y a de la corrélation entre la structure de population et le
...
= 10^{-5}$ pour l'estimateur $L_{2}$ et une proportion de ligne non nulle de
pour de $\B$ valant 1\% l'estimateur $L_{1}$.
...
#+begin_src shell
  ...
  vep --cache -i vep.input.txt -o vep.output.txt --force
#+end_src
...
#+begin_src R 
  ...
  vep.output <- vep.output %>%
    mutate(Impact = sub("IMPACT=", "",sub(";.*", "", Extra)))

  pl <- ggplot(vep.output, aes(x = Consequence)) +
    geom_bar()
  pl

  mean(is.na(vep.output$Feature_type))
