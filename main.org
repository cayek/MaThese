...
\celiac. De plus nous s'avons que la stratification des individus en sous
population peut être un facteur de confusion dans les GWAS. Il est habituelle de
corriger les GWAS en utilisant les score de l'ACP des données génétiques pour
...
cite:Browning_2016. Finalement, nous obtenons finalement un jeux de données
composée 281122 SNPs et 15155 individus. Afin d'estimer les variables latentes
pour corriger le test d'association entre les SNPS et la maladie \celiac nous
avons lancer les méthode lm, lmPCA, cate, ridgeLFMM et lassoLFMM sur le sous
ensemble de SNPs identifé par l'étape de LD pruning. Par la suite, nous avons
effectuer le test d'hypothèse présenté dans la partie [[sec:hypothese]] sur le jeux
de données génétique complet mais avec l'estimation des variables latentes
caluclé sur le sous ensemble de SPNs.
...
***** RUNNING Imputation des données manquantes
:LOGBOOK:
...
#+CAPTION: Dépend de [[gwas_CV_encore]]
#+begin_src R 
  library(MaTheseR)
...
***** TODO Validation croisée du modèle lfmmRidge
:LOGBOOK:
- State "TODO"       from "DONE"       [2017-08-17 jeu. 11:47]
...
#+begin_src R
  ...
  methods$m.lm <- method_lm(col.mask = col.mask,
                            ...
                            inter.res.file = "./OUTPUT/Expr/eas_inter_lm.rds")
  methods$m.ridgeLfmm <- method_ridgeLFMM(K = K.method,
                                  col.mask = col.mask,
                                  inter.res.saving.file = "./OUTPUT/Expr/eas_inter_ridgelfmm.rds",
                                  inter.res.file = "./OUTPUT/Expr/eas_inter_ridgelfmm.rds")
  methods$m.pca <- method_PCAlm(K = K.method,
                                col.mask = col.mask,
                                inter.res.saving.file = "./OUTPUT/Expr/eas_inter_PCAlm.rds",
                                inter.res.file = "./OUTPUT/Expr/eas_inter_PCAlm.rds"
                                )
  methods$m.cate <- method_cate(K = K.method,
                        col.mask = col.mask,
                        inter.res.saving.file = "./OUTPUT/Expr/eas_inter_cate.rds",
                        inter.res.file = "./OUTPUT/Expr/eas_inter_cate.rds",
                        hp = "lm"
                        )
  methods$m.lasso <- method_lassoLFMM(K = K.method, nozero.prop = nozero.prop,
                              lambda.K = 25, relative.err.epsilon = 1e-6,
                              ...
                              inter.res.file = "./OUTPUT/Expr/eas_inter_lassolfmm.rds"
                              )
