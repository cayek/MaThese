...
coefficients de métissage, une classe importante de méthodes a amélioré le
modèle bayésien de =structure= en incorporant des données géographiques à
travers la loi de distribution à priori des coefficient de métissage
...
Certaines méthodes bayésiennes sont basées sur des algorithmes
de Monte Carlo de chaîne de Markov qui nécessite beaucoup de calcul ~ \ citep
{Francois2010}. 

Les efforts récents visant à améliorer l'inférence des
coefficients ancestrales dans un contexte géographique ont principalement porté
sur la localisation des ancêtres récents cite:Baran_2013


Dans ces applications, l'information
spatiale est utilisée dans un cadre prédictif qui attribue aux ancêtres des
origines géographiques supposé.
