...
\end{equation*}
alors par le calcule on peut montrer que
\begin{equation*}
...
La matrice $\obP$ défie une transformation linéaire qui pour une variable
aléatoire de moyenne nulle réduit sa corrélation linéaire avec $\X$. Le
paramètre $\lambda$ permet de régulé la corrélation restante. Si $\lambda$ vaut
...
#+BEGIN_theorem
<<ridge_theorem>> 
Pour $\lambda$ strictement supérieur à zéro, les estimateurs
des moindres carrés régularisés en norme $L_{2}$, définis par
eqref:eq:RidgeLfmmEstomatorW et eqref:eq:RidgeLfmmEstomatorB, définissent un
point de minimum local de la fonction objective $\Lridge$
...
\end{align*}
qui minimise la fonction $\Lridge$.  Les dérivés partielles donnent
\begin{equation}
\begin{cases}
...
\end{cases}
\end{equation}
Un minimum local de $\Lridge$ vérifie nécessairement l'équation d'Euler, ce qui
revient a annuler les dérivé partielles. On a alors
\begin{equation}
\begin{cases}
...
\end{equation}
On tombe sur le problème de trouver la meilleur approximation de rang $\K$ qui
est données par la SVD de la matrice
$$ \sqrt{\obP} \Y.$$
Réciproquement, la solution
\begin{align*}
\hat{\U} \hat{\V} & =  \sqrt{\obP}^{-1} * \mathrm{svd}_{\K}(\sqrt{\obP} \Y ) \\
\hat{\B} & = (\X^{T} \X + \lambda \Id_{d})^{-1} \X^{T} (G - \hat{\U} \hat{\V})
\end{align*}
annule bien l'équation d'Euler. De plus, 
#+END_proof
...
***** TODO Comparaisons numériques sur des simulations
:LOGBOOK:
...
#+begin_src R :results output :exports both
  ...
  m.lasso <- method_lassoLFMM(K = K.method, nozero.prop = 0.15, lambda.K = 25, relative.err.epsilon = 1e-4)
  m.lm <- method_lm()
