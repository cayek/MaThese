...
définies de façon unique. Nous posons $$ \W = \U \V^{T} $$ la matrice latente.
Si l'on suppose qu'il y à $\K$ variables latentes sont indépendantes alors la
matrice latentes $\W$ est de rang $\K$. Dans la suite nous considérerons $\U$ et
$\V$ comme étant les matrices uniques obtenues grâce à l'analyse en composante
principale de la matrice latente $\W$.

**** Estimateurs des moindres carrées régularisés en norme $L_{2}$
:LOGBOOK:
...
modèle lfmm eqref:eq:model basé sur un problème des moindres carrées régularisé
en norme $L_{2}$. Nous montrerons que cette algorithme conspont a un point de
minimum local du problème d'optimisation.
...
\LfmmLridge
\end{equation}
où $\norm{.}_{F}$ est la norme de Frobenius, $\norm{.}_{2}$ est la norme $L_2$ et
$\lamba$ le paramètre de régularisation. Le terme 
$$
\frac{1}{2} \norm{\Y - \U \V^{T} - \X \B^{T}} 
$$
correspond à l'opposé de la log vraisemblance où la matrice de bruit $\E$ est
Gaussienne, isotrope et de moyenne nulle. 

Nous remarquons que le terme de régularisation
\begin{equation*}
\lambda \norm{\B}^{2}_{2}
\end{equation*}
est indispensable pour distinguer l'effet des variables lattentes de celui des
variables explicatives. En effet, si
\begin{equation*}
\lambda = 0 
\end{equation*}
...
\end{equation*}
Ainsi, les points minimuns d'une tel fonction objective ne sont pas pertinent pour
notre problème.

Les estimateurs sont calculé de la façon suivante 
\begin{align}
...
$\matr{A}$ données par la décomposition en valeurs singulières (SVD pour singular
value decomposition) et $\Id_{d}$ est la matrice identité de taille $d \times
d$. La matrice $\obP$ est définie tel que 
\begin{equation*}
\LfmmP. 
\end{equation*} 
Si l'on calcul la SVD de la matrice $\X$ tel que 
\begin{equation*}
\X = \matr{Q} \matr{\Sigma} \matr{R}^{T}
\end{equation*}
alors par le calcul on peut montrer que
\begin{equation*}
\obP = \matr{Q} \matr{D} \matr{Q}^{T}
\end{equation*}
où $\matr{D}$ est une matrice diagonale composée des termes
\begin{equation*}
\left\{ \frac{\lambda}{\lambda + \sigma_{1}^{2}}, ..., 
\frac{\lambda}{\lambda + \sigma_{d}^{2}}, 
1, ..., 1 \right\},
\end{equation*}
où $\sigma_{i = 1..d}$ sont les valeurs singulières de $\X$.
Enfin, la matrice racine carré de $\oqP$ est l'unique matrice tel que
\begin{equation*}
\obP = \sqrt{\obP} \sqrt{\obP},
\end{equation*}
calculé en prenant la racine carré des éléments de la matrice diagonale $\matr{D}$.

La matrice $\obP$ définie une transformation linéaire qui pour une variable
aléatoire de moyenne nulle réduit sa corrélation linéaire avec $\X$. Le
paramètre $\lambda$ permet de réguler la corrélation restante. Si $\lambda$ vaut
zéro multiplier par $\obP$ revient à prendre le résidu d'une régression linéaire
par $\X$, on enlève alors toute la corrélation linéaire avec $\X$. Si $\lambda$
est très grand alors $\obP$ tend vers la matrice identité. Dans ce cas
l'estimations des variable latente par l'équation eqref:eq:RidgeLfmmEstomatorW
revient à faire une analyse en composante principale de la matrice $\Y$. Il est
donc important de choisir une valeur de $\lambda$ qui enlève la bonne proportion
de corrélation avec la variable explicative $\X$. Nous expliquons dans la partie
[[sec:hyperparametre]] plus en détaille comment choisir les hyperparamètres.

Le calcul des estimateurs est justifié par le théorème suivant 
#+BEGIN_theorem
<<ridge_theorem>> 
Pour $\lambda$ strictement supérieur à zéro, l'estimateurs
des moindres carrés régularisés en norme $L_{2}$, défini par
eqref:eq:RidgeLfmmEstomatorW et eqref:eq:RidgeLfmmEstomatorB, défini un
point de minimum global de la fonction objective $\Lridge$.
eqref:eq:optim_ridge_reg.
#+END_theorem
...
\end{align*}
qui soit un minimum global de la fonction $\Lridge$. Commençons par remarquer
que la fonction $\Lridge$ est convexe le long de la variable $\B$ , on peut
donc écrire le point de minimum global qui en annulant la dérivé de $\Lridge$
par rapport à $\B$
\begin{equation}
...
singulières 
\begin{equation*}
\left \{ \sigma_{j} \left \}_{j = 1..\Xcol}.
\end{equation*}
Ainsi l'écriture de $\mathcal{L}^{'}$ se simplifie comme ceci
\begin{equation*}
...
& \hat{\B}^{T} = (\X^{T} \X + \lambda \Id_{\Bcol})^{-1} \X^{T} (\Y - \hat{\U} \hat{\V}) \\
& 0 = \hat{\U}^{T} (\hat{\U} \hat{\V}^{T} + \X \hat{\B}^{T} - \Y) \\
& 0 = (\hat{\U} ^\hat{\V}^{T} + \X \hat{\B}^{T} - \Y) \hat{\V}
\end{cases}
\end{equation}
En injectant la première ligne de eqref:eq:euler_Lridge dans les deux
autres on a
\begin{equation}
...
\end{cases}
\end{equation}
On peut trouver des solutions $\hat{\U}$ $\hat{\V}$ de eqref:eq:euler_UV_Lridge
en trouvant un point minimum de 
\begin{equation}
...
\sqrt{\obP} \Y,
\end{equation*}
qui est obtenue en tronquant la SVD pour ne garder que $\K$
...
\end{align*}
est bien un point critique de $\Lridge$.
#+END_proof
...
Dans cette partie nous proposons de remplacer la régularisation en norme $L_{2}$
de la fonction objectif $\Lridge$ eqref:eq:optim_ridge_reg par une
régularisation $L_{1}$. Le chois de cette régularisation est motivé par
intuitivement par le fait que l'on s'attend à ce que seulement une certaine
proportion des variable expliquées $\Y_{j}$ soit associé à la variable
explicative $\X$. Cela signifie qu'une partie des lignes de la matrice des
effets $\B$ est nulle. Or la régularisation $\L_{1}$ est connue pour produire des
matrices des effets creuses cite:Tibshirani_1996.

Nous écrivons la fonction objective de la façon suivante 
\begin{equation}
\label{eq:optim_lasso_reg}
\LfmmLlasso
\end{equation}
où $\norm{.}_{*}$ est la norme nucléaire. La norme nucléaire est introduite ici
car elle permet de définir la fonction objective $\Llasso$ convexe. Nous verons
par la suite que cela permet d'utilisé des algorithme rapide. De plus, cela
permet de définir la matrice latentes et la matrice des effets de façon unique.
En effet les 

il faut demontrer la strict convexité !!! voir
cite:cai10_singul_value_thres_algor_matrix_compl 


Where  is the nuclear norm. Contrary to $\Lridge$ finding minimum
...
#+begin_src R :results output :exports both
  ...
  ## save 
  saveRDS(indiv.df, "./Data/ThesisDataset/3Article/1000GenomesPhase3/G_EAS_indiv.rds")
  saveRDS(as.matrix(indiv.df$X), "./Data/ThesisDataset/3Article/1000GenomesPhase3/X_EAS.rds")
...
#+begin_src R :results output :exports both
  ...
  samplers <-  s * param(prop.outlier = c(0.01, 0.05, 0.1, 0.2), rho.c = c(0.1, 0.3, 0.5, 1.0))

  ## methods
  m.ridgeLfmm <- method_ridgeLFMM(K = K.method)
  m.lasso <- method_lassoLFMM(K = K.method, nozero.prop = 0.20, lambda.K = 25, relative.err.epsilon = 1e-4)
  m.lm <- method_lm()
...
#+begin_src R :results output :exports both
  ...
  ## methods
  methods <- method_PCA(K = 300, scale = FALSE) * param()
...
#+RESULTS:
[[./OUTPUT/Rplots/file3700288edbce.png]]
#+begin_example
> save_expr(expr)
Expr save in ./OUTPUT/Expr/file37004c9591a3.rds
#+end_example
****** RUNNING Validation croisée du modèle lfmmRidge sur 1000genome
:LOGBOOK:
...
****** TODO Etude du jeu de données avec cate lfmm lm et lmPCA
:LOGBOOK:
