...
***** TODO fusion des indiv dans un bed
:LOGBOOK:
...
#+begin_src shell
  ...
  ls *.bed > all.beds.txt
  sed -i 's/\.bed//g' all.beds.txt 
  plink --merge-list all.beds.txt --make-bed --biallelic-only --out Simons
  ...
  do
      ...
      plink --bfile $basefile --exclude Simons- --make-bed --out $basefile
  done
#+end_src
...
#+begin_src shell
  ...
  do
      echo "===== $file ====="
      plink --vcf $file --autosome --make-bed --out `basename $file .vcf.gz`
  done
#+end_src
** Résultats
:PROPERTIES:
...
catalog. Parmis les 20 premiers goupe de SNPs détecté par la méthode lassLFMM
...
J'annote le chromosome 6

#+NAME: code:gwas_lasso_annot_6
#+CAPTION: Dépend de [[code:gwas_expr]] [[code:gwas_clumping]]
#+begin_src R 
