...
La méthode cate proposé dans cite:wang2015confounder est comparable notre méthode
ridgeLFMM que nous avons présenté précédemment. Plutot que d'estimer les
variables latentes 

Cate procède d'abors
...
#+begin_src R 
  library(MaTheseR)
  dat.files <- readRDS("./OUTPUT/Dat/ValNum/dat_list.rds")
  ...
  df.res <-
    ...
    foreach(f = dats, .combine = 'rbind') %dopar%
    {
      dat <- readRDS(f)
      ## on force
      if (m$force) {
      ...
      } else if (exist_res(m, dat, f))
        m <- retrieve_res(m, dat, f)
    } else {
      m <- ExpRmouline(m, dat)
      ## save
      if (m$save) {
      ...
      ExpRextractor_fdr(dat, m, rep.sampler = dat$meta$i, rep.method = 1)
  }
  doParallel::stopImplicitCluster()
  parallel::stopCluster(cl)
