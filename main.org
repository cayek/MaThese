...
l'estimation des variables latentes calculées sur le sous ensemble de SNPs. Nous
avons choisi 9 variables latentes (voir Figure ref:fig:gwas_params A et B). Nous
avons choisi une proportion de lignes non nulles pour de $\B$ valant $1 \%$ pour
lassoLFMM. La validation croisée du paramètre $\lambRidge$ tend à faire choisir
une valeur pour celui-ci (Figure ref:fig:gwas_params C), nous avons choisi
$1e^{3}$. 
...
Figure ref:fig:gwas_qqplot_top B montre le nombre de SNPs référencé dans le
GWAS catalog comme étant associés avec la maladie \celiac qui sont retrouvés
dans les top listes des différente méthodes. 

On observe que lm retrouve moins
...
#+begin_src R 
  ...
  impact.all <- vep.output %>%
    ...
    dplyr::select(IMPACT)
  impact.all$IMPACT %>% table() / nrow(impact.all)
  ...
  length(union.candidates.df$colname)
  ...
  union.candidates.df.vep$method %>% unique()
  union.candidates.df.vep$IMPACT %>% table()
  impact.union <- union.candidates.df.vep %>%
    distinct(snsp) %>%
    dplyr::select(IMPACT)
  impact.union$IMPACT %>% table() / nrow(impact.union)

  union.candidates.df.vep %>% dplyr::filter(IMPACT == "LOW") %>%
    dplyr::select(IMPACT, Consequence, method, snps) %>%
    distinct(snps, method, keep_all = TRUE) %>%
      print.data.frame()

  ## plot
  pl <- ggplot(union.candidates.df.vep, aes(x = pos, y = -log10(pvalue))) +
    geom_point() +
    facet_grid(~ chrm, scales = "free")
  save_plot_png(pl, "eas_union_manhattan.png")
  ...
  ## amiGo2
  panter.input <- (union.candidates.df.vep %>%
    distinct(Gene))$Gene %>%
    paste0(collapse=" ")
#+end_src
...
situation ou des variables non observées provoquent des fausses associations si
on ne les considère pas explicitement dans l'étude. Dans une telle situation on
observe une forte inflation des \pvalues renvoyées par la régression linéaire de
$\Y$ par $\X$ dans les simulations numérique (Figure ref:fig:method_comp C et D)
ainsi que dans l'EWAS (Figure ref:fig:ewas_qqplot_top A), la GWAS (Figure
ref:fig:gwas_qqplot_top A) et la GEAS (Figure ref:fig:geas_qqplot A) que nous
avons présentés dans ce chapitre. En plus de la forte inflation des \pvalues on
observe également que lm ne permet de trouver les bonnes associations, on
observe ainsi dans les simulations numérique que l'AUC obtenu par lm est
largement inférieur à celui de la méthode oracle (Figure ref:fig:method_comp A
et B). De plus pour les études d'associations à partir de données réelle lm
donne les moins bon résultat quand il s'agit de retrouver les candidats pour
l'association trouvés par ailleurs dans la littérature (Figure
ref:fig:ewas_qqplot_top B et Figure ref:fig:gwas_qqplot_top B).
...
B). On retrouve le fait que les \pvalues sont bien calibré dans les études
...
De plus certain candidats trouvés dans d'autre étude ont trouvé avec des
méthodes similaires à PCAlm cite:Rahmani_2016,Price_2006.
...
correction des étude d'association pour les facteurs latents qui reposent sur
les solutions optimal de problèmes de moindres carré régularisé. Sur les jeux de
données simulées ces méthodes ont les mêmes performances que la méthode oracle
(Figure ref:fig:method_comp), elles sont bien calibré sur les vrais jeux de
données (Figure ref:fig:ewas_qqplot_top A, Figure ref:fig:gwas_qqplot_top A et
...
Figure ref:fig:gwas_venn et Figure ref:fig:geas_venn). Dans les études
d'association considérées ici il n'y a de vérité terrain et une méthodes peut
toujours donner de meilleurs résultats qu'une autre sur des simulations bien
choisis cite:Wolpert_1997. Ceux deux nouvelles méthodes s'ajoutes à l'arsenal
des méthodes permettant de corriger pour les test d'association statistique pour
les facteurs de confusion. Il serait intéressant de mettre en évidence des
situation mettant en défaut une méthodes par rapport à une autre. Il existe
aussi des méthodes permettant de combiner les résultats venant de plusieurs
méthodes, cela permet de d'augmenter la confiance que l'on a en les résultats
d'une étude d'association 

nous on a utilisé l'inter etc... mais il y a plus soffistiqué 
