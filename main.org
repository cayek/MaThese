...
analyse factorielle de la matrice $\Y$. On fait l'analyse factorielle à priori
et sans prendre en compte la variable $\X$. Les variables latentes sont ensuite
ajoutées au modèle de régression au coté des autres variables explicatives de
sorte que
\begin{equation}
...
calculé à partir de la matrice $\Y$. Les modèles mixes ont été largement utilisés
pour les GWAS cite:Kang_2008,Zhou_2014. Dans le cas des GWAS, l'effet aléatoire
permet d'expliquer la variation de $\Y_{j}$ qui est dû à la structure de
population. Dans ce cas la matrice de covariance est estimé à priori sur les
données génétiques.
...
avec $\X$. Les méthodes a contrôles négatifs utilisent donc les variable dites
...
bien des niveaux d'expression génique. La matrice $\X$, de taille $\Xrow \times
\Xcol$, regroupe toutes les variables explicatives. Ainsi les colonnes de $\X$
sont les variables d'intérêt pour l'association, c'est à dire les variables pour
...
indispensable pour séparer la variation de $\Y$ expliquée par les variables
latentes de celle expliquée des variables explicatives. En effet, si
\begin{equation*}
\lambRidge = 0, 
\end{equation*}
...
principale de la matrice $\Y$. Nous expliquons dans la partie [[sec:hyperparametre]]
plus en détail comment choisir l'hyperparamètre $\lambRidge$.
...
nécessitent pas de dupliquer la matrice $\Y$. La matrice $\Y$ est de taille
...
charger la matrice $\Y$ en RAM et d'accéder au données seulement quand cela est
nécessaire.
...
variables $\Y$ et les variables $\X$ afin de pouvoir estimer les variables
latentes. Lorsque le paramètre de régularisation $L_{2}$ tend vers zéro, les
variables $\Y$ et $\X$ sont linéairement décorrélées. Cependant on ne pourra
plus inverser la matrice diagonale $\D$. De plus dans le cas ou les variables
latentes sont trop corrélées avec $\X$ alors on risque de mal estimer celles-ci.
Lorsque le paramètre tend vers l'infini alors la matrice $\D$ tend vers la
matrice identité l'estimation des variables latentes sont données pas l'analyse
en composantes principales de $\Y$, sans prendre en compte $\X$. Le risque est
alors d'expliquer par les variables latentes une partie de la variance de $\Y$
qui devrait être expliquée par $\X$, et dont passer à coté de certaine
associations. Ainsi le choix du paramètre de régularisation $L_{2}$ est une
affaire de dosage, il doit être ni trop grand ni trop petit. Nous avons remarqué
dans les expériences que $\lambdaRidge$ petit donne les meilleurs résultats dans
de nombreux cas.

**** Paramètre de régularisation $L_{1}$
Le paramètre de régularisation $L_{1}$ à un impact sur le nombre de lignes non
nulles dans la matrice des effets $\B$. Seulement une partie des variables $\Y$
est corrélée avec les variables explicatives $\X$. Ainsi, il est possible
...
Nous commençons par calculer les valeurs singulières de la matrice $\Y$, que
l'on note $(\sigma_1, ..., \sigma_{\Yrow})$. Ensuite, nous calculons
\begin{equation}
...
partie de test, c'est a dire que séparons matrices $\Y$ et $\X$ en deux partie
selon leurs lignes. Nous notons $I$ l'ensemble des indices des individus
...
\end{equation}
Cette procédure permet  bien de mesurer une erreur sur des observations de
$\Y$ qui n'ont pas été utilisées pour estimer les paramètres du modèle.
...
Pour chaque variable $\Y_{j}$ nous avons la régression linéaire
suivante
\begin{equation}
...
$\beta_{j}$. Sous ces hypothèses on peut calculer pour chaque variable $\Y_{j}$
une statistique de test $z_{j}$, assimilable à un z-score. La statistique de
test suit sous l'hypothèse nulle le loi de Student à $\Yrow - \K - 1$ degrés de
liberté. On peut donc calculer une \pvalue pour chaque variable $\Y_{j}$. Le
détail du calcul de la statistique de test est donnée dans la section 3.2 de
cite:Hastie_2009.
...
des tests mal calibrés. Dans les exemples que nous présentons ici ont s'attend à
ce que la majorité des variables $\Y_{j}$ ne soit pas associé avec $\X$, ainsi
une large majorité des statistiques de test sont distribuées selon l'hypothèse
nulle. Nous utilisons l'approche choisie dans cite:Sun_2012, qui consiste à
calculer la médiane et la déviation absolue à la médiane (MAD pour median
absolute déviation) directement sur les $z$ statistiques. En effet, la médiane
donne une estimation robuste de la moyenne et le MAD de l'écart type. On a alors
...
méthode obtenir une liste de colonnes de $\Y$ candidate pour l'association avec
...
la méthode cate on commence par transformer la matrice des données $\Y$ afin
d'isoler les variations expliquées par les facteurs latents. Pour cela on
applique une matrice de changement de base aux lignes de $\Y$ de sorte que le
...
** Résultats
:PROPERTIES:
:header-args: :cache no :eval no-export :results output :exports none :session *krakR*
:END:
...
principale de la matrice $\Y$ et ne gardons que les $\K$ premières composantes
en fonction du nombre $\K$ de facteurs de confusion que l'on souhaite
simuler. On a alors
\begin{equation}
\Y = \U \V^{T} + \E
\end{equation}
...
des lignes de $\B^{'}$ soit non nul et simulés grâce à une loi normale. Enfin,
nous calculons la nouvelle matrice des variables à tester tel que
\begin{equation}
\Y^{'} = \U^{'} \V^{T} + \X^{'} \B^{'}^{T} + \E.
\end{equation}
...
simuler une matrice $\Y^{'}$ composé de 52211 variables pour 1758 individus.
Nous avons choisi de simuler 5 variables latentes pour plusieurs valeurs du
nombre de colonnes de $\Y^{'}$ associées avec $\X^{'}$ et plusieurs valeurs de
la corrélation entre $\X^{'}$ et les 5 variables latentes. Pour chaque variable
latente une valeur entre $-1$ et $1$ de la corrélation avec $\X^{'}$ est tirée
selon une loi uniforme et multipliée par un coefficient compris entre 0 et 1. Le
même coefficient est utilisé pour calculer toutes les corrélations. Plus le
coefficient est proche de 1 plus les variable latentes seront corrélées à
$\X^{'}$ et donc plus la confusion lors du test d'association de $\X^{'}$ avec
la matrice $\Y^{'}$ sera importante.

**** Mesure de comparaison des performances
Pour comparer les méthodes entre elles nous avons choisi deux critères. Toutes
les méthodes renvoie une \pvalue pour chaque colonne de $\Y$. Afin d'évaluer la
capacité des méthodes à détecter le plus possible d'association sans se tromper
nous avons calculer l'aire sous la courbe de précision-rappel pour chacune des
méthodes. Nous rappelons que pour une liste de candidats données la précision
est le nombre de vrais associations dans la liste divisé par la taille de la
liste et le rappel est le nombre de vrai association retrouvé dans la liste
divisé par le nombre total des vrais associations, parfois appelé puissance en
statistique. Une méthode qui donne les plus petites \pvalues d'abord à toutes
les vrais associations donne une courbe de précision-rappel de 1. Le deuxième
critère de comparaison des méthode permet d'évaluer la calibration des \pvalues
renvoyées par les méthodes. Pour cela nous calculons le facteur d'inflation sur
\pvalues des variables non associées avec $\X$. Si les \pvalues sont bien
calibrées alors la distribution des \pvalues sous l'hypothèse nulles suit une
loi uniforme et dans ce cas le facteur d'inflation vaux 1. Une méthode qui
renvoie des \pvalues correctement calibrées permet de calculé une liste de
candidats avec un taux de fausse découverte moyen contrôlé à une valeur choisie
en utilisant par exemple l'algorithme de Benjamini-Hoshberg
cite:benjamini1995controlling. Une bonne méthode doit donc à la fois être capable de
detecter les vrais associations sans se tromper mais aussi fournir des \pvalues
correctement calibrer.
...
\caption{Comparaison des méthodes sur des simulations faites a partir du jeux de
  données 1000Genomes. A-B) Aire sous la courbe precision-recall en fonction
  respectivement de la proportion d'outlier dans la simulation et la corrélation
  de la variable explicative et les variables latentes. C-D) Facteur d'inflation
...
#+begin_src R
  ...
  C.pl <- C.pl +
    ...
    D.pl <- gif.rho.pl +
      MaTheseR.params$gtheme +
      theme(legend.position = "none") +
      xlab("Corrélation entre et U et X") + 
      ylab("") 
    ## B
    B.pl <- auc.rho.pl +
      MaTheseR.params$gtheme +
...
*** Étude d'association entre des niveau de méthylation de l'ADN et la polyarthrite rhumatoïde (EWAS)
...
transcription en protéine. Pour cette étude nous avons le au niveau de
...
de l'ADN dépend de l'âge, du sexe et de la consommation de tabac des individus.
Nous savons aussi que le type de la cellule sur laquelle on mesure le nuveau de
méthylation influence le niveau de méthylation. Tous ces facteurs peuvent être
des facteurs de confusion pour l'étude d'association avec la maladie, ils ont
d'ailleurs été pris en compte dans les études d'association qui ont été faites à
partir des même données que celle étudié ici
cite:Rahmani_2016,Zou_2014,Liu_2013. Afin d'évaluer la capacités des méthodes à
bien corriger pour les facteurs de confusion, nous ne prenons pas en compte les
facteurs de confusion connus et nous comparons les résultats à ceux obtenus par
les études cite:Rahmani_2016 et cite:Zou_2014 qui prennent en compte
explicitement les facteurs de confusion connus.
...
l'ADN associés à la polyarthrite. Le nombre de facteurs latents à été choisi à
partir du scree plot et de la validation croisée de l'estimateur $L_{2}$ de LFMM
(voir Figure ref:fig:ewas_params). Nous avons choisi $\K = 10$ car il n'y a plus
d'amélioration significative de l'erreur de prédiction et de la proportion de
variance expliquée par les variables latentes après cette valeur sur les figures
A et B de ref:fig:ewas_params. Les valeurs du paramètre de régularisation
$L_{2}$ $\lambRidge$ entre $10^{-10}$ et $1$ donnent les mêmes valeurs d'erreur
de prédiction (Figure ref:fig:ewas_params C). On s'attend à detecter un petit
nombre d'association nous avons choisi une proportion de ligne non nulle pour
l'estimateur $L_{1}$ de $\B$ valant 1\%. 

#+BEGIN_EXPORT latex
\begin{figure}[!t]
\centering
\includegraphics{./OUTPUT/Rplots/ewas_hyperparams.pdf.png}
\caption{Choix des paramètres pour l'étude d'association entre des site
  méthylation de l'ADN et la maladie polyarthrite rhumatoïde. A) Proportion de
  variance expliquées par chacune des variables latentes estimé par l'algorithme
  ridgeLFMM pour déférente valeur $\lambda$ du paramètre de régularisation
  $L_2$. B)C) Erreur de prédiction calculée grâce à la validation croisée de
  l'estimateur $L_{2}$ des paramètre de LFMM pour différente valeurs du
  paramètre de régularisation $\lambda$ et du nombre variable lattentes $\K$. La
  ligne pointillée vertical marque le nombre de variables latentes choisi, 10.}
...
\pvalues sous l'hypothèse nulle, c'est l'effet entre $\X$ et le niveau de
méthylation de l'ADN vaux zéro. On constate une inflation du nombre de petites
...
une étude d'assocaition de celle-ci avec des données génomiques constitue un bon
test pour les méthode de corréction pour les variable latente. Nous pourrons en
effet comparer nos résultats à ceux des nombreuses autres GWAS de la maladie
\celiac. Nous proposons test d'association entre la maladie \celiac et des
données génétique a partir des données présentées dans cite:dubois2010multiple.
C'est données comporte 281122 SNPs (single nucléotyde polimorphism) pour 15155
individus, 10659 individus sains et 4496 atteint de la maladie \celiac.
Les SNPs sont ...

. De plus nous
s'avons que les données génomique prise dans différentes populations son très
structuré et que cette structure peut être un facteur de confusion . De plus,
nous avons choisi un pour confronter les méthodes à un grand jeux de données.
...
- State "TODO"       from              [2017-08-06 Sun 14:52]
:END:
***** TODO Imputation des données manquantes
:LOGBOOK:
- State "TODO"       from              [2017-08-06 Sun 14:52]
:END:
...
#+begin_src R 
  ...
  pl <- ggplot(res.cv$errs, aes(y = err, x = log(lambda))) +
    geom_smooth()
  pl
  save_plot_png(pl, "gwas_CV_lambda_lfmm_lambda_smooth.png")
...
#+begin_src R 
  ...
  ## cross validation K
  expr <- readRDS("./OUTPUT/Expr/gwas_CV_lfmm.rds")
  toplot <- expr$errs %>%
    mutate(lambda = as.factor(lambda)) %>%
    group_by(lambda, K) %>%
    summarise(err.mean = mean(err), N = length(err), sd = sd(err), se = sd / sqrt(N)) %>%
...
*** Étude d'association entre des données génétique et un gradient environmental (GEAS)
<<sec:GEAS>>
**** Scripts                                                    :noexport:
...
:LOGBOOK:
- State "DONE"       from "TODO"       [2017-08-14 lun. 14:14]
- State "TODO"       from "DONE"       [2017-08-09 mer. 15:18]
...
***** TODO Scaling des données et valeurs manquantes pour GEAS
:LOGBOOK:
- State "TODO"       from              [2017-07-13 jeu. 12:29]
:END:
...
******* RUNNING Scale, remove missing data and export to =matter=
:LOGBOOK:
- Note taken on [2017-08-14 lun. 14:47] \\
  tail -f 1000g_G.y2017_m08_d14.log
- State "RUNNING"    from "TODO"       [2017-08-14 lun. 14:47]
...
#+begin_src R 
  ...
  for(j in 1:ncol(G.r)) {
    nas[j] <- mean(is.na(G.r[kept.indiv.id,j]))
    mus[j] <- mean(G.r[kept.indiv.id,j], na.rm = TRUE)
    sds[j] <- sd(G.r[kept.indiv.id,j], na.rm = TRUE)
  ...
  for(j in notNA.ids {
    writeBin((G.r[kept.indiv.id,j] - mus[j]) / sds[j], con)
  }
  flush(con)
  close(con)
...
#+CAPTION: On scale et enlève les données manquantes. Dépend de [[code:1000g_G_noNA_scaled]] [[code:1000g_prunning]]
#+begin_src R :session *krakR* :results output :dir /scp:cayek@krakenator:~/Projects/Thesis/MaThese/
  library(MaTheseR)
  prune.in <- read_delim(file = "~/Projects/Thesis/MaThese/Data/1000Genomes/Phase3/1000GenomePhase3_CQ.prune.in", delim = " ", col_names = FALSE, col_types = cols(col_character()))
  ...
  keep.id <- colnames(G.matter) %in% prune.in
  G.prunned <- G[,keep.id]
  saveRDS(G.scale, "~/Projects/Thesis/MaThese/Data/ThesisDataset/3Article/1000GenomesPhase3/EAS_G_noNA_scaled_prunned.rds")
#+end_src
...
***** TODO Calcule du gradient climatique
:LOGBOOK:
...
***** TODO Validation croisée du modèle lfmmRidge sur 1000genome
:LOGBOOK:
