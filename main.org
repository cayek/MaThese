...
méthodes statistiques adaptées au traitement de données génétique afin de
...
est d'améliorer les outils statistique qui permettent aux biologistes de répondre
à des question concrète sur le vivant. Dans cette thèse nous nous sommes
intéressé à deux problématique en analyse de données génétiques : l'estimation
de la structure de population et les test d'associations. L'accent a été mis sur
la rapidité des méthodes développées afin qu'elles soient applicables aux
données génétiques modernes. En outre, afin que nos méthodes statistiques soient
utilisables par la communauté scientifique en génétique, il a été important de
rendre accessible une implémentation informatique efficace des nouvelles
méthodes.
...
génome par liaison génétique (LD, linkage disequilibrium). Nous avons donc filtré les SNPs très corrélés
...
une fenêtre de 100 SNPs et cela sur les SNPs ayant un coefficient de corrélation
linéaire au carré qui est supérieur à $0.2$. Cette procédure de prunning nous a
permis d'identifier un sous ensemble de 80275 SNPs. Les opérations de filtrage
ont été effectuées à l'aide du logiciel plink cite:Purcell_2007. Enfin nous
avons utilisé le logiciel beagle pour imputer les données manquantes de la
matrice de SNPs cite:Browning_2016.
...
avant les 5 candidats de la littérature (Table ref:table:ewas).
...
#+begin_src R :results output latex :exports results :session *R*
...
plus libérale est lm alors que PCAlm est la méthode la plus conservative. Les 6
candidats du GWAS catalog sur le chromosome 6 sont ont un rang plus faible avec
les \pvalues renvoyé par PCAlm et ridgeLFMM (Figure ref:fig:gwas_qqplot_top B).
Sur les autre chromosome ce sont les méthodes lassoLFMM et cate qui retrouve le
plus vite les candidats du GWAS catalog (Figure ref:fig:gwas_qqplot_top B).
...
:LOGBOOK:
- State "TODO"       from "DONE"       [2017-09-25 lun. 16:21]
- State "DONE"       from "TODO"       [2017-08-16 mer. 17:40]
- State "TODO"       from              [2017-08-06 Sun 14:52]
:END:
...
#+end_example
***** TODO Filtrage des individus trop apparenté
:LOGBOOK:
- State "TODO"       from "DONE"       [2017-09-25 lun. 16:21]
...
#+begin_example
  [1] 2.356696e-17
#+end_example
***** TODO Conversion au format =matter=
:LOGBOOK:
- State "TODO"       from "DONE"       [2017-09-25 lun. 16:21]
...
***** DONE Bricolage pour enlever le chrm 6
CLOSED: [2017-09-25 lun. 16:33]
:LOGBOOK:
- State "DONE"       from "TODO"       [2017-09-25 lun. 16:33]
- State "TODO"       from              [2017-09-25 lun. 16:24]
:END:

#+NAME: code:gwas_rm_chrm6
#+CAPTION: Dépend de 
#+begin_src R 
  library(MaTheseR)
  library(stringr)
  ...
  Y <- readRDS("./Data/ThesisDataset/3Article/Celiac/G.matter.rds")
  dim(Y)
  ...
  ## adding chrm 6 indices
  chrm6.snps.rs <- (snps.info.df %>% dplyr::filter(chromosome == 6))$marker.ID
  chrm6.snps.rs %>% length()
  col.mask.chrm6 <- union(which(colnames(Y) %in% chrm6.snps.rs), col.mask)

  ## test
  col.mask.chrm6 %>% length()
  col.mask.chrm6 %>% unique() %>% length()
  col.mask %in% col.mask.chrm6 %>% mean()
  str(col.mask.chrm6)
  ## save
  saveRDS(col.mask.chrm6, "./Data/ThesisDataset/3Article/Celiac/ind.clumpling.notChrm6.rds")
#+end_src
...
:END:
****** RUNNING Run des méthodes
:LOGBOOK:
- Note taken on [2017-09-25 lun. 16:42] \\
  j'ai lancé que PCA
- State "RUNNING"    from "TODO"       [2017-09-25 lun. 16:42]
- State "TODO"       from "DONE"       [2017-09-25 lun. 16:23]
...
#+begin_src R
  ...
  run_celiac <- function(m) {
    ...
    col.mask <- readRDS("./Data/ThesisDataset/3Article/Celiac/ind.clumpling.notChrm6.rds")
...
****** TODO Gwas catalogue et controle du fdr
:LOGBOOK:
- State "TODO"       from "DONE"       [2017-09-25 lun. 16:23]
...
***** TODO Annotation de la top liste de lasso LFMM
:LOGBOOK:
- State "TODO"       from "DONE"       [2017-09-25 lun. 16:22]
...
rs10908907 & ridgeLFMM, cate & Alcoholism (heaviness of drinking) \\ 
  ...
  rs17158483 & lassoLFMM & Obesity-related traits \\ 
   \hline
\end{tabular}
\caption{{\bf SNPs associés avec un grariant climatique répertoriés dans la littératures pour être associés avec des phénotypes}. SNPs présents dans l'union des SNPs détectés par les méthodes lassoLFMM, ridgeLFMM, cate et PCAlm pour un le FDR à $1\%$.} 
\end{sidewaystable}
#+END_EXPORT
**** Scripts                                                    :noexport:
...
proportion de variables expliquées associées avec la variable explicative.
...
observé que les \pvalues retournées par lm sont mal calibrées. En outre, la
méthode lm a montré moins de puissance pour retrouver les candidats sur les
données simulées et réelles. Cela montre que la présence de facteurs de
confusion influence la calibration des \pvalues ainsi que leur rang.
...
PCAlm propose moins de candidats passant aussi à coté de certaines associations
potentielles.
...
permanentent de retrouver les candidats trouvées par d'autres étude. Les
...
listes de candidats renvoyés pas ces trois méthodes. Il serait intéressant de
...
simulations bien choisis cite:Wolpert_1997. Nous avons implémenter nos
algorithmes dans un package R. Cela permet aux utilisateurs de les utiliser avec
d'autres algorithmes d'association. Ces deux nouvelles méthodes s'ajoutent ainsi
à l'arsenal des méthodes permettant de corriger les tests d'association
...
papier)
et après je me décale vers ce qui n'a pas été fait et (propose des solutions)
