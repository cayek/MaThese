...
**** Scripts                                                    :noexport:
***** DONE Échantillonage des données
...
méthylation. Nous avons ensuite lancé les méthodes cate, lm, PCAlm, sva,
lassoLFMM, ridgeLFMM afin de trouver les sites de méthylation de l'ADN associés
à la polyarthrite. Le nombre de facteurs latents à été choisi à partir du scree
plot et de la validation croisée de l'estimateur $L_{2}$ de LFMM (voir Figure
ref:fig:ewas_params). Nous avons choisi $\K = 10$ car il n'y a plus
d'amélioration significative de l'erreur de prédiction après cette valeur sur la
figure ....
