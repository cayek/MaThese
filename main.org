...
ref:fig:SNP). Les SNPs représentent $90 \%$ de l'ensemble des variations
...
*** Méthodes de corrections des facteurs de confusion pour les études d'association
...
pour estimer les facteurs de confusion et les intégrer aux tests d'hypothèses
statistiques citep:Rahmani_2016,Price_2006. D'autres méthodes utilisent les
...
#+ATTR_LATEX: :align lllll :float sideways
...
| cate                 | analyse factorielle et régression linéaire            | EM ou SVD et moindres carrés généralisé                             | test basé sur la distribution asymptotique de l'estimateur des effets | citet:wang2015confounder                        |
...
Nos algorithmes ont en commun qu'ils reposes sur des problèmes de factorisation
matricielle.
...
produit d'une matrice des coefficients de métissage et une matrice des
fréquences d'allèle dans les clusters génétiques. L'information spatiale est
ajouté au moyen d'une régularisation sur la matrice des coefficients de
métissage. La régularisation spatiale permet d'ajouter l'hypothèse que des
individus proches ont plus de chance de partager des ancêtres communs, que des
individus éloignés. Afin d'estimer les matrices d'ascendance, nous avons proposé
deux algorithmes appelé AQP et APLS. Nos algorithmes diffèrent dans les
approximations qu'ils font pour diminuer la complexité algorithmique. Plus
précisément, nous avons d'une part l'algorithme AQP qui alterne des résolutions
de problèmes d'optimisation quadratique. Le corollaire 2 établi par
citet:Grippo_2000 permet de montrer la convergeance de l'algorithme AQP vers un
minimum local de la fonction objectif définie par le problème factorisation
matricielle. Nous avons d'autre part, l'algorithme APLS pour lequel nous levons
les contraintes des problèmes d'optimisation quadratique. Cela permet de
transformer d'alterner des problèmes des moindres carrés régularisé par une
norme $L_{2}$. Ainsi, la complexité de l'algorithme APLS augmente linéairement
avec le nombre d'individus dans l'échantillon. De plus, nous avons mis en place
un test de détection de l'adaptation locale. La statistique de test est calculé
à partir des estimations spatiales des matrices d'ascendance génétique.

En utilisant des simulations de coalescantes, nous montrons que les deux
...
en utilisant un modèle bayésien. Sur les simulations que nous avons considérées,
nos algorithmes étaient 10 à 100 fois plus rapides que le logiciel =TESS= 2.3.
...
facteurs de confusions pour l'étude. Pour cela, nous construisons une fonction
objectif à partir l'égalité eqref:eq:model0 du modèle de régression avec facteur
latent, qui décompose en la matrice des variables étudié en la somme de deux
termes : l'effet des variables latentes et l'effet des variables d'intérêt pour
l'étude. L'attache aux données de la fonction objectif est construite à partir
de l'approximation des moindres carrés de l'égalité eqref:eq:model0. Nous
montrons que le terme d'attache au données seul ne permet de définir des
estimateurs de manière univoque pour notre problème. Ainsi, nous proposons
d'ajouter un terme de régularisation portant sur les effets d'intérêts. Nous
appelons nos méthodes ridgeLFMM, pour la régularisation $L_{2}$ et lassoLFMM,
pour la régularisation $L_{1}$. Pour l'agorithme ridgeLFMM, nous utilons la
formule du point de minimum global de la fonction objectif des moindres carrés
régularisé en norme $L_{2}$. Nous apportons la démonstration de cette formule.
Pour l'algorithme lassoLFMM, nous proposons un algorithme alterné de descente
par blocs de coordonnées. Les travaux de citet:Tseng_2001 permettent de
démontrer la congergence de l'algorithme lassoLFMM vers un minimum global de sa
fonction objectif. Cependant, nous apportons une preuve adapté aux résultats de
convergeance de notre algorithme.

Pour évaluer la capacité de nos méthodes a corriger les étude d'assocaition pour
les facteurs de confusions, nous réalisé des simulations a partir d'un jeux de
données réelle issu de génotypes humains. Nous avons ajouté à la comparaisons
une méthode qui ne prend pas en compte les facteurs. Ainsi que des les méthodes
de la litératures: cate, sva-irw et sva-twostep. Nous avons égualement considéré
la méthode calculant les facteus latents à l'aide de l'ACP. Ces simulations nous
ont permit de montrer que nos méthode ridgeLFMM et lassoLFMM ont la même
puissance que la méthode oracle qui connaire les variables latentes de la
simulation. De plus, les tests d'hypothèses statistique obtenus avec nos méthodes
sont correctement calibré. 

Enfin, nous avons illustré l'utilisation de nos méthode sur des études
d'assocaition faire a partir de données réelles. Sur les données réelles, nous
montrons que nos méthodes permettent de retrouver les associations découvert par
d'autres études. De plus, sur dans les études réelles nous observons que malgré
les resemblance conceptuelle entre les méthodes, les assocaitions découvertes
faite par les méthodes peuvent largement varier. Cela met en avant la nécésité
d'utiliser plusieurs méthodes dans les études d'association ainsi que d'etre
...
organismes $d$-ploïde, il existe $(d + 1)$ génotypes possibles à chaque
locus, et chaque valeur est encodée sous une forme disjonctive unique.
...
modéliser les sources de variation indésirable à l'aide de variables latentes.
Dans ce chapitre, nous introduisons deux nouveaux algorithmes qui permettent
d'estimer les variables latentes afin de corriger les études d'association pour
les sources de variations indésirable. Nous appelons ces variables latentes les
facteurs de confusions pour l'étude d'association. Nos algorithmes reposent sur
une décomposition de la matrice des variables étudiés en une matrice des effets
