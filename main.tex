...
\label{sec:org7b990d0}
\chapter{Résumé court}
\label{sec:org98a44b9}
...
d'ascendance génétique dans la Table \ref{tab:org1b3b017}

\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{sidewaystable}[htbp]
\caption{\label{tab:org1b3b017}
...
\ref{fig:orgede099d} illustre cette situation. Le problème des facteurs de
...
\ref{tab:orgb1a0093}, les méthodes reposant sur des modèles construits à
partir de l'équation \eqref{eq:model0} .

\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{sidewaystable}[htbp]
\caption{\label{tab:orgb1a0093}
\textbf{Méthodes reposant sur l'équation \ref{eq:model0} pour la correction des facteurs de confusion dans les études d'association.} Les méthodes ridgeLFMM et LassoLFMM sont présentées dans cette thèse. Les méthodes cate, sva-twostep et sva-riw sont présentées plus en détail dans la section \ref{org41f55bc}.}
\centering
...
\label{sec:orgf07eed8}
\label{orgb08a600}
\section{Résumé}
\label{sec:orgb39728c}
...
\begin{theorem}
\label{orgfd8fa29} L'algorithme AQP qui alterne les étapes d'optimisation des
...
la partie \ref{org3165e52}) qui provoquent la différenciation des distributions
...
\label{sec:orgdfcdd82}
\label{orge03b707}

\section{Résumé}
\label{sec:orgc4907e1}
...
\ref{org41f55bc}. Chacun des algorithmes que nous présentons découle de
...
expliquons plus en détail dans la section \ref{orgbd024c8} comment choisir
...
\ref{org3f5025b}). Nous supposons donc que les variables explicatives \(\X\) ont été
...
\(\D\) utilisée dans l'estimation \(L_{2}\) (voir la section \ref{org5149fa3}). Pour
...
croisée que nous présentons dans la section \ref{org48c7fe8}.

\subsubsection{Paramètre de régularisation \(L_{2}\)}
\label{sec:orge43f8fb}
\label{orgc84f998}
...
section \ref{org83d1639}). La valeur de \(\lambLasso\) correspondante est notée
...
utilisant les notations de la section \ref{org5149fa3}, cela correspond à faire
...
matrice \(\V\) dans notre méthode ridgeLFMM (voir partie \ref{org5149fa3}). Dans
...
\ref{org3b41063} \cite{1000Genome_2015}. Nous avons gardé seulement les chromosomes 1
...
oracle utilise le test d'association présenté dans la partie \ref{org3f5025b} avec
...
\ref{org68c8456}. Le contrôle du FDR a été réalisé à l'aide du package R \texttt{qvalue}
\citep{Storey_2011}.

\subsection{Étude d'association entre des données génétiques et la maladie c\oe{}liaque (GWAS)}
\label{sec:org6a8f780}
\label{orgfb7d23e} 
...
le test d'hypothèse présenté dans la section \ref{org3f5025b} sur les 281122 SNPs,
...
faible fréquence, de la même façon que l'étude GWAS (voir section \ref{orgfb7d23e}).
...
la partie \ref{org41f55bc}. Les méthodes de régression linéaire avec et sans
...
partie \ref{orgc84f998}, \(\lambRidge\) trop grand conduit la méthode ridgeLFMM à
...
chapitre \ref{orgb08a600}, l'effet du bruit dans les données géographiques sur les
