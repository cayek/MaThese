...
\label{sec:orgd2fe06a}
\chapter{Résumé court}
\label{sec:org53ee5c3}
...
\subsection{Méthodes d'estimation de la structure génétique des populations}
\label{sec:org161d2f2}
...
d'ascendance génétique dans la Table \ref{tab:org4b360d8}

\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{sidewaystable}[htbp]
\caption{\label{tab:org4b360d8}
\textbf{Méthodes spatiale et non spatiale d'estimation de coéficients d'ascendance.} Les méthodes TESS3-AQP/APLS sont présentées dans cette thèse.}
\centering
...
TESS3-AQP & factorisation matricielle régularisée sur graphe & oui & optimisation quadratique alterné & \\
...
\ref{fig:org7dec23b} illustre cette situation. Le problème des facteurs de
...
\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{sidewaystable}[htbp]
\caption{\label{tab:orgc9f3397}
\textbf{Méthodes reposant sur l'équation \ref{eq:model0} pour la correction des facteurs de confusion dans les études d'association.} Les méthodes ridgeLFMM et LassoLFMM sont présentées dans cette thèse. Les méthodes cate, sva-twostep et sva-riw sont présentées plus en détail dans la section \ref{org029c972}.}
\centering
\begin{tabular}{p{3cm}|p{4cm}p{4cm}p{5cm}|p{4cm}}
\hline
...
cate & analyse factorielle et régression linéaire & EM ou SVD et estimation par moindres carrés généralisée & test basé sur la distribution asymptotique de l'estimateur des effets d'intérêt & \citet{wang2015confounder}\\
ridgeLFMM & factorisation matricielle avec régularisation \(L_{2}\) & SVD et moindres carrés régularisés en norme \(L_{2}\) & test de Student & \\
lassoLFMM & factorisation matricielle avec régularisation \(L_{1}\) & soft-thresholded SVD et moindres carrés régularisés en norme \(L_{1}\) & test de Student & \\
...
\label{sec:org950bc9e}
Les travaux de cette thèse ont été financés par le LabEx PERSYVAL-Lab et est
co-encadrée par Olivier François du laboratoire TIMC-IMAG et Olivier Michel du
laboratoire GIPSA-lab. Nos travaux ont principalement été réalisés au sein de
l'équipe BCM (Biologie Computationnelle et Mathématique). L'équipe BCM du
laboratoire TIMC-IMAG est spécialisée en étude de données génétiques et en
développement de modèles mathématiques pour les systèmes biologiques complexes.
Les méthodes présentées dans de cette thèse sont donc dans la ligné des méthodes
développées au sein de l'équipe BCM. Ainsi, les logiciels que nous avons
développés sont la suite logique des logiciels développés dans l'équipe BCM,
qu'ils viennent compléter. Nous parlons en particulier de \texttt{TESS} 2.3, un
logiciel d'inférence spatiale de la structure génétique des populations
\citep{Durand_2009}; ainsi que de \texttt{LEA}, un package R proposant une suite de
fonctions dédiées aux études d'association génomique \citep{Frichot_2015}.

\section{Objectifs de la thèse}
\label{sec:org0c8ffdd}
...
\label{sec:org02e0b40}

Dans le cadre de cette thèse, nous avons proposé plusieurs algorithmes résolvant
des problèmes de factorisation matricielle. Nos algorithmes ont été implémentés
...
génotypes \emph{A.thaliana}, chacun incluant plus de 210k SNP. Notre méthode a permis
d'exiber la structure de population de l'espèce \emph{A.thaliana} en Europe. Par
...
\label{sec:org283c5e4}
\label{org9cec4d2}
\section{Résumé}
\label{sec:orgd3d87d5}
...
\subsection{Algorithme d'optimisation quadratique alterné (AQP)}
\label{sec:org7fdac59}
...
\begin{theorem}
\label{org1a85731} L'algorithme AQP qui alterne les étapes d'optimisation des
...
la partie \ref{orgd367563}) qui provoquent la différenciation des distributions
...
\label{sec:orge290071}
\label{org3aea188}

\section{Résumé}
\label{sec:org4de56c5}
...
\ref{org029c972}. Chacun des algorithmes que nous présentons découle de
...
expliquons plus en détail dans la section \ref{org1c31a8f} comment choisir
...
\ref{org1d3b691}). Nous supposons donc que les variables explicatives \(\X\) ont été
...
\(\D\) utilisée dans l'estimation \(L_{2}\) (voir la section \ref{org0e74778}). Pour
...
croisée que nous présentons dans la section \ref{org50e66b5}.

\subsubsection{Paramètre de régularisation \(L_{2}\)}
\label{sec:org85fc33e}
\label{org16acd98}
...
section \ref{orga8eb393}). La valeur de \(\lambLasso\) correspondante est notée
...
\subsection{Régression linéaire simple et avec les scores de l'ACP}
\label{sec:org7298c01}
...
utilisant les notations de la section \ref{org0e74778}, cela correspond à faire
...
matrice \(\V\) dans notre méthode ridgeLFMM (voir partie \ref{org0e74778}). Dans
...
\ref{orgab62a4f} \cite{1000Genome_2015}. Nous avons gardé seulement les chromosomes 1
...
oracle utilise le test d'association présenté dans la partie \ref{org1d3b691} avec
...
\ref{orge0416c5}. Le contrôle du FDR a été réalisé à l'aide du package R \texttt{qvalue}
\citep{Storey_2011}.

\subsection{Étude d'association entre des données génétiques et la maladie c\oe{}liaque (GWAS)}
\label{sec:org671d384}
\label{org219c444} 
...
le test d'hypothèse présenté dans la section \ref{org1d3b691} sur les 281122 SNPs,
...
faible fréquence, de la même façon que l'étude GWAS (voir section \ref{org219c444}).
...
la partie \ref{org029c972}. Les méthodes de régression linéaire avec et sans
...
Enfin, nous calculons les sites de méthylation candidats pour l'association avec
la polyarthrite lorsque l'on contrôle le taux de fausse découverte à \(1\%\). Nous
...
partie \ref{org16acd98}, \(\lambRidge\) trop grand conduit la méthode ridgeLFMM à
...
Enfin, nous calculons les SNPs candidats pour l'association avec la maladie
...
l'association avec le gradient climatique, lorsque nous contrôlons le FDR à \(1
...
chapitre \ref{org9cec4d2}, l'effet du bruit dans les données géographiques sur les
...
\label{sec:orgd79d505}
Dans cette thèse, nous avons développé deux logiciels d'analyse de données
répondant à des problèmes spécifiques de la biologie. Ces logiciels répondent à
l'évolution des données utilisées en biologie. Ainsi, nos travaux s'inscrivent
...
multiples. D'une part, nos travaux ouvrent des perspectives de développements
méthodologiques et logiciels, afin par exemple d'ajouter des fonctionnalités à
nos packages R en fonction des retours des utilisateurs. D'autre part, nos
travaux ouvrent des perspectives sur l'utilisation de la factorisation
matricielle dans d'autres problèmes d'analyse de données massives. Nous pensons
par exemple aux études d'association à partir de données d'expression génique
(données RNA-Seq), ou bien de données de méthylation de l'ADN utilisant les
technologies de séquençage dernière génération.
...
jeux de données Arabidopsis thaliana RegMap. Cinquièmes Rencontres R, 22-24
juin 2016.
