...
\label{sec:org5851b6a}
\label{org0e5a874}
\section{Résumé}
\label{sec:org6002ef6}
...
\begin{theorem}
\label{org01756f3} L'algorithme AQP qui alterne les étapes d'optimisation des
...
\label{sec:org391bd22}
\label{org2babd4b}

\section{Résumé}
\label{sec:orgef84e21}
...
\ref{orge885388}. Chacun des algorithmes que nous présentons découle de
...
expliquons plus en détail dans la section \ref{orge30881a} comment choisir
...
\ref{orgc6f623a}). Nous supposons donc que les variables explicatives \(\X\) ont été
...
\(\Dlambda\) utilisée dans l'estimation \(L_{2}\) (voir la section \ref{org988d8a4}). Pour
...
croisée que nous présentons dans la section \ref{org30b4e51}.

\subsubsection{Paramètre de régularisation \(L_{2}\)}
\label{sec:org3fa3f67}
\label{org8883c1f}
...
section \ref{org41ec2cd}). La valeur de \(\lambLasso\) correspondante est notée
...
utilisant les notations de la section \ref{org988d8a4}, cela correspond à faire
...
matrice \(\V\) dans notre méthode ridgeLFMM (voir partie \ref{org988d8a4}). Dans
...
\ref{org3430f2f} \cite{1000Genome_2015}. Nous avons gardé seulement les chromosomes 1
...
oracle utilise le test d'association présenté dans la partie \ref{orgc6f623a} avec
...
\ref{org1c30b62}. Le contrôle du FDR a été réalisé à l'aide du package R \texttt{qvalue}
\citep{Storey_2011}.

\subsection{Étude d'association entre des données génétiques et la maladie c\oe{}liaque (GWAS)}
\label{sec:org7ce45f8}
\label{org6c5a672} 
...
le test d'hypothèse présenté dans la section \ref{orgc6f623a} sur les 281122 SNPs,
...
faible fréquence, de la même façon que l'étude GWAS (voir section \ref{org6c5a672}).
...
la partie \ref{orge885388}. Les méthodes de régression linéaire avec et sans
...
partie \ref{org8883c1f}, \(\lambRidge\) trop grand conduit la méthode ridgeLFMM à
...
chapitre \ref{org0e5a874}, l'effet du bruit dans les données géographiques sur les
