...
\label{sec:orge621837}
\chapter{Résumé court}
\label{sec:orgf8a08e9}
...
d'ascendance génétique dans la Table \ref{tab:org9ce7787}

\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{sidewaystable}[htbp]
\caption{\label{tab:org9ce7787}
...
\ref{fig:org49e6309} illustre cette situation. Le problème des facteurs de
...
\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{sidewaystable}[htbp]
\caption{\label{tab:org5d14b63}
\textbf{Méthodes reposant sur l'équation \ref{eq:model0} pour la correction des facteurs de confusion dans les études d'association.} Les méthodes ridgeLFMM et LassoLFMM sont présentées dans cette thèse. Les méthodes cate, sva-twostep et sva-riw sont présentées plus en détail dans la section \ref{org51f5869}.}
\centering
...
lassoLFMM & factorisation matricielle avec régularisation \(L_{1}\) & \emph{soft-thresholded} SVD et moindres carrés régularisés en norme \(L_{1}\) & test de Student & \\
...
dans de cette thèse sont donc dans la ligné des méthodes développées au sein de
l'équipe BCM. Ainsi, les logiciels que nous avons développés sont la suite
logique des logiciels développés dans l'équipe BCM, qu'ils viennent compléter.
Nous parlons en particulier de \texttt{TESS} 2.3, un logiciel d'inférence spatiale de
la structure génétique des populations \citep{Durand_2009}; ainsi que de \texttt{LEA}, un
package R proposant une suite de fonctions dédiées aux études d'association
génomique \citep{Frichot_2015}.

\section{Objectifs de la thèse}
\label{sec:org9954a33}
...
\label{sec:org3161ae4}
\label{orgaf63d8f}
\section{Résumé}
\label{sec:orgdd52ba8}
...
\begin{theorem}
\label{orge165dc9} L'algorithme AQP qui alterne les étapes d'optimisation des
...
la partie \ref{org9bbe912}) qui provoquent la différenciation des distributions
...
\label{sec:org7ec10f9}
\label{orga417daa}

\section{Résumé}
\label{sec:org151ebf7}
...
\ref{org51f5869}. Chacun des algorithmes que nous présentons découle de
...
expliquons plus en détail dans la section \ref{orgdac79ed} comment choisir
...
\ref{org689a3f6}). Nous supposons donc que les variables explicatives \(\X\) ont été
...
\(\D\) utilisée dans l'estimation \(L_{2}\) (voir la section \ref{org04bcca7}). Pour
...
croisée que nous présentons dans la section \ref{org50f15c3}.

\subsubsection{Paramètre de régularisation \(L_{2}\)}
\label{sec:org523c262}
\label{orga8f115f}
...
section \ref{org5fa5781}). La valeur de \(\lambLasso\) correspondante est notée
...
utilisant les notations de la section \ref{org04bcca7}, cela correspond à faire
...
matrice \(\V\) dans notre méthode ridgeLFMM (voir partie \ref{org04bcca7}). Dans
...
\ref{org9f65187} \cite{1000Genome_2015}. Nous avons gardé seulement les chromosomes 1
...
oracle utilise le test d'association présenté dans la partie \ref{org689a3f6} avec
...
\ref{org1f21ab4}. Le contrôle du FDR a été réalisé à l'aide du package R \texttt{qvalue}
\citep{Storey_2011}.

\subsection{Étude d'association entre des données génétiques et la maladie c\oe{}liaque (GWAS)}
\label{sec:org5228e68}
\label{orged6b489} 
...
le test d'hypothèse présenté dans la section \ref{org689a3f6} sur les 281122 SNPs,
...
faible fréquence, de la même façon que l'étude GWAS (voir section \ref{orged6b489}).
...
la partie \ref{org51f5869}. Les méthodes de régression linéaire avec et sans
...
partie \ref{orga8f115f}, \(\lambRidge\) trop grand conduit la méthode ridgeLFMM à
...
chapitre \ref{orgaf63d8f}, l'effet du bruit dans les données géographiques sur les
