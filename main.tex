...
\label{sec:orgf83ab82}
\label{org0344a7c}
\section{Résumé}
\label{sec:org2874716}
...
\begin{theorem}
\label{org774d07a} L'algorithme AQP qui alterne les étapes d'optimisation des
...
\label{sec:orgd851fdf}
\label{org01aaf2d}

\section{Résumé}
\label{sec:orgffb112a}
...
\ref{orgb35595a}. Chacun des algorithmes que nous présentons découle de
...
expliquons plus en détail dans la section \ref{org543a596} comment choisir
...
\ref{org9ebcd03}). Nous supposons donc que les variables explicatives \(\X\) ont été
...
\(\Dlambda\) utilisée dans l'estimation \(L_{2}\) (voir la section \ref{org3b0ce73}). Pour
...
croisée que nous présentons dans la section \ref{orgd4100a1}.

\subsubsection{Paramètre de régularisation \(L_{2}\)}
\label{sec:org86acd4c}
\label{orgb7db4e3}
...
section \ref{org4f4fcab}). La valeur de \(\lambLasso\) correspondante est notée
...
utilisant les notations de la section \ref{org3b0ce73}, cela correspond à faire
...
matrice \(\V\) dans notre méthode ridgeLFMM (voir partie \ref{org3b0ce73}). Dans
...
\ref{org03aed39} \cite{1000Genome_2015}. Nous avons gardé seulement les chromosomes 1
...
oracle utilise le test d'association présenté dans la partie \ref{org9ebcd03} avec
...
\ref{orgd7e2f33}. Le contrôle du FDR a été réalisé à l'aide du package R \texttt{qvalue}
\citep{Storey_2011}.

\subsection{Étude d'association entre des données génétiques et la maladie c\oe{}liaque (GWAS)}
\label{sec:org4cd54fb}
\label{orgf973767} 
...
le test d'hypothèse présenté dans la section \ref{org9ebcd03} sur les 281122 SNPs,
...
faible fréquence, de la même façon que l'étude GWAS (voir section \ref{orgf973767}).
...
la partie \ref{orgb35595a}. Les méthodes de régression linéaire avec et sans
...
partie \ref{orgb7db4e3}, \(\lambRidge\) trop grand conduit la méthode ridgeLFMM à
...
chapitre \ref{org0344a7c}, l'effet du bruit dans les données géographiques sur les
