...
\label{sec:orgb551b9a}
\chapter{Résumé court}
\label{sec:org2b198d6}
...
\end{equation}
où \(\Pr(\Y_{i,j} = j)\) est la probabilité d'observer l'allèle \(j\) au locus
...
d'ascendance génétique dans la Table \ref{tab:org7bebfc3}

\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{sidewaystable}[htbp]
\caption{\label{tab:org7bebfc3}
...
\ref{fig:orge1880de} illustre cette situation. Le problème des facteurs de
...
\ref{tab:org2162177}, les méthodes reposant sur des modèles construits à
partir de l'équation \eqref{eq:model0} .

\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{sidewaystable}[htbp]
\caption{\label{tab:org2162177}
\textbf{Méthodes reposant sur l'équation \ref{eq:model0} pour la correction des facteurs de confusion dans les études d'association.} Les méthodes ridgeLFMM et LassoLFMM sont présentées dans cette thèse. Les méthodes cate, sva-twostep et sva-riw sont présentées plus en détail dans la section \ref{org5d1b509}.}
\centering
...
\label{sec:orgac9d0cb}
\label{org50e7a1d}
\section{Résumé}
\label{sec:org390d1e4}
...
de fréquences de génotypes dans les \(K\) clusters génétiques \(\mathbf{G}\), de
taille \((d+1)p \times K\). Nous notons \(\Q_{i,k}\) le coefficient de métissage de
l'individu \(i\) pour le cluster \(k\). Nous avons de plus
\begin{equation}
\label{eq:QConst}
...
\begin{theorem}
\label{org2b397dd} L'algorithme AQP qui alterne les étapes d'optimisation des
...
la partie \ref{org5b1ba9f}) qui provoquent la différenciation des distributions
...
\label{sec:org70282c8}
\label{org8bf67c0}

\section{Résumé}
\label{sec:org3f57c1d}
...
\ref{org5d1b509}. Chacun des algorithmes que nous présentons découle de
...
expliquons plus en détail dans la section \ref{org3810d0d} comment choisir
...
\ref{org1bbb465}). Nous supposons donc que les variables explicatives \(\X\) ont été
...
\(\Dlambda\) utilisée dans l'estimation \(L_{2}\) (voir la section \ref{org618b818}). Pour
...
croisée que nous présentons dans la section \ref{org2f36de7}.

\subsubsection{Paramètre de régularisation \(L_{2}\)}
\label{sec:org15fe7f8}
\label{orge327094}
...
section \ref{orgbf585ac}). La valeur de \(\lambLasso\) correspondante est notée
...
utilisant les notations de la section \ref{org618b818}, cela correspond à faire
...
matrice \(\V\) dans notre méthode ridgeLFMM (voir partie \ref{org618b818}). Dans
...
\ref{orgac425a2} \cite{1000Genome_2015}. Nous avons gardé seulement les chromosomes 1
...
oracle utilise le test d'association présenté dans la partie \ref{org1bbb465} avec
...
\ref{org9c1b1c2}. Le contrôle du FDR a été réalisé à l'aide du package R \texttt{qvalue}
\citep{Storey_2011}.

\subsection{Étude d'association entre des données génétiques et la maladie c\oe{}liaque (GWAS)}
\label{sec:org3f65387}
\label{orgd93181a} 
...
le test d'hypothèse présenté dans la section \ref{org1bbb465} sur les 281122 SNPs,
...
faible fréquence, de la même façon que l'étude GWAS (voir section \ref{orgd93181a}).
...
la partie \ref{org5d1b509}. Les méthodes de régression linéaire avec et sans
...
partie \ref{orge327094}, \(\lambRidge\) trop grand conduit la méthode ridgeLFMM à
...
chapitre \ref{org50e7a1d}, l'effet du bruit dans les données géographiques sur les
