...
\label{sec:org44f4df0}
\label{org8240c05}
\section{Résumé}
\label{sec:org9bc1a3a}
...
\begin{theorem}
\label{org3edf996} L'algorithme AQP qui alterne les étapes d'optimisation des
...
\label{sec:orgf645b97}
\label{orgec812b9}

\section{Résumé}
\label{sec:orgaeb1b78}
...
\ref{orgfc4b20d}. Chacun des algorithmes que nous présentons découle de
...
expliquons plus en détail dans la section \ref{org4ecfee9} comment choisir
...
\ref{orgc9f32f7}). Nous supposons donc que les variables explicatives \(\X\) ont été
...
\(\Dlambda\) utilisée dans l'estimation \(L_{2}\) (voir la section \ref{org4bbf425}). Pour
...
croisée que nous présentons dans la section \ref{orga0fbdbe}.

\subsubsection{Paramètre de régularisation \(L_{2}\)}
\label{sec:org114391a}
\label{orgc37495f}
...
section \ref{org2089026}). La valeur de \(\lambLasso\) correspondante est notée
...
utilisant les notations de la section \ref{org4bbf425}, cela correspond à faire
...
matrice \(\V\) dans notre méthode ridgeLFMM (voir partie \ref{org4bbf425}). Dans
...
\ref{org5078c9b} \cite{1000Genome_2015}. Nous avons gardé seulement les chromosomes 1
...
oracle utilise le test d'association présenté dans la partie \ref{orgc9f32f7} avec
...
\ref{org6328342}. Le contrôle du FDR a été réalisé à l'aide du package R \texttt{qvalue}
\citep{Storey_2011}.

\subsection{Étude d'association entre des données génétiques et la maladie c\oe{}liaque (GWAS)}
\label{sec:org4ff02dd}
\label{org2835926} 
...
le test d'hypothèse présenté dans la section \ref{orgc9f32f7} sur les 281122 SNPs,
...
faible fréquence, de la même façon que l'étude GWAS (voir section \ref{org2835926}).
...
la partie \ref{orgfc4b20d}. Les méthodes de régression linéaire avec et sans
...
partie \ref{orgc37495f}, \(\lambRidge\) trop grand conduit la méthode ridgeLFMM à
...
chapitre \ref{org8240c05}, l'effet du bruit dans les données géographiques sur les
