...
GENELAND & bayésien & oui & MCMC & \citet{GUEDJ_2011}\\
...
\label{sec:orga36a93d}
\label{org2109bcf}
\section{Résumé}
\label{sec:orgeebe0ca}
...
\begin{theorem}
\label{org3c9bdb9} L'algorithme AQP qui alterne les étapes d'optimisation des
...
statistiques \$ F\^{}Q\_\{\rm ST\}\$ pour effectuer des tests statistiques de neutralité
...
\label{sec:org8bede89}
\label{org059d8c1}

\section{Résumé}
\label{sec:org37c82e0}
...
\ref{org16ad365}. Chacun des algorithmes que nous présentons découle de
...
expliquons plus en détail dans la section \ref{orgabe594b} comment choisir
...
\ref{orgefb27ab}). Nous supposons donc que les variables explicatives \(\X\) ont été
...
proposition 2.7.1 de \citet{Bertsekas_1997} (ce dernier démontrant la convergence
de l'algorithme de descente par blocs de coordonnées dans le cas où la fonction
...
\(\Dlambda\) utilisée dans l'estimation \(L_{2}\) (voir la section \ref{org9d67a91}). Pour
...
croisée que nous présentons dans la section \ref{org122fb3a}.

\subsubsection{Paramètre de régularisation \(L_{2}\)}
\label{sec:orgf099958}
\label{orgcb313b7}
...
section \ref{org165f77c}). La valeur de \(\lambLasso\) correspondante est notée
...
utilisant les notations de la section \ref{org9d67a91}, cela correspond à faire
...
matrice \(\V\) dans notre méthode ridgeLFMM (voir partie \ref{org9d67a91}). Dans
...
\ref{orgbe42d97} \cite{1000Genome_2015}. Nous avons gardé seulement les chromosomes 1
...
oracle utilise le test d'association présenté dans la partie \ref{orgefb27ab} avec
...
\ref{orga133db1}. Le contrôle du FDR a été réalisé à l'aide du package R \texttt{qvalue}
\citep{Storey_2011}.

\subsection{Étude d'association entre des données génétiques et la maladie c\oe{}liaque (GWAS)}
\label{sec:org0569557}
\label{org8e5cad6} 
...
le test d'hypothèse présenté dans la section \ref{orgefb27ab} sur les 281122 SNPs,
...
faible fréquence, de la même façon que l'étude GWAS (voir section \ref{org8e5cad6}).
...
la partie \ref{org16ad365}. Les méthodes de régression linéaire avec et sans
...
partie \ref{orgcb313b7}, \(\lambRidge\) trop grand conduit la méthode ridgeLFMM à
...
fois plus d'annotation \emph{HIGH}, 8 fois plus de \emph{MODERATE} et \(1.7\) fois plus de
\emph{LOW} dans les 836 SNPs que parmi les 5397214 SNPs. Nous avons de plus testé si
...
chapitre \ref{org2109bcf}, l'effet du bruit dans les données géographiques sur les
