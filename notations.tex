...
\newcommand{\pvalue}{$p$-valeur} %% \pvalue
...
\newcommand{\lambRidge}{\lambda_{2}} %% reg devant norme L2
\newcommand{\lambLasso}{\lambda_{1}} %% reg devant norme L1
...
\Llasso(\W, \B) =  \frac{1}{2} \norm{\Y - \W - \X \B^T}_{F}^2 + \lambdaLasso \norm{\B}_{1} + \gamma \norm{\W}_{*}%
}
\newcommand{\LfmmP}{%
\obP = \Id_{\Yrow} - \X (\X^T \X + \lambRidge \Id_{\Xcol})^{-1}
\X^T%
}
